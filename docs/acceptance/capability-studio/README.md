...
| Governed Baseline Schema | [`capability-studio-governed-baseline-v1.schema.json`](../../schemas/resource-gateway-capability-studio/capability-studio-governed-baseline-v1.schema.json) | 固定 GP-07/08 开发验证的 9 × 3 矩阵、失败关闭和三项必须显示的限制 | `DEVELOPMENT_TEST_OWNED / NO_GO` |
...
- `POST /api/capability-studio/governed-baseline` 已成为 GP-07/08 Tool 页的真实运行入口。该 test/staging-only 端点无调用方 payload 和身份覆盖，复用上述受治理 compiler/Registry/suite runtime，严格要求 3 个唯一 suite run、27 个唯一 child run、9 个 Case 每轮恰好一次、全部通过、三轮 publication/provenance/source-map 不漂移且进程内真实调用为 0。成功投影仍强制 `DEVELOPMENT_TEST_OWNED / NO_GO`，并固定显示“业务结果指纹未导出、部署级 egress 未观测、Owner 未签署”三项限制；失败关闭响应不伪造 publication、Run 或 fingerprint。严格公开 Schema、独立 `CapabilityStudioGovernedBaselineVerifier`、production 装配否定、篡改负向和真实 Spring 集成均已自动化验证；
- GP-07/08 已在 Tool 页上提供“运行 9 × 3 受治理验证”主操作。运行中、原位失败恢复、开发通过/发布不可验收双结论、3 轮 suite 摘要、9 × 3 Case 矩阵、3 项限制和折叠技术证据已实现；真实 Chrome 已在中文 1440 × 1100 与 390 × 844 视口校验 DOM 基数、无横向溢出和 axe serious/critical 为 0；
...
仍未完成的是持久化且具备权限边界的 Dataset Authority、Feature 的字段级 source map 与企业身份授权、每 Case 每轮业务结果 fingerprint 和逐 Case Oracle/timeout/duplicate/forbidden-write 的受治理协议导出、不可变候选 build 与运行环境 fingerprint 绑定、部署级 network deny/egress 观测、契约与 Tutorial 全键盘/人工读屏/并发浏览器矩阵、GP-07/08 的英文与 1024 视口、GP-09/10 的开发切片和六人可用性签署。当前 `/scenario-dataset` 是 Stage 0 Golden Demo Pack 的不可变业务投影，不是客户生产 Dataset 的写入 Authority。当前页面已调用同一受治理编译/注册/执行链路，但服务端按协议强制将它标记为 `DEVELOPMENT_TEST_OWNED / NO_GO`，因此不能替代 Owner 签署的 `S0-AC-04` 证据。
