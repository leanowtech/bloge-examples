...
> 当前状态：`NO_GO`
>
> 本目录记录 Capability Studio 的治理边界、验收合同、证据追踪和未通过实例。GP-01 至 GP-08 已形成 Stage 0 开发切片，三项技术 Spike 均已有部分开发证据；GP-09/10、不可变发布候选验收、部署级 egress 和人工签署仍未闭合。任何局部完成都不表示整体产品验收通过，也不替代产品与架构签署。
...
| Stage Acceptance Result Schema | [`capability-studio-stage-acceptance-result-v1.schema.json`](../../schemas/resource-gateway-capability-studio/capability-studio-stage-acceptance-result-v1.schema.json) | 固定 `S*-AC-*` 单条验收结果、五项统一前置条件、证据和签署状态机 | 可消费，不代表任何合同已通过 |
...
| Governed Baseline Schema | [`capability-studio-governed-baseline-v2.schema.json`](../../schemas/resource-gateway-capability-studio/capability-studio-governed-baseline-v2.schema.json) | 固定 GP-07/08 开发验证的 9 × 3 矩阵、逐 Case 业务 Oracle/semantic fingerprint、失败关闭和五项必须显示的限制 | `DEVELOPMENT_TEST_OWNED / EXPLORATORY / NO_GO` |
...
- `POST /api/capability-studio/governed-baseline` 已成为 GP-07/08 Tool 页的真实运行入口。该 test/staging-only 端点无调用方 payload 和身份覆盖，复用上述受治理 compiler/Registry/suite runtime，严格要求 3 个唯一 suite run、27 个唯一 child run、9 个 Case 每轮恰好一次、全部通过、三轮 publication/provenance/source-map 不漂移且进程内真实调用为 0。Suite 完成后，服务端通过既有授权 API 回读每条完整签名 child evidence，校验 run/target/Fixture/integrity，导出 payload-free evidence fingerprint、semantic result fingerprint、断言与 Fixture 控制计数；同 Case 三轮结果指纹必须一致，timeout/duplicate/forbidden-write 由结构化事实形成专项证明。成功投影强制 `DEVELOPMENT_TEST_OWNED / EXPLORATORY / NO_GO`，固定显示不可变候选、环境认证、可认证证据、部署级 egress、Owner 签署五项限制；失败关闭响应不伪造 evidence class、publication、Run 或 fingerprint。v2 严格 Schema、独立 `CapabilityStudioGovernedBaselineVerifier`、production 装配否定、篡改负向和真实 Spring 集成均已自动化验证；
- GP-07/08 已在 Tool 页上提供“运行 9 × 3 受治理验证”主操作。运行中、原位失败恢复、开发通过/发布不可验收双结论、3 轮 suite 摘要、9 × 3 Case 矩阵、9/9 业务 Oracle、27/27 业务断言、三项专项证明、`EXPLORATORY` 等级、五项发布阻断和折叠技术证据已实现；本轮真实浏览器在中文 1280 × 720 与 390 × 844 视口复验，页面和结果表均无横向溢出，三轮信息完整可读；既有自动化 Chrome 矩阵继续覆盖中文 1440 × 1100、390 × 844 与 axe serious/critical 为 0；
...
仍未完成的是持久化且具备权限边界的 Dataset Authority、Feature 的字段级 source map 与企业身份授权、不可变候选 build 与运行环境 fingerprint 绑定、从 `EXPLORATORY` 升级为 `CERTIFIABLE` 的运行证据、部署级 network deny/egress 观测、契约与 Tutorial 全键盘/人工读屏/并发浏览器矩阵、GP-07/08 的英文与 1024 视口、GP-09/10 的开发切片和六人可用性签署。当前 `/scenario-dataset` 是 Stage 0 Golden Demo Pack 的不可变业务投影，不是客户生产 Dataset 的写入 Authority。当前页面已调用同一受治理编译/注册/执行链路并闭合开发 Oracle，但服务端按协议强制将它标记为 `DEVELOPMENT_TEST_OWNED / EXPLORATORY / NO_GO`，因此不能替代 Owner 签署的 `S0-AC-04` 证据。
