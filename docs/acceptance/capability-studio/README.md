...
仍未完成的是持久化且具备权限边界的 Dataset Authority、把新受治理发布/执行边界接入浏览器 9×3 发布候选运行、已声明 Tool 嵌套依赖的独立认证结果、Feature 的字段级 source map 与身份授权、把该闭包的 Graph/Contract/Dataset/Binding exact refs 固化进三轮发布候选 Run Evidence、部署级 network deny/egress 观测、运行环境 fingerprint、契约与 Tutorial 全键盘/人工读屏/并发浏览器矩阵、GP-07 至 GP-10 的开发切片和六人可用性签署。当前 `/scenario-dataset` 是 Stage 0 Golden Demo Pack 的不可变业务投影，不是客户生产 Dataset 的写入 Authority。当前一轮 Spring 同闭包集成是开发证据；Feature Rehearsal、Tool 微图和页面 9×3 基线仍使用 test-owned runtime material。在页面与候选 API 切换前，它们不能替代 Owner 签署的 `S0-AC-04` 证据。
