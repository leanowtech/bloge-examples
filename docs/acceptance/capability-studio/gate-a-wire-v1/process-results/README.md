...
```text
Gate A fixtures valid: 33 positive, 24 negative
```

The validator resolves the non-business common vocabulary locally, checks every Schema with Draft 2020-12, discovers and validates every `valid-*` fixture without a hardcoded positive-file allowlist, performs process time and A2 cross-document byte/time closure checks, and prints the exact matched keyword/path or semantic code for every `invalid-*` fixture.
