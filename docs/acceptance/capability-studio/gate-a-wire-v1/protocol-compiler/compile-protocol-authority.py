...
def main() -> None:
    ...
    if not authority_path.exists():
        raise SystemExit(f"AUTHORITY_NOT_FOUND: {authority_path}")
    if not SCHEMA_ROOT.exists():
        raise SystemExit(f"SCHEMA_ROOT_NOT_FOUND: {SCHEMA_ROOT}")

    if args.self_test:
        _run_self_test(authority_path, output_root)
    else:
        _run_compile(authority_path, output_root)


def _run_compile(authority_path: pathlib.Path, output_root: pathlib.Path) -> None:
    try:
        ...
        raise SystemExit(exc.detail or exc.CODE)
...
def _run_self_test(
    ...
    def do_compile(root: pathlib.Path) -> None:
        compile_authority(
            ...
            output_root=root,
            schema_root=SCHEMA_ROOT,
            overwrite=True,  # temp dirs already exist
        )

    # Two independent temp directories for double-compile verification
    ...
    # Write verified output (overwrite to support repeated self-test runs)
    compile_authority(
        authority_path=authority_path,
        output_root=output_root,
        schema_root=SCHEMA_ROOT,
    ...
    # Verify manifest source fingerprint matches authority
    manifest = json.loads(
    ...
    # Verify all expected projection files present
    expected_names = {
