...
|---|---|---|---|---|---|
| `GP-01` | `/capabilities/`；4/1/1/9 Demo Pack | 组件、Loader/Controller；本地干净候选固定 60 格真实 Chrome 矩阵覆盖中英文 1440/1024/390，页面级无横向溢出和内部状态泄漏 | 异常状态完整浏览器矩阵、CI Candidate/Environment Authority、产品签署；英文演示数据仍保留中文权威业务名称 | Product + QA | `PARTIAL` |
...
| `NFR-02` | 双语、三视口、完整键盘路径 | 双语组件测试；本地干净候选固定 60 格全部执行，真实 Chrome 记录每格实际视口、键盘完成度、页面溢出、axe serious/critical、技术 ID/Raw JSON 泄漏和 P0/P1；60/60 通过、0 跳过、0 P0/P1 | 人工屏幕阅读器、错误/冲突/断网状态的完整三视口矩阵、CI Candidate/Environment Authority；业务资产内容未本地化 | UX + QA | `PARTIAL` |
