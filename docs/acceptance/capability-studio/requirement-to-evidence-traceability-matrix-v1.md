...
| `GP-07` | Tool 业务契约、精确依赖摘要和“9 场景 × 3 轮”业务正确性目标；技术证据默认折叠 | Tool 组件/协议测试；真实 Chrome 从 Tool 任务打开契约并展示唯一主操作；中文 1440/390 与英文 1024，无溢出且 axe serious/critical 为 0 | 契约 round-trip、依赖影响报告、中文 1024、英文 1440/390、完整键盘/读屏矩阵和 Product/Runtime 签署 | Product + Runtime | `PARTIAL` |
...
| `GP-09` | test/staging-only 质量与影响 API 由同一 Dataset 确定性投影：9 `DRAFT`、0 `ACTIVE`、0 `STALE`，五项覆盖 100%，新鲜度 `UNVERIFIED`，准入 `BLOCKED`；两个 blocker 同屏可解释；37 节点/81 边闭合 9 Case 的 Source、Oracle、Contract、四个 runtime dependency 与 Target；每 Case 影响资产为 6，孤立 Case 为 0；页面明确未导出 Payload 不等于已语义脱敏 | 公开严格 v1 Schema；服务端确定性/授权/配置/基数测试；独立 Test Kit 对真实 wire bytes 复算 projection fingerprint、exact-ref/Scope/多 Authority/排序/图闭包/准入/汇总并递归拒绝 Payload 字段；前端严格 parser、认证 purpose、业务文案、选择/恢复测试；真实 Spring + Chrome 中文 1440/390 与英文 1024 验证 9 Case、37/81、所选路径 9 节点/8 边、五项 100%、两个 blocker、无横向溢出和 axe serious/critical 为 0；英文 1024 已闭合仅键盘 Case 切换以及真实 503、业务化错误说明、Retry 后恢复，页面不泄漏内部协议码；Browser 插件在 1280×720 再验证场景切换为 9 节点/8 边且页面溢出为 0；启动脚本探测同一协议事实 | 持久化 Dataset/freshness Authority、Active 生命周期与审批、客户级数据分类和语义脱敏证明；中文 1024、英文 1440/390、异常状态其余两视口、人工读屏、六人可用性和 Data Owner 签署 | Data Owner | `PARTIAL` |
| `GP-10` | Tool 的 9 × 3 Case 矩阵可打开原 child run 的精确证据；test/staging-only GET 只读已持久化 Run，输出 Tool/Contract/Dataset/Case/runtime target/Binding Plan/Fixture/Behavior/依赖/source map/provenance 与结构级 Data Lens 完整闭包；URL 固定 `task/runId/scenarioId/nodeId` | v1 严格 Schema；独立 Test Kit 对真实 wire bytes 重算引用闭包、Binding Plan/Data Lens/projection fingerprint、焦点和 Payload 边界；服务端证明重复读取 bytes/fingerprint 相同且不重跑；合同漂移、篡改、错误 Case、未知 Run、越权均失败关闭；真实 Spring + Chrome 从 Tool 矩阵打开 timeout child，进入精确 Feature 子图、刷新并返回，保持原 Run/Case/Node；中文 1440/1280/390 与英文 1024 无页面横向溢出；英文路径再次断言原 Run、Case、焦点、DAG 对齐和 axe serious/critical 为 0 | 目标环境 Candidate attestation、部署级 egress、客户级 ABAC/Scope Authority、中文 1024、英文 1440/390、异常恢复、人工读屏和 QA/Integration 签署 | QA + Integration | `PARTIAL` |
...
| `NFR-02` | 双语、三视口、完整键盘路径 | 双语组件测试；中英文 1440/1024/390 Chrome 开发切片；真实 Tab/Enter/Space Dataset 路径、Feature 权限切换和英文 1024 Quality Case 选择；六种组件状态、GP-09 503/Retry、GP-10 精确证据/DAG 和真实 Chrome axe-core serious/critical 为 0；已修复选中行、精确 DAG 上下文文字对比度、DAG 滚动区焦点和错误态内部协议码泄漏 | 固定 60 格仍未全部执行；契约与 Tutorial 的完整键盘路径、人工屏幕阅读器、异常状态 1440/390；业务资产内容未本地化 | UX + QA | `PARTIAL` |
