...
| `GP-05` | 现有 Graph/Canvas 可复用；`TestRunEvidence` 的权限化 Data Lens 后端投影已实现 | 既有 layout 单元测试；structure-only/payload-visible、稳定坐标、首差异、重试/回退、截断和确定性投影测试 | 取消争议 Feature Graph、Data Lens API/画布、字段血缘和浏览器像素验收 | Canvas Owner | `PARTIAL` |
| `GP-06` | 现有 TIMEOUT lowerer 可复用；9 个 Canonical Case metadata 已映射到 test-owned material，并经真实 `TestRunService` 路径验证 connector counter 为 0 | TIMEOUT/RETURN/顺序消费/MUST_NOT_CALL/ERROR、禁止 fallback-to-real 和 fail-fast HTTP connector observer 测试 | 运行 SPIKE-A 产出的同一批注册资产、Feature Oracle、复杂 DAG 和部署级 network deny | Runtime Owner | `PARTIAL` |
...
| `SPIKE-B` | Data Lens 能否可读且不泄露 Payload | 字段来源、边值、差异、权限投影和无遮挡 | 直接投影既有 `TestRunEvidence.nodeTrace/edgeTrace`；structure-only 隐藏值但保留 fingerprint，payload-visible 显示受权值；稳定 graphPath/invocationSite/correlation/occurrence、状态、重试/回退、首个运行时差异、确定性排序、上限/截断与 payload 容器冻结测试通过 | 尚无 Capability API、取消争议 Feature Graph、字段映射/血缘、画布交互与浏览器像素证据 | `PARTIAL` |
...
| `SEC-02` | 隔离运行静默访问真实服务 | `realExternalCallCount=0` 且 egress deny | Capability Studio in-process 真实 TestRun 路径使用 fail-fast HTTP delegate observer，9 个 metadata Case 的 connector counter 为 0；fallback-to-real 在执行前拒绝 | 缺发布候选环境 network deny、OS/sidecar 计数和复杂 DAG 全调用点闭包 | Security + Runtime | `PARTIAL` |
...
| `NFR-02` | 双语、三视口、完整键盘路径 | 双语组件测试；中英文 1440/1024/390 Chrome；真实 Tab/Enter/Space Dataset 路径；六种组件状态和真实 Chrome 完整 axe-core serious/critical 为 0；已修复选中行对比度缺陷 | 契约与 Tutorial 的完整键盘路径、人工屏幕阅读器、异常状态三视口；业务资产内容未本地化 | UX + QA | `PARTIAL` |
