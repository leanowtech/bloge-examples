...
| `NFR-03` | 三次运行语义 fingerprint 一致 | payload-free 开发基线固定执行 9×3；每个 Case 三轮 semantic/business fingerprint 一致，27 个 `runId` 唯一；两批并发仍隔离 | 尚未绑定发布候选 exact refs、环境 fingerprint、部署级 egress 和 Owner 签署 | Runtime | `PARTIAL` |
