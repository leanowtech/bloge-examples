...
| `GP-09` | 当前 Dataset 已有每 Case 的 Owner、Source、Oracle、Contract 和 runtime-control dependency；黄金样例的真实状态为 9 个 `DRAFT`、0 个 `ACTIVE`，五项覆盖均为 100%，但没有可信来源复核时间 | 既有 Loader/Scenario Dataset verifier 已证明基础引用闭包；待新增 Quality/Impact v1 Schema、独立 Test Kit 对 exact-ref/内容指纹/图基数/排序/准入矛盾/Payload 泄漏的复算，以及服务端和前端选择测试 | 独立质量与影响投影、Case 到 Source/Oracle/Contract/dependency/Target 的影响图、`NO_ACTIVE_CASES` 与 `FRESHNESS_EVIDENCE_MISSING` 阻断回显、中英文/桌面/移动/键盘/读屏证据、Data Owner 签署；`PAYLOAD_NOT_EXPORTED` 不得冒充源数据已语义脱敏 | Data Owner | `NO_GO` |
| `GP-10` | Tool 的 9 × 3 Case 矩阵可打开原 child run 的精确证据；test/staging-only GET 只读已持久化 Run，输出 Tool/Contract/Dataset/Case/runtime target/Binding Plan/Fixture/Behavior/依赖/source map/provenance 与结构级 Data Lens 完整闭包；URL 固定 `task/runId/scenarioId/nodeId` | v1 严格 Schema；独立 Test Kit 对真实 wire bytes 重算引用闭包、Binding Plan/Data Lens/projection fingerprint、焦点和 Payload 边界；服务端证明重复读取 bytes/fingerprint 相同且不重跑；合同漂移、篡改、错误 Case、未知 Run、越权均失败关闭；真实 Spring + Chrome 从 Tool 矩阵打开 timeout child，进入精确 Feature 子图、刷新并返回，保持原 Run/Case/Node；中文 1440/1280/390 无页面横向溢出 | 目标环境 Candidate attestation、部署级 egress、客户级 ABAC/Scope Authority、英文/1024/异常恢复/人工读屏和 QA/Integration 签署 | QA + Integration | `PARTIAL` |
