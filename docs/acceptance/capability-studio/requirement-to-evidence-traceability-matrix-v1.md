...
| `GP-08` | Tool 页调用 test/staging-only `POST /api/capability-studio/governed-baseline`，复用同一受治理 compiler/Registry/exact-suite runtime；页面同时展示 9/9 场景、9/9 Oracle、3/3 轮次、27/27 业务断言、0 真实调用、3 suite、9 × 3 Case 矩阵、稳定业务指纹、三项专项证明和“开发通过/发布不可验收”双结论 | v2 严格公开 Schema；真实 Spring 产生 3 个唯一 suite run、27 个唯一 child run，固定 9 Case 每轮恰好一次且全部 `PASSED`；通过授权 API 回读完整签名 child evidence，闭合 target/Fixture/integrity/semantic fingerprint/断言/Fixture 控制；timeout fallback、duplicate 幂等、forbidden-write 无写入成立；publication/provenance/source-map 三轮稳定，进程内真实外呼为 0；独立 Test Kit 验证成功、失败关闭和篡改 | Evidence 当前为 `EXPLORATORY`；另缺不可变候选 build、运行环境认证、部署级 egress 和 QA/Runtime/正确性 Owner 签署 | QA + Runtime | `PARTIAL` |
...
| `NFR-03` | 三次运行语义 fingerprint 一致 | 受治理 Tool v2 基线产生 3 suite/27 child 唯一 Run，publication/provenance/source-map 三轮一致；每 Case 三轮 `semanticResultFingerprint` 一致并由独立 Test Kit 复验 | Evidence 仍为 `EXPLORATORY`；另缺候选 build、环境认证、部署级 egress 和 Owner 签署 | Runtime | `PARTIAL` |
