...
| `GP-08` | Tool 页调用 test/staging-only `POST /api/capability-studio/governed-baseline`，复用同一受治理 compiler/Registry/exact-suite runtime；页面同时展示 9/9、3/3、27/27、0、3 suite、9 × 3 Case 矩阵和“开发通过/发布不可验收”双结论 | 严格公开 v1 Schema；真实 Spring 产生 3 个唯一 suite run、27 个唯一 child run，固定 9 Case 每轮恰好一次且全部 `PASSED`；publication/provenance/source-map 三轮稳定，进程内真实外呼为 0；独立 Test Kit 验证成功、失败关闭和篡改；中文 1440/390 真实 Chrome | 每 Case 每轮业务结果 fingerprint、逐 Case Oracle/timeout/duplicate/forbidden-write 受治理导出、不可变候选 build、运行环境 fingerprint、部署级 egress 和 QA/Runtime/正确性 Owner 签署 | QA + Runtime | `PARTIAL` |
...
| `NFR-03` | 三次运行语义 fingerprint 一致 | 旧 payload-free Oracle 基线固定执行 9 × 3 且每 Case semantic/business fingerprint 一致；新受治理 Tool 基线产生 3 suite/27 child 唯一 Run，publication/provenance/source-map 三轮一致并由独立 Test Kit 验证 | 两类证据尚缺每 Case 每轮业务结果 fingerprint 的同一受治理协议导出；另缺候选 build、环境 fingerprint、部署级 egress 和 Owner 签署 | Runtime | `PARTIAL` |
