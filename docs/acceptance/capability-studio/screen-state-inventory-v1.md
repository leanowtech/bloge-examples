...
| `GP-04` | Tutorial Branch 行为编辑器 | 以「当什么条件、依赖如何表现、持续多久」编辑；保存生成数据库 revision；预检显示未解析依赖、真实调用和 fallback | 409 保留未保存值并可重载最新版本；错误摘要与恢复按钮自动进入视口，焦点可达；断网与非法响应显示原因、影响和恢复动作 | `data-testid=capability-tutorial-branch`；数据库 CAS/重建/漂移测试；中文实际保存/预检；英文 1024 Chrome；Test Kit 实际 HTTP 互验；组件自动滚动/焦点测试；`7112211d4` canonical candidate 的 ERROR/OFFLINE/CONFLICT 三格真实 viewport 几何、键盘恢复、axe 和候选 exact binding 通过 | `PARTIAL_DEVELOPMENT_VERIFIED_CONFLICT_RECOVERY` |
...
| `GP-09` | 场景数据中心 → 质量与影响 | 首屏并列覆盖事实与准入结论：五项覆盖均为 100%，但 9 个 Case 均为 `DRAFT`、0 个 `ACTIVE`、新鲜度 `UNVERIFIED`，所以准入 `BLOCKED`；选择 Case 后只呈现其 Source、Oracle、Contract、runtime dependency 与 Target 影响闭包；0 个孤立 Case | `NO_ACTIVE_CASES`、`FRESHNESS_EVIDENCE_MISSING`、Schema/权限/网络失败均显示发生了什么、影响和恢复动作；失败时不以本地常量补图；错误态不显示内部协议码；页面明确「当前投影不导出 Payload」不等于源数据已经语义脱敏 | `data-testid=capability-quality-impact`；严格 v1 Schema/Test Kit；真实 wire fingerprint/闭包/37 节点/81 边/9 Case 基数；中文 1440/390 与英文 1024 真实 Chrome；英文 1024 仅键盘选择、真实 503/Retry；Case 切换后 9 节点/8 边高亮；axe serious/critical 为 0；无横向溢出；Browser 插件 1280×720 交互复核 | `PARTIAL_DEVELOPMENT_VERIFIED_QUALITY_IMPACT` |
| `GP-10` | Tool 精确证据 + Feature Deep Link | 从 9 × 3 矩阵读取原 child run，不重跑；展示 Tool、Contract、Dataset、Case、runtime target、Binding Plan、Fixture、Behavior、依赖、source map、provenance 和结构级 Data Lens；进入 Feature 后聚焦原节点，刷新与返回保持同一 Run/Case/Node | 未知 Run/Case 返回可恢复 404；合同漂移、指纹篡改、引用缺失或越权时失败关闭，不展示部分可信结果 | `data-testid=capability-governed-run-evidence` / `feature-dag`；严格 v1 Schema、独立 Test Kit、真实 Spring + Chrome；中文 1440/1280/390 与英文 1024，无页面横向溢出、DAG 对齐且 axe serious/critical 为 0 | `PARTIAL_DEVELOPMENT_VERIFIED_EXACT_READ` |
...
| [`capability-studio-gp07-gp08-governed-tool-en-1024.png`](../../assets/capability-studio/capability-studio-gp07-gp08-governed-tool-en-1024.png) | 英文 1024 Tool 契约与 9 × 3 双结论、三轮和 Case 矩阵；固定 60 格机器矩阵另闭合双语三视口正常态 | 异常恢复三视口、人工键盘与读屏、CI Authority 和 Owner 签署 |
| [`capability-studio-gp09-quality-admission-zh-1440.png`](../../assets/capability-studio/capability-studio-gp09-quality-admission-zh-1440.png) | 中文桌面同屏呈现五项 100%、9/0/0、两个业务阻断与 Payload 边界；真实响应直达 UI，页面无横向溢出且 axe serious/critical 为 0；固定 60 格机器矩阵另闭合双语三视口正常态 | 正式数据准入、语义脱敏、异常恢复三视口、人工读屏、CI Authority 与 Data Owner 签署 |
...
| [`capability-studio-gp09-quality-keyboard-en-1024.png`](../../assets/capability-studio/capability-studio-gp09-quality-keyboard-en-1024.png) | 英文 1024 仅键盘切换 Case 后的 9 节点/8 边闭包 | 其余固定矩阵、人工读屏和六人测试 |
| [`capability-studio-gp09-quality-error-en-1024.png`](../../assets/capability-studio/capability-studio-gp09-quality-error-en-1024.png) | 真实 503 业务错误说明、无内部协议码、单一 Retry 动作 | 1440/390 错误态与并发故障 |
| [`capability-studio-gp09-quality-recovered-en-1024.png`](../../assets/capability-studio/capability-studio-gp09-quality-recovered-en-1024.png) | Retry 后恢复同一真实质量投影 | 重试风暴、熔断与人工签署 |
...
| [`capability-studio-gp10-exact-graph-context-zh-1440.png`](../../assets/capability-studio/capability-studio-gp10-exact-graph-context-zh-1440.png) | Deep Link 上下文保持；Feature DAG 只呈现同一 graph path 的 6 个业务节点，完整 7 节点 Data Lens 保留外层 Tool 运行；固定 60 格机器矩阵另闭合双语三视口正常态 | 人工视觉签署、异常恢复和更复杂嵌套图 |
...
| [`capability-studio-gp10-exact-evidence-en-1024.png`](../../assets/capability-studio/capability-studio-gp10-exact-evidence-en-1024.png) | 英文 1024 原 child run、Case 和引用闭包保持；固定 60 格机器矩阵另闭合双语三视口正常态 | 异常恢复三视口、人工读屏、CI Authority 和 Owner 签署 |
| [`capability-studio-gp10-exact-dag-en-1024.png`](../../assets/capability-studio/capability-studio-gp10-exact-dag-en-1024.png) | 英文 1024 同一 Run/Case/Node 的精确 Feature DAG 与 Data Lens | 更复杂嵌套图和人工视觉签署 |
