...
Status: Implemented for resource-gateway-examples core loop · Scope: resource-gateway-examples · Extends `docs/bloge-visual-orchestration-*` (ADR-001..010)
...
**Conclusion:** not greenfield. The backend mock-run, builtin export, React authoring canvas, and React showcase migration gaps are now closed for the example's core loop; remaining work is production hardening and future physical separability. Greenfield is rejected — it would discard a tested schema engine and re-derive solved JSON Schema problems.
...
- **Phase 2 Authoring UI (completed for core loop):** static Custom Composer now has core-loop HUD + Simulate overlay/trace badges; React Flow has operator-library validate/import intake, palette grouping/facets + Cmd-K search, typed handles, deterministic Auto Layout, palette click-or-drag placement, candidate preflight, selected-node connection guide, fixture editors, explicit output selection, server-authoritative Validate action with readiness/action diagnostics, output-aligned simulate requests, local draft JSON export with `nodeFixtures`, and a result summary that surfaces terminal output, real/mock trust, fixtures, and diagnostics before the raw trace. The in-canvas coach now explains the next action and can open compatible target discovery directly for disconnected nodes. React JS core-loop coverage verifies pasted library validation/import, catalog refresh, palette exposure, Cmd-K filtering, click add-to-canvas, drag/drop placement, target discovery, direct connect, Auto Layout, coach-driven target discovery, draft validation feedback, simulation summary feedback, and draft export payloads. The `-Pfrontend` Selenium smoke now verifies the packaged `/author/` React bundle boots, loads the operator palette, adds by click, validates the transient draft, and adds by HTML5 drop onto the canvas. Future work is polish/parity, not backend feasibility. *Verify:* Selenium/JS e2e core loop.
...
- **Phase 4 Showcase migration (completed for parity scope):** React `/showcase/` loads the backend scenario catalog, preserves the seven-scenario order, exposes run/sample/decision-table metadata, renders the same diagram endpoint with selectable node summaries, runs editable sample inputs through the public gateway endpoints, shows request execution receipts, shows preset expectation matched/missing feedback, and shows SSE stream lanes with stop control for the streaming scenario. Future work is fine-grained legacy runner feedback polish rather than major capability parity. *Verify:* scenario catalog + diagram + request/stream runner parity.

## 9. Testing
Unit (generator, classification, fixture non-semantics), integration (mixed-graph simulate, security caps, tenant isolation), React JS (`npm test`) for author/showcase core loops, Selenium e2e (`/author` and `/showcase` via `mvn -Pfrontend ...`), portable golden snapshot determinism/import, and final `mvn clean verify`.
