...
- **Phase 4 Showcase migration (in progress):** React `/showcase/` now loads the backend scenario catalog, preserves the seven-scenario order, exposes run/sample/decision-table metadata, renders the same diagram endpoint with selectable node summaries, and runs editable sample inputs through the public gateway endpoints with an output panel. Remaining work is stream UX and the finer runner feedback parity from the legacy `/examples/gateway` page. *Verify:* scenario catalog + diagram + request runner parity.
