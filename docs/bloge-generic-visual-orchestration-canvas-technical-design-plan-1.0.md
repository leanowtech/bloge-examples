...
- **Phase 4 Showcase migration (in progress):** React `/showcase/` now loads the backend scenario catalog, preserves the seven-scenario order, exposes run/sample/decision-table metadata, renders the same diagram endpoint with selectable node summaries, and is packaged beside `/author/`. Remaining work is runner parity with the legacy `/examples/gateway` page. *Verify:* scenario catalog + diagram parity.
