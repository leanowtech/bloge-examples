...
- **Phase 4 Showcase migration (in progress):** React `/showcase/` now loads the backend scenario catalog, preserves the seven-scenario order, exposes run/sample/decision-table metadata, renders the same diagram endpoint with selectable node summaries, runs editable sample inputs through the public gateway endpoints, shows preset expectation matched/missing feedback, and shows SSE stream lanes with stop control for the streaming scenario. Remaining work is fine-grained legacy runner feedback polish rather than major capability parity. *Verify:* scenario catalog + diagram + request/stream runner parity.
