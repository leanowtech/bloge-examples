...
4. 同一份 `.bloge + 合法 schema view` 可以通过 `POST /api/visual/dsl-imports/rewrite-gate` 或 `/author/` `Check Rewrite` 得到 source replacement allow/block 结论。
...
| DSL rewrite gate API | 已落地 | `DslImportControllerTest.rewriteGateAllowsSupportedTransformDsl` / `rewriteGateBlocksSemanticDrift` 覆盖 `ALLOW_REWRITE` 与 `BLOCK_SEMANTIC_DRIFT` |
| `/author/` Check Rewrite | 已落地 | `AuthorCanvas.test.tsx` 点击 `legacy-dsl-rewrite-gate` 后断言显示 `Rewrite gateALLOW_REWRITE` 和 `Auto rewrite allowed` |
...
```json
{
  ...
  "rewriteGateDecision": "ALLOW_REWRITE / BLOCK_SEMANTIC_DRIFT / BLOCK_INCOMPLETE_EVIDENCE / BLOCK_IMPORT_DIAGNOSTICS",
  "sourceMapRefs": 6,
...
| 源码 writer / VCS 集成未落地 | rewrite gate 已能阻止不等价自动替换，但系统还不会直接修改业务代码库里的 `.bloge` | 增加源码回写 API、VCS PR bot 或 Studio 批量迁移器，并强制消费 rewrite gate 结论 |

结论：本轮把“合法 schema + DSL 可以在通用画布渲染、审阅源码映射、保存为 governed stored draft，并预检 generated DSL 是否可安全替换源文件”的核心路径闭合到 API 和浏览器单元测试层；后续差距集中在 framework/Studio 复用化、复杂语义保留和真正的源码 writer / VCS 集成。
