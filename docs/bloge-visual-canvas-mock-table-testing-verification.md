# BLOGE Visual Canvas Mock/Table Testing Verification
...
| 为 mock 数据提供明确产品化支持 | Done | 既保留节点级 `Mock Setup` / `Output Pin` / `Expected Input`，又新增 Test Table 行级 fixture override |
| 为表格测试提供明确产品化支持 | Done | `/author/` inspector 新增 `Test Table`，支持 add/remove/edit/run/clear 和逐行状态 |
| 表格测试不依赖真实下游 API | Done | Test Table 调用现有 transient simulate；`VisualGraphSimulationService` 对非 primitive 节点使用 `SimulationOperator` mock，行级 fixture 覆盖本次 request |
| 示例内置复杂测试路径，用户不用从空白摸索 | Done | 3 个示例各带 2 条 table cases：happy path + fixture override 分支 |
| 更新操作手册 | Done | `docs/bloge-visual-canvas-product-and-system-guide.md` 新增 Test Table 使用说明和与后端 suite/golden 的关系 |
| 自动化验证 | Done | 前端 pure helper、React DOM 交互、后端表达式函数测试均已覆盖 |

## 2. 设计决策

### 2.1 Test Table 放在画布内，而不是另起测试工具
...
Mock Setup 基础 nodeFixtures
  -> Test Table 行级 fixtureOverrides
...
Test Table 不在浏览器里解释 DSL，不私自执行 graph，也不绕过服务端 schema/readiness 规则。每行仍调用：
...
### 2.3 Test Table 是 authoring runner，不替代治理级 suite

画布内 Test Table 解决的是“作者快速构造和调试多路径测试”。治理级资产仍走已有后端能力：

| 层级 | 能力 |
| --- | --- |
| `/author/` Test Table | 快速调试、演示、构造路径 |
...
| `resource-gateway-examples/src/main/frontend/src/AuthorCanvas.tsx` | 新增 Test Table UI、行编辑、批量运行、结果展示、示例加载自动带入测试行 |
| `resource-gateway-examples/src/main/frontend/src/styles.css` | 新增 Test Table 样式和 passed/failed/running 状态 |
...
| `docs/bloge-visual-canvas-product-and-system-guide.md` | 新增 Test Table 使用说明、fixture merge 规则、工业化测试分层 |
...
复杂示例
  ...
  -> Test Table 多 case fixture override
  -> transient simulate
...
1. Test Table 还没有一键保存为后端 stored suite 或 publication golden case。
2. Test Table 目前只做 exact terminal output assertion，尚未在 UI 内暴露 JSON Pointer、schema assertion、numeric tolerance 等高级断言模式。
3. Test Table 运行结果尚未落入 run history/HTML trend report；CI 侧应继续使用后端 batch runner。
