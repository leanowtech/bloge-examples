...
| 详情浮层展示 input/output schema 与专有属性 | Done | `SchemaPortCards` 展示每个端口的 JSON Schema；详情区展示 operatorRef、source、lowering、readiness、property、config preview |
...
| 文件 | 变更 |
| --- | --- |
| `resource-gateway-examples/src/main/frontend/src/AuthorCanvas.tsx` | 新增 `OperatorDetailDialog`、`SchemaPortCards`、`ForeachLoopGuide`；统一节点双击打开详情；Test Suite 改为右侧摘要按钮 + 浮层表格 |
| `resource-gateway-examples/src/main/frontend/src/AuthorCanvas.test.tsx` | 覆盖普通节点双击详情、foreach 循环向导、decision table 嵌入详情、Test Suite 浮层入口和运行路径 |
...
3. Operator Detail 目前展示 schema preview，尚未提供 schema diff、sample payload 对照和端口级数据探针。
