...
4. 存量业务迁移专线：补 opaque diagnostics 修复向导、semantic round-trip，并提供 `bloge.capabilityCatalog.v1` 到 visual library 的 adapter 作为推荐迁移路径；schema-neutral preview/commit 和 source map 行定位已经落地。
