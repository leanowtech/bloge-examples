...
| 能力域 | 当前状态 | 代码证据 |
| --- | --- | --- |
| 用户算子库导入 | 已落地。支持 `bloge.visualOperatorLibrary.v1`、生命周期、policy、schema/lowering/DSL 可寻址性校验、`oneOf`/`anyOf` visual union schema、H2 持久化、跨库 `operatorRef` 归属保护，以及面向 stored draft 和 immutable publication 的 replacement/delete impact preflight；same-ref fingerprint drift warning 会带 changed-surface 摘要，说明 input/output/config schema、capability、policy 或 lowering 哪个面发生变化。 | `visual/catalog/OperatorLibrary.java`、`OperatorLibraryValidator.java`、`OperatorLibraryAdminController.java`、`OperatorDefinitionChangeSummary.java`、`DatabaseOperatorLibraryRegistry.java` |
