...
| Java OperatorRegistry 深化 | 已支持基础投影、typed DTO/record 运行、普通/streaming/suspendable Java source kind 可见性、`UnionSchema` 到 visual `oneOf` 的保守连接约束，以及 operator surface drift 摘要工具；高级泛型展示、streaming UX、suspend/resume authoring 和更丰富注解语义仍需深化。 | 下一阶段 |
