...
4. **Run history / Golden 深化**：当前已经保存 shape-only run record，并补了查询过滤、浏览器最近运行列表、基础 SLO stats、shape-only run trace、publication 级 golden case、suite run、latest certification、stale-aware promotion-readiness status、基础 golden assertions 和 output-schema assertions；下一步可补 tolerance 断言、节点级耗时/错误聚合和事件流回放。
