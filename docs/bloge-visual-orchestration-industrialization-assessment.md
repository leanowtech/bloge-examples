...
| 画布产品化体验 | 9.9 | Browser Composer palette、schema-aware picker、hover preflight、readiness panel、diagnostic queue、impact inspector，前端本地 schema type/validator mirror 已覆盖 required-only object 与 contains-only array；selected-node Connectability 直接展示服务端候选 schema 类型、替换影响、target runtime-binding debt，并在候选窗口被截断时显式提示 partial server window / local fallback 风险，已提供 Prev/Next 候选窗口控制，并支持按 target/reason/schema 文本发起服务端全局 query、按 ready/blocked/wired 做服务端全局状态过滤并返回 `statusCounts`；服务端候选结果现在返回 `facetCounts` 和候选行级 `facetValues`，覆盖 surface、schemaType、operatorRef、operatorLibraryId、runtimeReadiness、sourceKind、loweringMode，Connectability 已把这 7 个维度全部暴露为交互式服务端 facet filter，且 facet 统计口径保持在 query/status 之后、facet filter 与分页之前；selected-node Connectability 现在还提供 Endpoint/source-handle 筛选，多输出算子可把可见行和服务端候选请求收窄到单个输出端点；候选行渲染已窗口化，默认和过滤状态都限制首批展示并用行级 overflow chip 显示被裁剪的 ready/match 数，overflow 后的行级 Prev/Next 可继续浏览同一 source 的后续 chip，箭头键可在 action 之间推进并跨行级窗口前进，且服务端当前窗口候选会优先排在 local fallback 前；大量 source handle 现在也有 source-row 窗口，默认只展示 8 个 source endpoint，并把服务端候选请求 scope 收敛到当前可见 source rows，真实浏览器已覆盖 40 输出端口 source 的 1-8、9-16、17-24、25-32、33-40 窗口切换、server source key 收敛和 390px 尾页无横向溢出；候选 row、target action、row-window/source-window 控件现在带稳定 DOM id、`aria-labelledby`/`aria-describedby`、`aria-controls`、`aria-live`、`aria-posinset`/`aria-setsize` 与焦点态 `aria-activedescendant`，真实浏览器覆盖首窗口、行内 Next 和方向键跨窗口后的 active descendant 更新；YAML 用户算子库导入 / palette / diagram / selected operator inspector、OpenAPI Resource Contract preview/save 面板、AsyncAPI discovery/selection/projection/import 面板、Draft/Publication/Golden/Run History lifecycle 面板均已在 390px mobile emulation 下通过 no-overflow 回归，同一 260 target 回归还在 390px mobile emulation 下断言 Connectability panel/filter/targets/action 与页面级横向滚动无溢出；真实浏览器还覆盖 filter 输入重绘后的焦点保留、状态筛选、Clear 恢复，以及 260 个 target 下的服务端窗口、行级 24 个 chip 上限、overflow 提示、行级 Next 到 25-48、键盘右箭头推进到 49-72、服务端全局 query/status/schema/lowering facet 命中末尾 target、Next 翻页和无横向溢出；复杂用户算子 schema outline 已在 selected operator contract panel 中覆盖 nested object、patternProperties、additionalProperties、contains、oneOf/anyOf、dependentRequired、prefixItems 和 annotation，并用浅层优先 + 高风险 schema 语义优先排序、24 行窗口与 overflow 提示避免复杂 schema 把 inspector 撑爆；contract panel 现在还提供跨 input/output 的 `Schema Search`，按完整 outline 池过滤 path/kind/type/detail 后再窗口化展示，可命中默认 24 行之外的深层 path，并保留重绘后的输入焦点和值；draft dependency report 现在输出 schema drift `path`、`savedType`、`currentType`、`reviewHint`、bounded `schemaChanges`、bounded `schemaPreview` 和 `schemaRebaseDecisions`，selected operator contract panel 会把当前节点的 breaking/compatible schema drift 叠到 port summary、outline row、并排 Frozen/Current Schema Review 以及 Schema Rebase Queue 上，搜索 `breaking`、`type: number -> integer`、`frozen integer` 或 `rebase` 可定位 drift row；Draft Dependencies 可按 `ready-rebase`、`ready-capture`、`repair-review`、`blocked` 展示 rebase 决策队列，并通过现有 revision-guarded rebase API 批量刷新 eligible 节点；schema outline/search/drift 区现在也带稳定 DOM id、`aria-controls`、`aria-describedby`、`aria-live`、`role=list/status/listitem`、`aria-posinset`/`aria-setsize` 和 drift label 描述关联，真实浏览器覆盖复杂 schema outline、drift overlay/side-by-side preview、decision queue、批量 rebase 和 390px mobile no-overflow；服务端候选接口已把大窗口成本限制到当前页，并通过 `targetSurface=canvas` 与画布 target handle 语义对齐 | 单文件前端复杂度高，复杂 schema outline 已覆盖正向审阅、path 搜索、draft-dependency drift overlay、字段级 type transition/review hint/keyword diff、bounded side-by-side frozen/current schema preview、schema rebase 决策队列主路径和基础 a11y/DOM 合同，但仍缺逐字段 accept/reject merge IDE、负路径/漂移矩阵和跨资源/AsyncAPI/OpenAPI drift 联动，query/status/facet filter 已下沉到服务端候选合同并覆盖当前服务端 facet 维度，Endpoint filter 已能收窄单 source，source-row 和候选行都已窗口化并有真实浏览器证据，候选区与 schema 合同区已有基础 a11y 状态合同，用户自带 schema/协议定义的主要入口和核心生产生命周期面板已有 390px 移动视口回归，但还不是完整虚拟化列表，也没有完整 axe/VoiceOver/NVDA 级无障碍审计；大图回归仍缺完整移动矩阵、更极端 source handle、多算子族性能矩阵 | 抽更小 UI 模块，补逐字段 schema merge IDE、真正虚拟化候选列表、完整移动布局矩阵、完整 a11y audit、更极端 source handle 和多 operator family 大画布回归 |
...
这个分数不是贬低当前成果。相反，它说明项目已经跨过“画布玩具”阶段，并且在“严肃生产级可用示例项目 / industrializable reference implementation”口径下已经进入 2.6% 左右差距；但它仍不是完整商业低代码平台，平台化阶段还差治理、runtime、观测和维护性闭环。
...
20. selected-node Connectability source rows 已窗口化：默认只展示 8 个 source endpoint，source-window key 与 query/status/facet/source filter 隔离，并把 `/api/visual/connections/candidates` 的请求 scope 收敛到当前可见 source rows；真实浏览器 40 输出端口 source 回归覆盖 1-8、9-16、17-24、25-32、33-40 source rows，证明 server `resultsBySourceKey` 只包含当前窗口 source keys，并在 390px mobile emulation 下复核尾页 source window controls、source rows 和页面级横向滚动无溢出。
...
33. schema-neutral DSL preview/commit import 已落地到后端和 `/author/`：`POST /api/visual/dsl-imports/preview` 接受 `.bloge` 源码、已导入 catalog id 或 preview-only inline visual libraries，复用官方 DSL parser 投影为 `GraphDraft + sourceMap + coverage + diagnostics`；没有 operator/function schema 时也会先从 DSL AST 推演 topology-only draft，保留普通 node、transform、decision_table、输入绑定、输出引用、data/dependency/route edge、函数调用名和 graph input/output schema，并在 `draft.visualLayout.import` 记录 `projectionMode=topology-only`、`schemaPrecision=inferred`、operator/function 引用和缺失清单。`POST /api/visual/dsl-imports/commit` 接受同一 request，服务端重新投影后保存 governed stored draft/revision，并返回 validation/dependency report。missing operator/function 现在是 warning diagnostics，而不是渲染级 error；浏览器 Legacy DSL 面板可以 Render DSL、Commit Draft、展示 source map 行列表、点击 source map 行选中画布节点，并在导出 draft 中保留 `visualLayout.import.sourceMap` 与 stored draft identity。
...
38. 协议草案已补齐 schema-neutral DSL import/rewrite 的当前 wire contract：`docs/bloge-visual-orchestration-protocol-v1.md` 现在显式记录 `bloge.dslVisualProjection.v1`、`bloge.dslRewriteGate.v1`、`bloge.visualGraphDraftImportResult.v1`、GraphDraft input/output schema、topology-only projection metadata、旧 layout output schema 兼容边界，以及 rewrite semantic fingerprint 以一等 `draft.outputSchema` 为准。剩余协议债务从“核心 DSL/GraphDraft 合同缺失”降为“全量 wire contract 长期维护”。
39. DSL 批量迁移报告 API 已落地：`POST /api/visual/dsl-imports/batch-report` 接受多份 `.bloge` DSL、已导入 library ids 和 preview-only inline visual libraries，逐份复用 single-source preview 与 rewrite gate，返回 `bloge.dslImportBatchReport.v1`。报告包含 source 级 `renderable`、`fullyProjected`、`needsRepair`、coverage、round-trip、rewrite decision 和 diagnostics，也包含仓库级 source count、render/repair/block count、rewrite allow/block count、coverage totals、round-trip status counts、rewrite decision counts 和 diagnostic level counts。它把存量业务从“逐文件 UI 试导入”推进到“CI/迁移前批量评估”。
40. DSL 批量迁移保存 API 已落地：`POST /api/visual/dsl-imports/batch-commit` 接受与 batch-report 相同的 `sources[]` 和 schema-neutral catalog view，并按 `renderable`、`fully-projected` 或 `rewrite-allowed` 策略批量保存 governed draft。响应 `bloge.dslImportBatchCommitResult.v1` 同时返回 commit summary、`COMMITTED_*` / `SKIP_*` / `FAILED_PERSISTENCE` 计数、每个 source 的 report item 和 `GraphDraftImportResult`，让迁移脚本可以一次性产出可审阅 GraphDraft，而不是循环调用单源 commit 或只停在 readiness report。
41. Legacy DSL 导入现在把“可理解”和“可精确执行/安全改写”分成两层：topology-only projection 面向业务理解和迁移审阅，不要求完整 schema；schema-backed projection 面向 schema-aware validation、模拟、执行、发布和 rewrite safe 证据。这让存量业务可以先把手写 DSL 的整体拓扑可视化出来，再渐进补齐 operator/function schema，而不是被 schema acquisition 卡在入口前。
...
6. 缺失 operator 仍生成 topology-only node，并返回 warning 级 `visual.dslImport.operatorMissing`，避免迁移 preview 丢图。
...
### 2026-07-07：DSL Batch Governed Draft Commit

触发问题：

批量报告能告诉迁移负责人哪些 `.bloge` 文件可渲染、哪些需要修复、哪些可进入 rewrite gate，但它仍停留在“评估态”。存量业务要真正迁到可视化交付，需要把一批可接受的 DSL 直接沉淀成 governed `GraphDraft`，并保留逐份 validation / dependency evidence，而不是让脚本循环调用单文件 commit。

本轮完成：

1. 新增 `POST /api/visual/dsl-imports/batch-commit`，请求接受多份 `sources[]`、同一套 schema-neutral `operatorLibraryIds/catalogIds` + `inlineLibraries`，以及 `commitPolicy`。
2. `commitPolicy` 支持 `renderable`、`fully-projected`、`rewrite-allowed` 三档：默认 `renderable` 与单源 commit 一致，允许 missing operator/function 保存为 repairable draft；`rewrite-allowed` 只保存 semantic round-trip 通过的低风险 source。
3. 新增 `bloge.dslImportBatchCommitResult.v1`、`DslImportBatchCommitSummary` 和 `DslImportBatchCommitItem`，逐份输出 `COMMITTED_*`、`SKIP_*` 或 `FAILED_PERSISTENCE`，并携带 `GraphDraftImportResult`、validation 和 dependency report。
4. `DslImportService` 抽出 `reportItem()` / `summarize()`，让 batch-report 与 batch-commit 复用同一套 renderable / fullyProjected / needsRepair / rewrite decision 分类，避免批量保存和批量报告漂移。
5. 协议、产品指南、迁移设计文档、实现状态索引和本评估报告同步更新，明确 batch-commit 只保存 governed draft，不写 `.bloge` 源文件，也不是 VCS PR/source writer。
...
### 2026-07-07：Legacy DSL Topology-Only Projection

触发问题：

存量业务最先需要的是看懂手写 DSL 的整体拓扑，而不是立刻证明每个 operator/function 的 schema 都完整。把合法 schema 作为首次可视化前置条件，会让迁移用户在还没看到业务结构之前就被 schema acquisition 阻断。

本轮完成：

1. `DslImportService` 把 missing operator/function 从渲染级 error 调整为 warning，仍保留可见 draft node、表达式文本、输入绑定、输出引用、依赖边和 graph contract。
2. `draft.visualLayout.import` 新增 topology metadata：`projectionMode`、`topologyOnly`、`schemaPrecision`、`operatorRefs`、`functionNames`、`missingOperatorRefs` 和 `missingFunctionNames`。
3. `/author/` Legacy DSL notice 会直接提示 `topology-only projection`，让用户知道当前结果适合拓扑审阅，但不是 schema 精确或 rewrite safe 证据。
4. 单元/API 测试覆盖 missing operator 和 missing built-in function 的 warning 级诊断、topology-only metadata，以及 batch report rewrite decision 从 import error 转为 incomplete evidence。
5. 产品指南、迁移设计、协议草案、实现状态审计和覆盖矩阵同步改成“DSL AST 先可视化，schema-backed 后增强”的产品口径。
...
topology-only draft 只负责理解和迁移审阅，不能被当作 fully projected、runtime executable 或 auto rewrite safe。后续还需要把缺失 schema 的补齐动作、opaque snippet 修复向导和 Studio 迁移队列做成更顺手的协作流程。综合分从 97.1 提升到 97.4。

### 2026-07-05：Schema Rebase Decision Queue

触发问题：

上一轮已经能在 selected operator contract 中看到 frozen/current schema preview，但作者仍然缺一个生产工作台视角：当一个 draft 里有多个 drifted 或 missing snapshot 节点时，系统必须告诉作者哪些可以 review-and-rebase、哪些需要 repair-before-rebase、哪些因为 catalog/scope 不可操作，并且批量 rebase 不能绕过 revision guard 和 dirty draft guard。

本轮完成：

1. `GraphDraftDependencyReport` 新增 `schemaRebaseDecisions` 与 `schemaRebaseDecisionStateCounts`，把每个需要处理的节点归类为 `ready-rebase`、`ready-capture`、`repair-review` 或 `blocked`。
2. 每条 decision 携带稳定 `decisionId`、node/operator/library、recommended action、`rebaseEligible`、blocking reason、issue counts、affected surfaces/paths、downstream nodes 和 bounded review summary。
3. Draft Dependencies 面板新增 `Schema Rebase Queue`，可 focus 节点、显示 repair/blocked/ready 状态，并对所有 eligible queue item 通过现有 `/operator-fingerprints/rebase` 执行 revision-guarded 批量 rebase。
4. selected operator contract 也展示当前节点的 `Schema Rebase Queue`，把 schema review 证据、decision queue 和 rebase action 放到同一个作者上下文中。
5. UI 继续复用 dirty draft guard：本地未保存改动时单节点和批量 rebase 都禁用，避免 queue 操作绕过保存/重载语义。
...
mvn -q -f resource-gateway-examples/pom.xml -Dtest=VisualAuthoringBrowserDomTest#composerRebasesFingerprintDriftFromDraftDependencyPanelInRealBrowser+composerOverlaysSchemaDriftOnSelectedOperatorContractInRealBrowser test
```

剩余风险：

这轮把 rebase 从“单个 drift 按钮”推进到“服务端决策队列 + 批量 guarded mutation + selected-node 局部队列”，所以评估从 94 分推进到 95 分，已经满足本目标定义的 5% 以内差距。但这仍不是完整 schema merge IDE：没有逐字段 accept/reject、树形 diff 编辑、跨 OpenAPI/AsyncAPI/resource contract 的统一 drift workbench，也没有覆盖大量负路径/并发/批量混合失败矩阵。

### 2026-07-05：Schema Drift Side-by-Side Preview

触发问题：

上一轮已经能显示字段级 type transition 和 keyword diff，但作者仍然只看到摘要。生产编排画布在 rebase 前必须能看到“冻结快照 schema”和“当前 catalog schema”的局部正文，否则 `enum`、`minimum`、`pattern`、`items` 这类变化仍需要作者跳到算子库 JSON 里人工对照。

本轮完成：

1. `GraphDraftDependencyReport.SchemaCompatibilityIssue` 新增 bounded `schemaPreview`，包含 drift path、`savedSchema`、`currentSchema` 和 `truncated` 标记。
2. 服务端只截取 drift path 对应的 schema 片段，并限制深度、对象 key 数、数组项数和字符串长度，避免大 OpenAPI/AsyncAPI schema 把 dependency report 撑爆。
3. selected operator contract 新增 `Schema Review` 区，按 breaking/compatible issue 渲染并排 `Frozen schema` / `Current schema` preview。
4. `Schema Search` 索引纳入 schema preview 正文，所以搜索 `frozen integer`、`minimum`、`current schema` 这类 review 语义可定位 drift row。
5. API、JS probe 和真实 Chrome 回归覆盖 breaking/compatible `schemaPreview`、review HTML、preview search、真实 inspector 文本和 390px mobile no-overflow。

验证：

```bash
mvn -q -f resource-gateway-examples/pom.xml -Dtest=VisualGraphDraftControllerTest#dependenciesClassifyCompatibleOperatorSchemaDrift+dependenciesClassifyBreakingOperatorSchemaDrift clean test
mvn -q -f resource-gateway-examples/pom.xml -Dtest=VisualAuthoringAppJsTest test
...
这轮把 drift review 从“keyword 摘要”推进到“bounded side-by-side schema preview”，所以评估从 93 分推进到 94 分。但这仍不是完整 schema merge IDE：没有逐字段 accept/reject，没有批量 rebase 决策队列，没有跨 OpenAPI/AsyncAPI/resource contract drift 的统一工作台，也没有大量负路径 drift matrix。
...
上一轮 source-row 窗口只证明了 12 个输出端口的 1-8 / 9-12 两段切换。这个证据能说明窗口机制成立，但工业画布里的用户算子可能暴露几十个字段级输出、指标输出或事件 payload 分支；如果只覆盖 12 个 source handle，还不能证明 inspector 在多页 source 矩阵下真的把服务端候选请求和 DOM 渲染控制住。

本轮完成：

1. 将 `VisualAuthoringBrowserDomTest#composerConnectabilityWindowsLargeSourceHandleSetInRealBrowser` 的测试算子从 12 个输出端口扩展到 40 个输出端口。
2. 浏览器回归现在验证 1-8、9-16、17-24、25-32、33-40 五段 source window，且每一段等待服务端 `resultsBySourceKey` 只包含当前可见 source rows，证明 `ensureNodeConnectabilityServerCandidates()` 的 source-window scope 没有偷偷退回全量请求。
3. 在尾页 33-40 source endpoints 下切到 390x980 mobile viewport，复核 Connectability panel、filter controls、source-window controls、source rows 和页面级横向滚动均无溢出。
...
这轮把“大量 source handle 证据”从 12 个输出端口推进到 40 个输出端口，并补上尾页移动回归，所以评估从 86 分推进到 87 分。但这仍不是完整虚拟化：source rows 仍是分页窗口，不是滚动容器虚拟渲染；多 operator family 大图、完整 a11y 审计、复杂 schema field rendering 负路径和更极端 source handle 性能矩阵仍需继续补。

### 2026-07-05：Draft / Publication / Golden / Run History 生命周期 390px 回归

触发问题：

前几轮已经证明用户能从 YAML/OpenAPI/AsyncAPI 把 schema 带进画布，也能在 Connectability 大候选面板里按 schema 约束拖线。但严肃生产项目不是“能画完一张图”就结束。作者还必须在同一个画布里保存草稿、查看依赖、导出/导入、发布不可变 artifact、保存和运行 golden case、查看认证状态、按来源过滤运行历史。如果这些生产生命周期面板在移动宽度下撑破页面，系统仍会退回 demo 级体验。

本轮完成：

1. 移动端 CSS 把 `.draft-controls`、`.draft-revision-controls`、`.draft-transfer-controls`、`.golden-assertion-controls`、`.run-history-controls` 在 760px 以下收敛为单列布局，并让 golden assertion row、run history row/stats 在窄屏下单列展示，避免 select、按钮、长 run id 和认证状态把页面撑宽。
2. 扩展 `VisualAuthoringBrowserDomTest#composerSupportsOpenApiSaveLibraryImportSchemaConnectionRunAndPublishInRealBrowser`：在完整 OpenAPI resource contract、用户 operator library 导入、schema-aware connection、draft save/export/import、transient run、publish、publication run、golden save/run/suite/certify/delete 和 run-history source filter 之后，切到 390x980 mobile viewport。
3. 同一浏览器用例逐面板复核 draft controls/revision controls/transfer controls/dependencies/bundle/status、publication bundle、golden case select/assertion controls/assertion list/certification/status、run-history controls/stats/node-stats/list/rows 和页面级横向滚动均无溢出。
...
这轮把移动矩阵从“导入入口 + Connectability”推进到“导入入口 + Connectability + 生产生命周期主路径”，所以评估从 85 分推进到 86 分。但当时仍不是完整工业验收：复杂 schema field rendering、draft revision diff 的更多冲突态、publication bundle diff、更多 run-history 异常态、多 operator family 大图、真正虚拟化列表、完整 a11y 审计和更极端 source handle 性能矩阵还没有形成系统矩阵。后一轮已把 source-handle 回归扩到 40 输出端口并覆盖尾页移动布局，剩余风险继续集中在虚拟化、完整 a11y、多算子族大图和更复杂异常态。
...
这轮把移动矩阵从“YAML + Connectability”推进到“YAML + OpenAPI + AsyncAPI + Connectability”，所以评估从 84 分推进到 85 分。但当时仍不是完整移动端验收：draft/history、publication/golden、run history、复杂 schema field rendering、source-row 大矩阵、更大 source handle、多 operator family 大图和真正虚拟化列表还没有形成系统矩阵。后一轮已补核心生产生命周期面板移动回归，剩余风险转向更复杂异常态、虚拟化、无障碍和多算子族大图。
