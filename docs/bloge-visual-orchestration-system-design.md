...
| `GET` | `/api/visual/publications/summaries` | 当前已实现：返回轻量 `bloge.visualGraphPublicationSummary.v1`，支持 scope 与 `artifactKind` 的 `EXECUTABLE` / `DESIGN` 过滤，不需要拉完整 artifact payload 即可做设计制品/可执行制品资产索引 |
...
| `GET` | `/api/visual/assets/overview` | 当前已实现：返回 `bloge.visualAssetOverview.v1`，聚合 draft/publication/operator catalog readiness、runtime implementation binding、adapter activation、rollout observation、executable lowering integration 以及 complete/partial/failed runtime evidence chain 健康计数，并用 summary actionReadiness、runtimeBindingRequirements 和 runtime evidence chain 风险派生可按 severity/type/targetKind/operatorRef/operatorLibraryId 查询、计数和分页的 action queue；runtime evidence action item 只用于派工和审阅，不会直接改变 graph artifact、runtime lifecycle 或 operator readiness |
