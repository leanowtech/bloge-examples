...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/supersede` | 当前已实现：用同 operatorRef 的 replacement proposal 替换 active bound binding，写入 supersede lineage 和 lifecycle events；同一 supersede lifecycle pair 的 exact replay 返回 idempotent `200 OK` 且不重复追加 lifecycle event，不同 already-superseded 请求仍冲突；仍不直接改 graph artifact 或伪造 executable readiness |
