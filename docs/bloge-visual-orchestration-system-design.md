...
的其他事务/partial-failure 硬化与跨系统/异步 runtime workflow 的更广义 idempotency、更深跨系统 reimplementation rollout 策略和更完整 JSON Schema 兼容性推理门禁。没有这些后续硬化，
...
- implementation metadata：adapter kind、entrypoint、capabilities、runtime owner、test evidence、policy evidence、rollback target，以及可选 implementationVersion、reimplementationOfBindingId、reimplementationStrategy。
- contract diff：输入/输出/config/policy/lowering/runtime readiness 的 breaking / compatible / runtime / governance / metadata 变化分类已进入 implementation validate，input/output/config schema drift 已复用 shared JSON Schema compatibility 做保守分类并返回 breaking reason；runtime implementation submit 已补基础 SemVer 与 reimplementation base 对账；后续需要补更完整 JSON Schema 兼容性推理和跨系统 rollout/canary/rollback 策略。
