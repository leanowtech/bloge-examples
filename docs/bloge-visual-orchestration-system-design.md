...
| `GET` | `/api/visual/assets/overview` | 当前已实现：返回 `bloge.visualAssetOverview.v1`，聚合 draft/publication/operator catalog readiness、runtime implementation binding、adapter activation、rollout observation、executable lowering integration 以及 complete/partial/failed runtime evidence chain 健康计数，并用 summary actionReadiness 和 runtimeBindingRequirements 派生可按 severity/type/targetKind/operatorRef/operatorLibraryId 查询、计数和分页的 action queue；runtime evidence 仍是只读 control-plane fact，不会直接改变 graph artifact 或 operator readiness |
