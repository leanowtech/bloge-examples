...
`visual.resourceContract.deletePersistenceFailed`，metadata 固化 resourceId、
contractId、mutationAction 和异常类别，让 OpenAPI/resource schema 进入画布的入口
可以区分平台存储故障、schema gate、impact gate 和 runtime-readiness 缺口。
...
| Method | Path | 说明 |
| --- | --- | --- |
| `GET` | `/api/visual/operators` | 当前已实现：查询可用算子，支持 pattern、tag、tenant、namespace、environment、source/lowering/capability/runtime-readiness facets，并返回 `runtimeBindingProjections[]` / `runtimeBindingProjectionStateCounts` 与 `executablePromotionProjections[]` / `executablePromotionStateCounts`；projection 明细跟随当前 catalog page，projection state counts 覆盖完整过滤匹配集，把 active bound implementation record、active adapter activation、executable lowering integration 和仍缺 readiness recompute 的 promotion blocker 投影为 palette 可见状态，但不修改 trusted `OperatorDefinition.runtimeReadiness` |
| `GET` | `/api/visual/operators/{operatorRef}` | 当前已实现：按 catalog 可见性门禁获取单个算子定义，支持 `tenantId`、`namespace`、`environment`、`includeDeprecated`、`resourceOnly` 和 source/lowering/capability/runtime-readiness filters，不可见或不存在返回 404；默认保持 `bloge.visualOperator.v1` 兼容响应，`includeProjections=true` 返回 `bloge.visualOperatorDetail.v1`，把详情页所需的 `runtimeBindingProjection` 与 `executablePromotionProjection` 同步带回 |
...
它还会把 implementation binding、adapter activation、rollout observation 和 executable lowering integration 四段 runtime evidence 串成 Runtime Evidence Chain，只读展示 kind/operator/binding/activation/lifecycle/rollout/guardrail signal 过滤后的证据链状态，让作者看到 post-binding promotion 卡在绑定、激活、rollout、executor bridge 还是 readiness recompute，而不把 runtime-plane fact 混入 graph artifact 语义。
...
服务端历史证据处理。
operator-library 写入型 mutation 也必须把 registry 持久化失败表达成机器可审阅的
`409` 合同：普通 create/import-text/replace 使用 `visual.library.importPersistenceFailed`，
bundle import 使用 `visual.library.importBundlePersistenceFailed`，revision restore 使用
`visual.library.restorePersistenceFailed`，delete 使用
`visual.library.deletePersistenceFailed`，metadata 固化 libraryId、mutationAction 和异常类别。
这让用户算子库导入作为画布入口时，可以把存储面故障和 schema/readiness/impact gate
分开处理，而不是泄漏成通用 500。
...
| `POST` | `/api/visual/drafts/{draftId}/publish` | 发布为 graph version；浏览器和 API 默认 `artifactKind=EXECUTABLE`，也支持 `artifactKind=DESIGN` 冻结非执行型设计制品；warning-level diagnostics 需 `ackWarnings=true` 后才写入；响应在成功和拒绝时都保留 validation/readiness/actionReadiness，供客户端按 artifact kind、warning gate 和 repository persistence failure 纠偏；publication 写入失败返回 `visual.publication.persistenceFailed` |
| `GET` | `/api/visual/publications` | 当前已实现：按 tenant/namespace/environment/source artifact kind 查询 immutable publication asset，能在资产列表层区分 `EXECUTABLE` 与 `DESIGN` |
| `GET` | `/api/visual/publications/summaries` | 当前已实现：返回轻量 `bloge.visualGraphPublicationSummary.v1`，支持 scope 与 `artifactKind` 的 `EXECUTABLE` / `DESIGN` 过滤，并暴露 frozen dependency schema compatibility drift 摘要，不需要拉完整 artifact payload 即可做设计制品/可执行制品资产索引 |
...
| `POST` | `/api/visual/publications/import-bundle` | 当前已实现：导入 portable publication bundle，返回 `bloge.visualGraphPublicationImportResult.v1`，包含 `sourceBundleFingerprint`、source bundle dependency report、基于目标环境当前 catalog 计算的 target dependency report、frozen readiness 派生的 target runtime-binding handoff requirements 和 stable keys，并对 unsupported bundle/publication schemaVersion、fingerprint mismatch、缺失 snapshot、重复 publicationId 和 repository 写入失败做结构化拒绝；目标写入失败返回 `visual.publication.importPersistenceFailed` |
| `GET` | `/api/visual/assets/overview` | 当前已实现：返回 `bloge.visualAssetOverview.v1`，聚合 draft/publication/operator catalog readiness、schema breaking/compatible drift、runtime implementation binding、adapter activation、rollout observation、executable lowering integration 以及 complete/partial/failed runtime evidence chain 健康计数，并用 summary actionReadiness、schema drift、runtimeBindingRequirements 和 runtime evidence chain 风险派生可按 severity/type/targetKind/operatorRef/operatorLibraryId/handoffLane/handoffKind/handoffTarget/readinessState/artifactKind/evidenceKind/evidenceId/bindingId/activationId/adapterKind/runtimeEnvironment/rolloutSignal 查询、计数和分页的 action queue；schema drift / runtime evidence action item 只用于派工和审阅，不会直接改变 graph artifact、runtime lifecycle 或 operator readiness |
| `GET` | `/api/visual/assets/runtime-binding-requirements` | 当前已实现：返回 `bloge.visualRuntimeBindingRequirements.v1`，把 active draft 和 immutable publication 的 runtime binding gaps 暴露为 scope-aware/filterable/pageable 事实索引，并支持 `operatorRef` / `operatorLibraryId` / publication `artifactKind=EXECUTABLE\|DESIGN` 过滤/计数和 `requirementKey` 精确回查 |
| `GET` | `/api/visual/assets/runtime-binding-requirements/runtime-evidence` | 当前已实现：返回 `bloge.visualRuntimeEvidenceWindow.v1`，把 implementation binding、adapter activation、rollout observation 和 executable lowering integration 合成一个可分页 fact 窗口，支持 `evidenceKind`、operator、binding、activation、lifecycle、rollout、rollout signal 与 breached-only 过滤；`evidenceKind` 与 signal 条件组合时先按 rollout 找链路，再只返回匹配 kind 的链路上下文 |
| `GET` | `/api/visual/assets/runtime-binding-requirements/handoff-bundle` | 当前已实现：返回 `bloge.visualRuntimeBindingHandoff.v1`，把当前 runtime-binding 查询窗口导出为 portable handoff bundle，包含 source index lineage、scope/filter、stable requirement keys、artifactKind/operatorRef/operatorLibraryId/routing 计数、requirement 明细、当前窗口相关 `operatorContracts[]` 契约快照和 `bundleFingerprint` |
...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/validate` | 当前已实现第一刀：接收 `bloge.visualRuntimeBindingImplementationBinding.v1`，把 runtime team 提交的 source handoff fingerprint、source requirement keys、implementation metadata、test evidence、policy evidence、rollback target、rollout/canary/rollback plan、可选 implementation SemVer 和 reimplementation strategy 对准 handoff `operatorContract` snapshot、当前 runtime-binding read model 与当前 catalog fingerprint 做无状态 pre-bind 校验；current requirement key 若属于其他 operator 会 rejected，stale/missing source evidence 会 requires-review；非法 rollout strategy 或 traffic window 会 rejected，缺失 rollout plan、rollback trigger、高风险 immediate rollout 或高风险缺少 rollout evidence 会 requires-review；返回 `bloge.visualRuntimeBindingImplementationValidation.v1` 的 ready-to-bind/requires-review/rejected 裁决；不写入 binding 状态 |
...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/bind` | 当前已实现：把 ready-to-bind 或 review-acknowledged proposal 推进到 `bound` lifecycle state，要求 actor/reason 审计证据；可用 `expectedRevision` 拒绝 stale lifecycle acknowledgement，同一治理意图的 exact replay 返回 idempotent `200 OK` 且不重复追加 lifecycle event，不同 already-bound 请求仍冲突；拒绝同一 operatorRef 的第二个 active binding；repository update 失败会返回 `state=failed` 和 `visual.runtimeBindingImplementation.bindPersistenceFailed` 诊断，而不是泄漏为通用 500 或伪造 active bound fact |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/unbind` | 当前已实现：把 active bound implementation 推进到 `unbound` lifecycle state，要求 actor/reason 审计证据，并可用 `expectedRevision` 拒绝 stale unbind acknowledgement；级联把当前 active executable lowering integration 与 adapter activation 标记为 `inactive`，从 active catalog/promotion projection 中移除但保留审计事实；同一治理意图的 exact replay 返回 idempotent `200 OK` 和已保存的 inactive evidence，不同 already-unbound 请求仍冲突；若 cascade 后续写入失败，会返回 `state=failed`，恢复已 inactive 的 runtime evidence，并把 integration 对齐到恢复后的 activation revision |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/supersede` | 当前已实现：用同 operatorRef 的 replacement proposal 替换 active bound binding，写入 supersede lineage 和 lifecycle events；可用 `expectedRevision` / `expectedReplacementRevision` 同时保护 current 与 replacement binding，同一 supersede lifecycle pair 的 exact replay 返回 idempotent `200 OK` 且不重复追加 lifecycle event，不同 already-superseded 请求仍冲突；仍不直接改 graph artifact 或伪造 executable readiness |
...
| `GET/POST` | `/api/visual/assets/runtime-binding-requirements/adapter-activations` | 当前已实现：healthy/current activation assertion 可持久化为 `bloge.visualRuntimeAdapterActivation.v1`，并按 bindingId/operatorRef/state 查询；同一 stable `activationId` 的精确 replay 返回已有 fact 作为 idempotent `200`，同 id 但 submitted evidence 不同返回结构化 `409`，repository 写入失败返回 `visual.runtimeAdapterActivation.persistenceFailed` 诊断，并拒绝 unbound binding、fingerprint/adapter drift、重复 active activation；catalog projection 可展示 `adapter-active`，但仍不伪造 executable readiness |
| `POST` | `/api/visual/assets/runtime-binding-requirements/rollout-observations/validate` | 当前已实现：接收 `bloge.visualRuntimeRolloutObservationRequest.v1`，把 runtime-plane canary/ramp/rollback execution feedback 对准 active activation、bound implementation、当前 catalog fingerprint、bound rolloutPlan、traffic percent、rollback signal、actor/reason 和 evidence 做无状态校验；degraded/rolled-back 是可记录的 runtime fact，只有身份、revision、fingerprint、计划或证据不一致才 rejected |
| `GET/POST` | `/api/visual/assets/runtime-binding-requirements/rollout-observations` | 当前已实现：rollout execution observation 可持久化为 `bloge.visualRuntimeRolloutObservation.v1`，并按 activationId/bindingId/operatorRef/state 查询；同一 stable `observationId` 的精确 replay 返回已有 fact 作为 idempotent `200`，同 id 但 submitted evidence 不同返回结构化 `409`；该事实是 rollout 反馈和治理输入，不直接改 graph artifact、operator definition 或 executable readiness |
...
| `GET/POST` | `/api/visual/assets/runtime-binding-requirements/executable-lowering-integrations` | 当前已实现：当前 executor bridge assertion 可持久化为 `bloge.visualExecutableLoweringIntegration.v1`，并按 activationId/operatorRef/state 查询；同一 stable `integrationId` 的精确 replay 返回已有 fact 作为 idempotent `200`，同 id 但 submitted evidence 不同返回结构化 `409`，repository 写入失败返回 `visual.executableLoweringIntegration.persistenceFailed` 诊断，并拒绝 missing/stale activation、unbound binding、catalog drift、`loweringMode=design`、重复 active integration；catalog promotion 可展示 `readiness-recompute-required`，但仍不伪造 executable readiness |
...
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/apply` | 当前已实现：服务端重新执行 readiness recompute preview，要求 `ackWarnings=true` 与 actor/reason 审计证据，拒绝 fingerprint drift、缺失 owner library、缺失 candidate 或 `design` lowering，并可用 `expectedCurrentOperatorFingerprint` / `expectedCandidateOperatorFingerprint` 拒绝 stale preview acknowledgement；把 candidate operator surface 作为 owning operator-library 的新 immutable revision 写入，若 revision 写入失败则返回 `state=failed` 与 `visual.executableReadinessRecompute.libraryRevisionWriteFailed` 诊断，不把失败泄漏成通用 500；不接受客户端提交的 operator surface，且用户导入路径不能伪造 server-managed runtime-binding lowering parameters |
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/evidence-refresh` | 当前已实现：apply 后要求 `ackWarnings=true` 与 actor/reason 审计证据，用当前 trusted operator fingerprint 重建 bound implementation、adapter activation 和 executable lowering integration evidence chain，并可用 `expectedPreviousOperatorFingerprint` / `expectedCurrentOperatorFingerprint` 拒绝 stale evidence-refresh acknowledgement；旧 binding 标记为 superseded，旧 activation/integration 保留为审计事实；若 refreshed binding create 失败会结构化返回 `state=failed`，若 binding transition、refreshed activation 或 integration 写入失败，会把 partial refreshed binding/activation/integration 标记为 `failed` 并恢复 source binding 为 `bound`；接受 native executable 与 external-runtime-bound 的 runtime-binding/metadata 变化面；只有当前 binding、activation、integration 三段 active evidence 完整对齐时才返回 `current`，否则暴露 incomplete current chain blocking diagnostic |
...
| `POST` | `/api/visual/golden-cases` | 当前已实现：为已发布版本保存样例 context、outputNode、期望输出、可选 output assertions、numeric tolerance assertions 和 output schema assertions；case 保存失败返回 `visual.golden.savePersistenceFailed` |
| `DELETE` | `/api/visual/golden-cases/{caseId}` | 当前已实现：删除单个 golden case；删除写入失败返回 `visual.golden.deletePersistenceFailed`，不移除当前 case |
| `POST` | `/api/visual/golden-cases/{caseId}/run` | 当前已实现：用 frozen publication DSL 执行 golden case，写入 run history，并返回 exact-output、value/tolerance assertion、schema assertion 或 `visual.golden.runHistoryPersistenceFailed` diagnostics |
| `POST` | `/api/visual/golden-cases/publications/{publicationId}/run` | 当前已实现：执行某个 publication 绑定的全部 golden cases，汇总 total/passed/failed 并为每个 case 写入 run history；run-history 写入失败会汇总为 suite-level `visual.golden.runHistoryPersistenceFailed` |
| `POST` | `/api/visual/golden-cases/publications/{publicationId}/certify` | 当前已实现：执行 suite，持久化该 publication 的最新 golden certification，并记录当次 case-set fingerprint；run-history 或 certification 写入失败分别返回 `visual.golden.runHistoryPersistenceFailed` / `visual.golden.certificationPersistenceFailed` |
...
4. **Golden Test Cases**：当前已支持每个发布版本绑定样例输入、期望输出、基础 value/path assertions、`PATH_APPROX_EQUALS` numeric tolerance assertions 和 `OUTPUT_MATCHES_SCHEMA` schema assertions，并用 frozen publication DSL 执行 single-case 与 suite-level 回归；浏览器 Publications 面板已支持从最近一次 publication run 输出保存 exact-output case，或用多断言队列保存 value/path assertion、numeric tolerance assertion 与 output-schema assertion 组合；latest certification 作为独立控制面状态保存，不反写 immutable publication；promotion-readiness status 通过 case-set fingerprint 识别 stale certification，并以 `promotionReady` 与 diagnostics 支撑推广准入；case save/delete、run-history write 和 certification write 的存储失败均以结构化 `visual.golden.*PersistenceFailed` 返回，避免回归链路把平台故障误判成业务断言失败。
...
handoff contract 与当前 catalog，并已支持 bind/supersede/unbind lifecycle fact 和 exact replay 幂等返回；但 active bound fact
...
runtime rollout observation 和 executable lowering integration 四类 stable id submit endpoint 也已支持精确 replay 幂等返回；runtime implementation
metadata 已支持 `implementationVersion`、`reimplementationOfBindingId` 和 `reimplementationStrategy`，
submit 会对 base binding 做同 operator 与 forward SemVer 校验，并把 strategy/version 不协调降为
requires-review；implementation metadata 也已支持 `rolloutPlan`，validate 会校验 `strategy`、
traffic window、rollback signal/window 和 rollout evidence，并把高风险 immediate rollout 路由到
requires-review；runtime rollout observation registry 已能接住 canary/ramp/rollback 执行反馈，并把 degraded/rolled-back 作为可审计事实存储。剩余缺口不再是“完全不能写回、无法续接、
无法退出、非 native lowering 无法 apply、完全不知道 drift 形态、基础 retry 一定冲突或重实现完全无治理”，而是还缺跨 repository mutation
的其他事务/partial-failure 硬化与跨系统/异步 runtime workflow 的更广义 idempotency、更深跨系统 reimplementation rollout 编排、指标消费和执行决策闭环，以及更完整 JSON Schema 兼容性推理门禁。没有这些后续硬化，
...
| 优先级 | 缺口 | 为什么卡住生产级 |
| --- | --- | --- |
| P0 | Runtime implementation binding 与 readiness 派生闭环 | 当前已有 validate gate、proposal record、bind/supersede/unbind lifecycle fact 与 exact replay 幂等返回、bind 写入失败诊断、supersede partial-failure compensation、unbind/deactivation partial-failure compensation、adapter activation registry、runtime rollout observation registry、executable lowering integration registry、四类 runtime evidence submit stable-id 精确 replay 幂等返回和提交写入失败诊断、operator-library create/import-text/import-bundle/replace/restore/delete registry 写入失败诊断、ResourceDesignContract upsert/delete registry 写入失败诊断、golden save/delete/run-history/certification 写入失败诊断、catalog response 级 `OperatorRuntimeBindingProjection` / `OperatorExecutablePromotionProjection`、native executable 与 external-runtime-bound readiness recompute preview、受治理 apply mutation 写入 trusted operator-library revision，并在 revision 写入失败时返回结构化 failed result，post-apply evidence refresh/rebind mutation 续接当前 trusted fingerprint，并对 refreshed binding create、binding transition、activation/integration partial failure 做结构化失败与显式补偿，governed unbind/deactivate 退出 active evidence，复用 shared JSON Schema compatibility 的 implementation contract-diff gate、runtime implementation SemVer/reimplementation submit gate，以及 implementation rolloutPlan 门禁；下一步需要补其他尚未覆盖的跨 repository mutation 的事务/partial-failure 硬化、跨系统/异步 runtime workflow 的更广义 idempotency、更深跨系统 reimplementation rollout 编排、指标消费和执行决策闭环，以及更完整 JSON Schema 兼容性推理，否则 runtime-binding requirement 虽然能被导出、预检、保存提案、形成绑定、激活、rollout observation、lowering integration 事实、写回可信 library revision、续接/退出 active evidence，但仍不能覆盖所有长期运行治理场景 |
| P0 | Contract diff 与兼容性门禁 | 当前 handoff snapshot 可防篡改、可对账，implementation validate 已能在 catalog fingerprint drift 时按 input/output/config breaking、compatible surface、runtime/lowering、governance/policy、metadata 分类输出 diagnostics，并用 shared JSON Schema compatibility 把确定不兼容的 schema drift 作为 blocking error、把保守可兼容 schema drift 路由到 requires-review，breaking metadata 会携带 `schemaCompatibilityIssues`；implementation metadata 已增加 `implementationVersion`、`reimplementationOfBindingId`、`reimplementationStrategy` 和 `rolloutPlan`，submit 会拒绝非法 SemVer、缺失/跨 operator base binding、非 rollback 的非前进版本，并把 compatible 跨 major、breaking/adapter-migration 未升 major、rollback 升版、高风险 immediate rollout 或缺少 rollout evidence 路由为 review；runtime rollout observation 已能接住第一层 canary/rollback 执行事实；下一步仍需要更完整 JSON Schema 语义推理和跨系统 rollout / canary / rollback 指标消费与决策闭环，而不是只停在第一层兼容性工具复用 |
...
- golden test cases 基础版、output assertion modes、numeric tolerance assertion、output schema assertion、publication suite run、latest certification、stale-aware promotion gate 和 golden save/delete/run-history/certification 写入失败诊断已落地。
...
- 当前已落地 implementation bind / supersede / unbind API，记录 active bound fact、replacement lineage、unbound evidence 和 actor/reason lifecycle events，并支持同治理意图的 exact replay 幂等返回。
...
- 当前已落地 runtime rollout observation validate/submit/list API，持久化 canary/ramp/rollback execution feedback 为 `bloge.visualRuntimeRolloutObservation.v1`，并把 degraded/rolled-back 作为可审计 warning/error fact。
...
- 已补 native governed library revision mutation、external-runtime-bound governed library revision mutation、apply revision-write failed result、post-apply evidence refresh/rebind mutation、覆盖 binding create/transition/activation/integration 的 refresh partial-failure compensation、bind 写入失败诊断、supersede partial-failure compensation、unbind/deactivation partial-failure compensation、governed unbind/deactivate mutation、bind/supersede/unbind lifecycle exact replay、implementation contract-diff gate、semantic JSON Schema-compatible drift classification、runtime implementation SemVer/reimplementation submit gate、implementation rolloutPlan validate gate、runtime rollout observation registry、四类 runtime evidence submit stable-id 精确 replay 幂等返回和提交写入失败诊断、operator-library create/import-text/import-bundle/replace/restore/delete registry 写入失败诊断、ResourceDesignContract upsert/delete registry 写入失败诊断，把 readiness recompute preview 写成可信 operator-library 事实后继续续接或退出当前 runtime evidence，并在旧 handoff contract 回流时区分 breaking / compatible / runtime / governance / metadata drift；后续补其他尚未覆盖的跨 repository 事务/partial-failure 硬化、跨系统/异步 runtime workflow 的更广义 idempotency、更深跨系统 rollout/canary/rollback 编排与指标消费闭环和更完整 JSON Schema 兼容性推理。
...
- implementation metadata：adapter kind、entrypoint、capabilities、runtime owner、test evidence、policy evidence、rollback target、rollout plan，以及可选 implementationVersion、reimplementationOfBindingId、reimplementationStrategy。
- contract diff：输入/输出/config/policy/lowering/runtime readiness 的 breaking / compatible / runtime / governance / metadata 变化分类已进入 implementation validate，input/output/config schema drift 已复用 shared JSON Schema compatibility 做保守分类并返回 breaking reason；runtime implementation submit 已补基础 SemVer 与 reimplementation base 对账，validate 已补 rollout/canary/rollback plan 门禁，runtime rollout observation 已补第一层执行反馈事实；后续需要补更完整 JSON Schema 兼容性推理和跨系统 rollout/canary/rollback 指标消费与决策闭环。
