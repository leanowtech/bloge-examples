...
| `ResourceDescriptor` 无精确 payload schema，画布无法类型安全连接 | 已缓解：独立 `ResourceDesignContract` 补足 request/payload schema 并投影 resource virtual operator；已提供 OpenAPI operation discovery 和 operation 到 contract 草案的 preview endpoint；AsyncAPI operation/message discovery 可先暴露候选 readiness，再选择子集生成 event-source/message-handler/webhook operator-library 草案并复用 validator/profile/impact，浏览器可把 preview 结果回填到 operator-library import 编辑器 | 继续深化 descriptor 生成建议、schema diff、多选/批量导入治理和更广义接口导入 |
