...
1. **说清世界观**：这件事需要哪些事实，可能产生哪些处置结果；Codex 据此形成第一版业务契约库。
2. **接入事实来源并提供样例**：把同一契约库中的四项能力接到事实来源，并保存典型样例。
...
五幕不是五段互不相干的提示词。上一幕的服务端资产必须成为下一幕的输入：

| 幕次 | 本幕必须产出的资产 | 下一幕开始前的交接门 |
| --- | --- | --- |
| 会前 | 四个只读沙箱来源及其权威输入、输出契约 | 平台目录能发现四个来源，逐个契约可读 |
| 1 世界观 | `ride-cancellation` 业务契约库，含四个设计态事实能力 | 库可回读；四项能力都存在；当前明确为“仅契约” |
| 2 来源与样例 | 同一契约库的四个实现指向，以及受治理样例 | 四项能力的契约与来源一致；都显示“已接入”；样例可用 |
| 3 规则 | `ride-cancellation-policy` 候选能力 | preview、gate 和保存消费同一契约库版本 |
| 4 标准案例 | `ride-cancellation-golden` 及其人工批准、RED/GREEN 证据 | 案例与候选能力关联一致；批准后才可执行 |
| 5 发布 | 当前实现的实景证明、签署和不可变发布物 | 规则、案例、来源或实现任一变化，旧证明和签署失效 |

上表中的内部名称只供演示保障人员核对，不向业务人员展示。前一幕的交接门未通过时不得继续；应停在当前幕修复，不要继续输入下一段提示词，否则后续失败只是在放大前置资产缺失。
...
保障人员还要冻结一张来源清单，至少记录“业务名称、平台目录名称、业务负责人、沙箱环境、契约已确认”五项。来源的地址、认证和字段定义只进入平台目录，不进入小李的提示词。

这里准备的是**事实来源及其权威契约**，不是第 1 幕的业务契约库。业务契约库要等小李说清世界观后，由 Codex 在第 1 幕创建；第 2 幕再把它的四项逻辑能力映射到这里准备好的四个来源。把两者混为一谈，会导致 Codex 要么找不到 `libraryRefs`，要么拿资源名猜业务含义。
...
### 3.4 固定幕后资产坐标和契约库验收基线

本次演示只使用以下一条资产线：

| 资产 | 固定坐标 | 产生时机 |
| --- | --- | --- |
| 业务契约库 | `ride-cancellation` | 第 1 幕 |
| 候选能力 | `ride-cancellation-policy` | 第 3 幕 |
| 标准案例集 | `ride-cancellation-golden` | 第 4 幕 |
| 业务签署 | `取消费政策演示-v1` | 第 5 幕 |

第 1 幕契约库的可编译验收基线见
[取消费 Agent TDD 设计态契约库](examples/ride-cancellation-agent-tdd-library.yaml)。它定义订单、责任、城市政策、历史风险四类事实和四个只读逻辑能力，但故意不含运行实现指向。该文件有三个用途：

1. 彩排时核对 Codex 从业务描述生成的库是否完整；
2. Agent 生成失败时帮助平台负责人定位契约创作问题；
3. 作为自动化测试的可编译文档样例。

不要把该 YAML 粘进小李的提示词，也不要在正式演示的目标项目中提前写入它。正式演示要证明的是“小李说清业务世界观后，Codex 能自行形成契约库”；保障基线不能替代这一步。
...
系统沙箱拒绝 Codex 读取仓库。认证使用“按用户编号决定客服接待方式”的纯业务提示词，证明 Agent 能自行找到资料来源、
完成带 premium/默认分支的候选能力、生成至少两条待审批案例，并让同一 Tool 的真实看板展示业务流程和规则表；若候选收到阻断反馈，还必须在限定轮次内修正。Agent 在标准案例需要人工批准时停下。
...
现在先不要连接真实系统，也不要设计具体规则。请先把你理解的业务范围整理成一份“业务世界观草稿”，并在后台保存好，后面接入事实来源和设计规则时继续使用同一份内容。告诉我：有哪些事实、有哪些可能结论、还有没有含义不清的地方。
```
...
- 在平台中创建 `ride-cancellation` 契约库，形成四个只有输入、输出约定而没有运行实现的只读事实能力。
- 回读刚创建的契约库，确认四项能力均存在且处于“仅契约”状态；后续调用统一引用这一库，不临时创建第二套同义契约。
- 在平台中把这一版保存为“草稿世界观、待接来源、待用例检验”。
- 只向小李反馈业务摘要；不展示生成的契约、代码或内部标识。

### 小李在看板核对

打开「第 1 幕 · 你的世界观与可用积木」。确认右侧出现四组业务事实、四个待接入的事实能力和三个业务结论。此时四项能力都应显示“仅契约”或“尚未接入”。
...
- 看板能用业务名称展示四类事实。
- 当前状态明确标记为“草稿世界观、待接来源、待用例检验”。
- 后台能回读 `ride-cancellation`，其中恰好包含订单、责任、城市政策、历史风险四项逻辑能力。
- 四项能力此时都没有运行实现；这不是错误，而是“契约先于实现”的预期状态。
- Codex 没有要求小李提供接口、字段类型或代码。

### 幕间交接：没有契约库，不进入第二幕

保障人员在后台确认 `ride-cancellation` 已创建且四项能力完整。若契约库不存在、不可回读、能力数量不对或出现阻断诊断，立即停止：让 Codex 根据服务端契约创作反馈修正，最多三轮。仍不收敛时交给平台负责人，不要继续讲“接入来源”，也不要让小李补写 YAML。
...
- 自动发现并匹配四个沙箱事实来源。
- 逐个读取来源的权威契约；不能只凭目录名称猜输入和输出。
- 更新第 1 幕的同一个 `ride-cancellation` 契约库，为四项逻辑能力补充运行实现指向；不得另建同义库绕过契约差异。
- 确认逻辑能力与事实来源的输入、输出和只读效应一致。任何一项不一致都停在第二幕，并交给相应来源负责人修正。
- 校验样例是否符合相应业务来源的约定。
- 把样例保存为可复用验证数据。
...
- 四个事实来源都显示“已接入”。
- `ride-cancellation` 的四项能力已从“仅契约”变为“已接入”，且仍是第 1 幕创建的同一契约库。
- 样例显示为“可用于验证”。
- 若有来源缺失，旅程停在第二幕，不带病进入规则设计。

### 幕间交接：来源绑定完成后才允许编排

保障人员确认四项能力的当前实现指向都可解析、契约一致、效应为只读，并且样例已经保存。第 3 幕的 reference、preview、gate、compose、simulate 和 baseline 必须始终使用 `ride-cancellation`；出现“库不可见”时应回到本幕检查项目作用域和契约库，不要在第 3 幕临时删除 `libraryRefs`。
...
- 只使用前两幕已经形成并接入的 `ride-cancellation` 契约库，不用资源名称绕过逻辑契约，也不新建重复契约。
...
| --- | --- | --- |
| 1 世界观 | 查询平台积木；创建并回读 `ride-cancellation` 设计态业务契约库 | 四项逻辑能力完整且都为“仅契约”；看板标记“待接来源、待用例检验” |
| 2 来源与样例 | 发现只读资源；逐项读取权威契约；更新同一库补齐实现指向；校验并保存样例 | 四项能力均已接入且契约一致；样例不暴露敏感值 |
| 3 口述规则 | 基于 `ride-cancellation` 读取当前参考；生成编排；最多三轮预览修正；gate 后绑定 receipt 保存 | reference/preview/gate/compose 使用同一库版本；看板有白话流程和规则矩阵 |
...
- 四个取消费沙箱来源没有全部准备好。
- 第 1 幕结束后没有可回读的 `ride-cancellation` 契约库，或四项逻辑能力不完整。
- 第 2 幕另建了同义契约库，或四项逻辑能力没有在同一库中完成来源绑定。
