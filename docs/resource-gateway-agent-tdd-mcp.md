...
`semantic-ranker-enabled` 在 v1.4.6 只表示部署请求。当前版本没有独立语义 ranker，因此即使配置为 `true`，初始化结果仍返回 `semanticRankerEffective=false` 和 `semanticRankerState=NOT_AVAILABLE`。能力召回继续使用确定性的业务契约匹配，不会把配置值伪装成已生效能力。
...
- ACTIVE GOLDEN 的受保护正文采用 365 天滚动 receipt。平台后台任务在到期前 30 天追加直接后继，并用 case-set revision CAS 切换权威 receipt；RETIRED 案例只保留 30 天恢复期。该任务不向 Codex 暴露工具、案例正文或资产坐标。
...
- 受控 Feature adapter 不持有真实 `FeatureEvaluationBackend`，受控 Instruction adapter 不持有真实 `InstructionDispatchChannel`。测试进程触达 HTTP、真实 Feature 后端或真实 Instruction 通道时，服务端在保存 GREEN evidence 前返回 `CONTROLLED_TEST_EGRESS_DENIED`。旧 `PLATFORM_AUTHORING` case-set 只有以当前 Instruction ref 为键的显式 `RETURN`、`ERROR`、`TIMEOUT` 或 `MUST_NOT_CALL` stub 可以转换为同一受控 adapter；空 stub 不会恢复真实 READ 通道。
...
- 库概览先于首个实体写入；Feature、Scenario、Instruction、Solution 四次写入必须携带当次
  `authoringPatternsFingerprint`，服务端在落库前与当前生产模板校验一致；证书关联该指纹和
  `snapshotFingerprint`，因此截图所示四实体属于同一次服务端创作上下文；
