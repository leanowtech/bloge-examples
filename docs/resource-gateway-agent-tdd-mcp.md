...
需要准备现场演示时，使用 [Resource Gateway Agent TDD 演示导演脚本](resource-gateway-agent-tdd-demo-script.md)。该脚本以“不写代码的客服政策负责人”为主角，按五幕业务旅程组织自然语言对话、看板核对、人工停点、成功信号和失败回退。第 1 幕先盘点业务能力库：业务含义完全一致的能力才可复用；缺失事实先建立业务契约与工程交接。工程履约后，Codex 必须重新读取能力库和当前契约；所有必需能力就绪后，才能继续规则、案例、验证和发布。业务画面只显示能力含义、状态与下一责任方，算子契约和绑定只出现在技术幕后。前一幕的交接门未通过时不得继续。
