...
来源：catalog 按 §7.3 规范化后（含 contractEntry 内 evidenceRoles/ownerRoles/externalFactRequirements 排序）的 Domain2 algorithm（RG-CS-CATALOG-SEMANTIC-v1）
...
来源：registry descriptor canonical JSON array（typeId 升序，3 个 set-like 数组升序）的 Domain2 algorithm（RG-CS-PRIMITIVE-REGISTRY-v1）
