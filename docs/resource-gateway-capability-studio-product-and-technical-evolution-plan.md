...
4. Provider 的 atomic binding fingerprint 必须同时包含现有 Authority binding 与上述 target binding；Provider 不得从 Stage Result、环境变量或自身报告推导缺失坐标。正式验收先用 target authority 构造可信上下文，逐项比较 Stage Result v2 的所有相关 projection，再执行既有 post-run Evidence/Owner verification。
...
2. 两份 Attestation 均由各自 Authority 使用受信 issuer、Scope、Key Set、逻辑时钟和有效期验证；Candidate 与 Environment 的 issuer 不得由 Resource Gateway 自签或互相代签。
...
4. Target binding 由部署 Authority out-of-band pin，且 `resultId/revision/contract`、`executionLeaseId`、两份 Attestation coordinate 和 trusted target identities 均可由独立 verifier 复算；Provider atomic binding fingerprint 与部署 pin 相等。
...
- Oracle 是：两份外部签名事实分别证明候选身份和目标环境；target binding 精确绑定 Stage Result v2 与执行租约；Provider atomic binding fingerprint、Stage Result v2 projection 和 post-run Authority Envelope v1 closure 可独立复算，且无任何跨 Authority 推导或隐式 fallback。
- 系统不变量是：执行前准入与执行后证据分层；候选和环境坐标 immutable；binding out-of-band 且不可由请求覆盖；legacy provider 可以继续消费 v2，但没有 target binding 时正式 target-bound verification 必须为 `BLOCKED`；不得静默扩大 v1 枚举或把缺失准入事实标成 `PASS`。
...
- Evidence manifest 必须恰好包含：两份 Attestation 原始 bytes/ref/fingerprint、issuer/Key Set/Scope/TTL 验证转录、Baseline/Demo Pack/execution intent exact refs、Stage Result v2 原始 bytes 与 projection compare、target binding 原始 bytes/ref/fingerprint、execution lease、trusted target identities、Provider atomic binding fingerprint、admission/执行窗口、失败或阻断 reason code、post-run Authority Envelope v1/evidence closure ref、Owner signoff 和 rollback decision。manifest 不含业务 Payload，自身必须有 fingerprint、保留期和独立复算命令。
...
| ID | 继承共享上下文 | 可执行验收标准与 Oracle | 直接失败/阻断 | 行级证据与 Owner |
|---|---|---|---|---|
| `CANDENV-AC-01` | `CANDENV-PRE-v1` | Candidate Attestation v1 为 closed object，canonical bytes、Schema、签名和 fingerprint 可独立复算 | 未知/重复字段、非法 canonical bytes 或签名不匹配为 `FAIL`；Schema/Key Set 不可用为 `BLOCKED` | Candidate attestation verifier；Candidate Authority + Security |
...
| `CANDENV-AC-05` | `CANDENV-PRE-v1` | Environment Attestation v1 精确绑定 Candidate Attestation coordinate，并独立复算 canonical bytes | 指向其他候选、坐标省略、签名或 fingerprint 不一致为 `FAIL`；Environment Authority 不可用为 `BLOCKED` | environment attestation verifier；Environment Authority + SRE |
...
| `CANDENV-AC-08` | `CANDENV-PRE-v1` | out-of-band target binding 精确绑定 `resultId/resultRevision/contractId/contractRevision/executionLeaseId` 与两份 attestation coordinate | 缺字段、调用方覆盖、错 revision/contract/lease 或从 Stage Result 自发现为 `FAIL`；部署 pin 缺失为 `BLOCKED` | binding bytes、pin receipt、lease record；Deployment + Platform |
| `CANDENV-AC-09` | `CANDENV-PRE-v1` | target binding 的 `trustedTargetIdentities` 与 Provider 真实装配身份 exact 相等，并进入 Provider atomic binding fingerprint | 身份漂移、二次装配、fingerprint 不一致或 fallback 为 `FAIL`；唯一 Provider/部署 classpath 不可用为 `BLOCKED` | atomic binding transcript、classpath pin；Platform + Security |
...
| `CANDENV-AC-11` | `CANDENV-PRE-v1` | admission window 内执行；legacy provider 无 target binding 时仍可被消费，但 formal target-bound verification 明确返回 `BLOCKED` | 缺 target binding 却 `PASS`、用旧坐标补全或窗口外复用为 `FAIL`；legacy capability/lease 缺失为 `BLOCKED` | capability probe、blocked result、lease timeline；Platform + Acceptance |
| `CANDENV-AC-12` | `CANDENV-PRE-v1` | post-run 只沿用既有 Evidence/Owner envelope 流程；Evidence manifest 与最终 closure exact 相等，12 项 obligation 全执行 | 缺 Evidence/Owner、闭包漂移、重放旧 attestation 或漏 obligation 为 `FAIL`；外部 Evidence/签署设施不可用为 `BLOCKED` | complete CANDENV manifest、v2 result、v1 envelope、Owner receipts；QA + Evidence Owner |

**迁移与回滚兼容规则：**

1. 先发布 `CANDENV-CONTRACT-v1` Schema、独立 verifier、target binding pin 和 capability probe；不修改 `Stage Acceptance Result v2`、`Authority Envelope v1` 的字节、字段、枚举或 verifier。
...
3. 新 Provider 采用双读：读取原 v2 Result 和 out-of-band target binding；只有 12 个 obligation 全部完成并通过既有 v2/v1 verifier，才允许正式流程进入 post-run Evidence/Owner 验证。缺失 target binding 不得降级为旧 `PASS`。
...
截至 2026-08-20 的开发观测为 Bundle loader 15/15、跨版本 Provider/TCK/CLI/shell 58/58、参考 Provider 6/6、Test Kit `clean verify` 1030/1030，均为 0 失败、0 错误、0 跳过；这些数字只能作为当次证据 manifest 的 observations，不能替代 24 个 `ABP-*` obligation、10 个 `AUTHBUNDLE-AC-*` 或企业外部签署。`S0-DEV-GOV-29` 仍只能是 `DEVELOPMENT_VERIFIED`，正式 `formalPassCount=0/27`、`NO_GO`。
