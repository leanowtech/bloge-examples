...
新增后端包与核心文件如下。清单只展示稳定边界，不枚举全部 Controller、Repository 和 receipt 类型：
...
com.leanowtech.bloge.gateway.testing.correctness
  ...
  workspace/
    ...
    *CorrectnessWorkspaceComponentSource.java
  coverage/
    CoverageInventoryService.java
  oracle/
    BusinessOracleService.java
    AssertionSetService.java
  scenario/
    ScenarioDraftSetV2Service.java
    ScenarioClosureValidator.java
  fixture/
    FixtureCatalogService.java
    FixtureMaterialService.java
    FixtureMaterialResolver.java
  compilation/
    CorrectnessCompilationService.java
    CorrectnessCompiler.java
    CorrectnessCompilationValidator.java
    CorrectnessScenarioLowerer.java
    CorrectnessAssertionLowerer.java
    CorrectnessFixtureRuleLowerer.java
    CorrectnessSourceMapBuilder.java
    CorrectnessCompilationCanonicalizer.java
    CorrectnessExecutionRiskAnalyzer.java
    CorrectnessPublicationService.java
  publication/
    CorrectnessPublicationRepository.java
    DatabaseCorrectnessPublicationRepository.java
  run/                           # COR-08
...
机器合同见
[`bloge-stored-coverage-inventory-v1.schema.json`](schemas/bloge-stored-coverage-inventory-v1.schema.json)、
[`bloge-coverage-inventory-changed-v1.schema.json`](schemas/bloge-coverage-inventory-changed-v1.schema.json)、
[`bloge-coverage-inventory-frozen-v1.schema.json`](schemas/bloge-coverage-inventory-frozen-v1.schema.json) 和
[`bloge-coverage-freeze-receipt-v1.schema.json`](schemas/bloge-coverage-freeze-receipt-v1.schema.json)，以及
[`bloge-coverage-impact-proposal-v1.schema.json`](schemas/bloge-coverage-impact-proposal-v1.schema.json)。Impact proposal
可以携带供作者审阅的义务文本差异，但不进入 outbox，也不存在自动 apply/freeze 语义；跨系统事件只携带 exact coordinate、数量和审核身份。
...
[`bloge-assertion-set-changed-v1.schema.json`](schemas/bloge-assertion-set-changed-v1.schema.json)。编译和命令回执合同见
[`bloge-assertion-compilation-report-v1.schema.json`](schemas/bloge-assertion-compilation-report-v1.schema.json) 与
[`bloge-oracle-approval-receipt-v1.schema.json`](schemas/bloge-oracle-approval-receipt-v1.schema.json)。canonical revision、head CAS 与
...
或 review authority；所有 Case 固定为 `EXPLORATORY`。认证预览端点为
`POST /api/visual/scenario-draft-sets-v2:migrate-v1-preview`，要求 `CORRECTNESS_WRITE` purpose，并对含 Given/expected 的响应强制
`Cache-Control: no-store, private` 与 `Pragma: no-cache`；普通 Workspace 不复用该 payload-bearing projection。
...
且 HIGH/CRITICAL 分母强制 author/reviewer 双人复核。Contract/DAG/Policy 的 target 或 source snapshot 漂移只产生稳定指纹的
...
clock、network、random 或 secret manager。v2 Scenario 继续降为现有 `FixtureBundle`、`TestSuite` 与 Assertion compiler 协议；任何
不支持语义都会生成 error diagnostic，不会静默丢弃。连续 100 次编译、locale/timezone 变化、phase composition、source map 和
payload leakage 均有回归测试。
