...
| RG-BM-003 | P0 | PackageCompiler | **内核与部署纵向切片已完成：** Authority freeze/fence、确定性 Readiness/Snapshot/Closure、PostgreSQL append-only facts、跨副本 revision allocator、durable receipt、认证 compile/read API、动态 readiness 和独立 Test Kit 复验；真实 Authority Adapter 继续实施 | 100 组乱序、tamper、Scope、cycle、TOCTOU、H2/PostgreSQL 双连接、响应丢失和 Spring HTTP 全绿；完整退出仍需真实 Adapter 与大型编译容量认证 |
...
| RG-BM-008 | P1 | 实现交付与 Conformance | SDK scaffold、runtime port、binding API、同源 suite | 实现绑定后复用原套件；模拟/实现 Diff 可追溯 |
| RG-BM-009 | P1 | L0-L3 关系与 Impact | BusinessAssetLink、reverse index、Deep Link、change event | L0 变化可定位 L1/L2/L3 受影响资产和场景 |
| RG-BM-010 | P1 | Package Evidence/Fidelity | package evidence index、portfolio fidelity、stale/drift task | 无单一总分掩盖维度；每个结论回到 exact source |
