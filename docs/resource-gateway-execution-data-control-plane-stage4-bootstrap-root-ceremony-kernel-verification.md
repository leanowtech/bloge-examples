...
时刻 key lifecycle。未知 root key 与无效签名使用不同的内部 reason，为下一步 unknown-key refresh
提供精确触发条件。

## 6. Durable floor 语义校正

bootstrap-root floor 与旧 serving-inventory floor 的 genesis 语义不同：一个新副本可能首次看到
sequence N，但它已经从 pinned genesis 重放了完整 1..N 链。因此空 floor 必须允许提交任意已完整
验证的 head；floor 非空后才只接受 exact current 或 contiguous successor，并拒绝 rollback、fork、gap
和 predecessor mismatch。

本步只冻结专用 floor interface；数据库实现与事务测试属于下一接线步骤。直接复用“空 floor 只能从
sequence 1 开始”的旧实现是错误的，已经明确禁止。
...
128 代上限、`additionalProperties: false` 和 public-only 约束。本步聚焦执行 9 tests，0 failures、
0 errors、0 skips。
...
- strict HTTPS/ETag/no-redirect complete-bundle source；
- 专用数据库 floor，允许首次 verified head N 并事务性拒绝后续 rollback/fork/gap；
- refresh failure 立即使 root verification fail closed；
- unknown root key 全局 cooldown single-flight refresh；
...
- restart、并发、`304`、过期、错误 successor 和数据库重建测试。
