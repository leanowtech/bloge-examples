...
It makes the queue repository the only authority allowed to publish cancellation, expiry, or
worker-failure terminal states.
...
4. signed-parent winner verification that binds scope, request, classification, evidence, and
   detached signature;
5. queue convergence to `SUCCEEDED` when valid signed parent evidence won the race.
...
The 51 focused tests pass with zero failures, errors, or skips. In addition to queue lifecycle and
