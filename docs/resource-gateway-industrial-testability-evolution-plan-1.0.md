...
floor 实现免重启轮换。bootstrap-root 消费侧完整链热轮换、不接触私钥的 ceremony producer 和
可嵌入数据库权威 maker/checker journal/coordinator 现已闭合；企业 IAM/PDP、HSM/KMS custody、执行
heartbeat、后台恢复调度、发布失败对账、根源 anti-equivocation 与生产认证、HA/chaos/外部 SLO 仍待完成。
...
kernel、不接触私钥的 ceremony producer 与可嵌入数据库权威 maker/checker journal/coordinator 已闭合；
企业 IAM/PDP、执行 heartbeat、后台恢复调度、发布失败对账、根源 anti-equivocation、
