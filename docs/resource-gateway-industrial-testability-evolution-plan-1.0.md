...
`3f+1 / 2f+1` Byzantine quorum，并由 staging 强制 `f>=1`。bootstrap-root 轮换 ceremony、external
notary key 热轮换与生产认证、HA/chaos/外部 SLO 仍待完成。
...
external-first 双流 non-equivocation 现已闭合；neutral cohort/crypto kernel、bootstrap-root ceremony、
external notary key 热轮换、KMS/HSM/mTLS/HA 与跨数据库/DR 认证仍是后续工作。验证见
