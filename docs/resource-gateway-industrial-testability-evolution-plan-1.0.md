...
处理。测试执行的即时四维配额已由独立 admission authority 执行；原子远程 worker acquisition、
排队/公平/优先级 backpressure 与跨进程 supervisor 仍待实现。
...
远程 worker acquisition、hard cancellation、非 H2 方言和生产负载认证，因此不能宣称完整运维产品化。
