...
14. **受信组合持久化基座与 execution/checkpoint 聚合已闭合，BLOGE suspend/async 生命周期尚未闭合**：`bloge.executionServiceStateSnapshot.v1` 在公平读写锁边界原子冻结 logical time、哈希 scope cursor 与 usage，绑定 plan/binding-set fingerprint；`bloge.fixtureConsumptionStateSnapshot.v1` 约束 rule use 与哈希动态 occurrence cursor，`InvocationRecorder` 只在不存在待执行 binding/执行中 attempt 的静止调用边界 capture，非静止边界 fail closed，restore 拒绝篡改和向已运行 recorder 合并，`maxUses` 通过 CAS 原子消费防止并发超领。运行开始即只把版本化哈希 cursor key 放入游标表，持久值不含原始坐标；该哈希是稳定伪名而非低熵值保密机制。`bloge.durableTestExecutionCheckpoint.v1` 再把完整 plan/exact fixture/side-effect/identity 摘要、两类 state、engine closure、scope 与 owner/epoch/revision fence 绑定为单一内容身份。受信仓库允许 engine mutation 加入同一 test-runtime 本地事务，陈旧 fence、回调故障和并发 CAS 输家均整体回滚；读取端重算嵌套/整体指纹并核对索引列，cursor/time/usage/version 只可单调前进。BLOGE 源码提交 `bcbb19694` 提供公共 `CheckpointFailurePolicy.FAIL_FAST`；RG 的 test-profile durable session 现已强制该策略，以调用方指定 execution id 开启单执行 stage，继承完整 `ExecutionOptions` 的 operator resolver/provider，并把 BLOGE `ExecutionStore` lifecycle/lease 与 node/loop/sequential-foreach `ExecutionCheckpointStore` 状态分别冻结后，再以 `bloge.testDurableStateMutation.v1` 聚合为一个可幂等重试、与 engine id/完整 `EngineState` 强绑定的 mutation。跨实例竞态证明只有 control CAS 胜者的 execution 状态可提交，关闭 stage 后 mutation 失效；冷读可重建完整 `ExecutionInstance`。协议不携带 seed、原始 scope、fixture payload 或 authority value，内容指纹也不是签名。`WaitStore`（包括 timer wait）、`WorkItemStore`、stream offset/checkpoint 协议、断点前 invocation/attempt evidence、exact dependency 重新授权、公共 durable run/resume/owner claim 仍未完成，因此仍不能声明 cold-start durable resume。

### 十一、明确排除（v1 不做）

流式时间行为（STREAM）、sandbox conformance 双验与反熵漂移检测（保真度 F5）、identity/feature-flag/test-secret authority、BLOGE `WaitStore`/`WorkItemStore` 事务参与 adapter、stream offset/checkpoint 协议与公共 resume/owner claim、真实 ANEKE cross-version conformance、独立 witness gossip/跨域一致性证明、确定性并发 scheduler、独立 test-runtime 部署、mutation/property testing。F4 record-replay、动态 selector、签名证据链、run-scoped TIME/RANDOM/UUID、environment-dependent built-in resolver、semantic result fingerprint、provider-state snapshot/compiler restore seam、受信组合 checkpoint 本地事务基座与 BLOGE execution/checkpoint staged aggregate 已经落地；其余能力均已在北极星文档 Stage 3-5 有宿主。
