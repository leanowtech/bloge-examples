...
进程内强绑定 inventory、generation 防回滚/同代漂移和有界公平 worker kernel 亦已闭合；provider-confirmed
cancellation/process isolation、signed dynamic resolver inventory、durable 跨副本 cursor/sharding、公平调度与
fleet 产品接线、publisher mTLS/pinning、response-key 热轮换、根源 anti-equivocation 与生产认证、HA/chaos/外部 SLO
仍待完成。
...
跨 root-set recovery 的进程内强绑定 inventory/generation/fair-worker kernel 已闭合；企业 IAM/PDP、
provider-confirmed cancellation/process isolation、signed dynamic resolver inventory、durable 跨副本
cursor/sharding/fairness 与 fleet 产品接线、publisher mTLS/pinning、
