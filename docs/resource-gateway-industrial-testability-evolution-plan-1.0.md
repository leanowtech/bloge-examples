...
| Stage 4 deterministic runtime | 进行中 | run-scoped TIME/RANDOM/UUID、environment-dependent built-in resolver、effective plan v3 provider binding/usage audit、`bloge.testRunEvidence.v2.semanticResultFingerprint`、脱敏后重算、重复运行 `GraphContext` 隔离、签名前/读取时一致性校验，以及 `bloge.executionServiceStateSnapshot.v1` 原子快照、精确 compiler 恢复、plan/config/policy/cursor fail-closed 校验已完成；BLOGE durable/suspend store 接线、identity/flag/secret authority 和确定性并发待完成 |
...
14. **provider-state 恢复协议已闭合，durable 生命周期尚未闭合**：`bloge.executionServiceStateSnapshot.v1` 在公平读写锁边界原子冻结 logical time、哈希 scope cursor 与 usage，绑定 plan/binding-set fingerprint；恢复端重算 fingerprint、restorable policy 和 cursor/usage 闭合，system random 等不可精确恢复状态 fail closed。协议不携带 seed、原始 scope 或 authority value。`snapshotFingerprint` 只提供内容身份与意外篡改检测，不提供来源认证；未来 adapter 必须依赖受信 store、fence 或签名 attestation。BLOGE suspend/durable checkpoint 尚未把 plan、fixture consumption cursor、in-flight node/timer/journal 与该 snapshot 原子持久化，当前也没有公共 resume endpoint，因此只能声明“可恢复 provider-state seam”，不能声明 cold-start durable resume。

### 十一、明确排除（v1 不做）

流式时间行为（STREAM）、sandbox conformance 双验与反熵漂移检测（保真度 F5）、identity/feature-flag/test-secret authority、BLOGE durable/suspend store 对 provider-state/plan/consumption cursor 的原子接线、真实 ANEKE cross-version conformance、独立 witness gossip/跨域一致性证明、确定性并发 scheduler、独立 test-runtime 部署、mutation/property testing。F4 record-replay、动态 selector、签名证据链、run-scoped TIME/RANDOM/UUID、environment-dependent built-in resolver、semantic result fingerprint 与 provider-state snapshot/compiler restore seam 已经落地；其余能力均已在北极星文档 Stage 3-5 有宿主。
