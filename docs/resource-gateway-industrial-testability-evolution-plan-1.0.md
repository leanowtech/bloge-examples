...
replay 也留下事件。claim token 只进入成功 claim response。仍缺 finding retention/archive、metrics/SLO、
外部 WORM/tamper-evident audit anchoring、非 H2 方言和生产负载认证，因此不能宣称完整运维产品化。
...
当前严格验收：Resource Gateway `-Pfrontend clean verify` 共 2086 tests、0 failures、0 errors、34 个既有条件跳过，
...
capability/schema/application 组合聚焦 24 tests；authenticated durable GRAPH creation 的 runtime/repository/
