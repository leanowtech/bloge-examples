...
处理。原子远程 acquisition、backpressure、配额/容量控制与跨进程 supervisor 仍待实现。
...
fail closed。各 authority table 已增加运维时间/状态索引。仍缺配额与 admission/backpressure 执行、
外部 alert routing、自动清理命令、外部 WORM/tamper-evident audit/archive anchoring、非 H2 方言和
生产负载认证，因此不能宣称完整运维产品化。
