...
启用，production 仍无 bean。该子步当时尚未包含独立 test-kit，已由第十三子步闭合；真实 IAM adapter、
取消语义审计事件、fleet membership、poison repair、非 H2/soak/chaos/DR 与 hard process cancellation
仍未闭合。验证见
...
既有浏览器条件跳过并成功重打包。真实 IAM adapter、取消 semantic audit、fleet membership、
