...
test/staging 单 root-set Spring publication composition/aggregate health 亦已闭合；provider-confirmed
cancellation/process isolation、默认 Spring recovery 接线、跨 root-set
...
authenticated-conflict quarantine 及 test/staging 单 root-set Spring publication composition 已闭合；企业
IAM/PDP、provider-confirmed cancellation/process isolation、默认 Spring recovery 接线、跨 root-set worker、publisher mTLS/pinning、
