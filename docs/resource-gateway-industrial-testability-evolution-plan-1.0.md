...
floor 实现免重启轮换。bootstrap-root 消费侧完整链热轮换和不接触私钥的 ceremony producer kernel
现已闭合；durable ceremony service/审批恢复状态机、HSM/KMS custody、根源 anti-equivocation 与生产
认证、HA/chaos/外部 SLO 仍待完成。
...
kernel 与不接触私钥的 ceremony producer kernel 已闭合；durable ceremony service/审批恢复状态机、
