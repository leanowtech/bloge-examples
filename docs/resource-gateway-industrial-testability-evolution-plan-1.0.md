...
14. **受信组合持久化基座已闭合，BLOGE durable 生命周期尚未闭合**：`bloge.executionServiceStateSnapshot.v1` 在公平读写锁边界原子冻结 logical time、哈希 scope cursor 与 usage，绑定 plan/binding-set fingerprint；`bloge.fixtureConsumptionStateSnapshot.v1` 约束 rule use 与哈希动态 occurrence cursor；`bloge.durableTestExecutionCheckpoint.v1` 再把完整 plan/exact fixture/side-effect/identity 摘要、两类 state、engine closure、scope 与 owner/epoch/revision fence 绑定为单一内容身份。受信仓库允许 engine mutation 加入同一 test-runtime 本地事务，陈旧 fence、回调故障和并发 CAS 输家均整体回滚；读取端重算嵌套/整体指纹并核对索引列，cursor/time/usage/version 只可单调前进。协议不携带 seed、原始 scope、fixture payload 或 authority value，内容指纹也不是签名。BLOGE checkpoint/wait/timer/work-item/stream store 尚未调用该边界，`InvocationRecorder` 尚未 capture/restore cursor，exact dependency 重新授权与公共 resume/owner claim 也未完成，因此仍不能声明 cold-start durable resume。
