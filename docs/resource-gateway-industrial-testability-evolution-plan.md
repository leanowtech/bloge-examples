...
> Stage 4 physical-attempt 状态校正：表内“terminal projection Spring/worker wiring 待完成”已被第十四、
> 十五增量部分取代。durable work registration/lifecycle、bounded one-shot worker 与产品 proof resolver
> 已闭合；仍待完成的是自主 scheduler、Spring test/staging composition、telemetry/health/capability、fairness/cohort、
> backfill/retention 和生产认证，因此不能把 worker core 解读为已开放产品能力。
