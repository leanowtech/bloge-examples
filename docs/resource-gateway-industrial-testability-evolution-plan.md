...
下述独立反熵循环处理。公开 polling/acquisition、backpressure、容量 SLO 与跨进程 worker
supervisor 仍未提供。
...
和 replay 也追加事件，claim token 只出现在成功 claim response。日志仍只有聚合计数。当前剩余 finding
retention/archive、SLO/metrics、外部 WORM/tamper-evident audit anchoring、非 H2 方言与生产负载认证。
...
当前严格验收基线：Resource Gateway `-Pfrontend clean verify` 共 2086 tests、0 failures、0 errors、
34 个既有条件跳过，真实浏览器回归与 Spring Boot JAR 打包成功；Canvas suite 聚焦 68 tests、
...
独立追加；capability probe、权威 JSON Schema、运行配置和操作手册同步。应用层审计无 update/delete API，
但仍缺外部 WORM/tamper-evident anchoring、finding retention/archive、SLO/metrics、非 H2 方言与生产负载认证，且仍无公开 worker poll、
