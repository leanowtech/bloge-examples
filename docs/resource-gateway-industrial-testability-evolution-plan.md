...
bootstrap-root 消费侧完整链热轮换与不接触私钥的 ceremony producer kernel 现已闭合；durable
ceremony service/审批恢复状态机、HSM/KMS custody、根源 anti-equivocation 与生产认证、HA/chaos/
外部 SLO 仍待完成。
...
cohort/crypto kernel 与不接触私钥的 bootstrap-root ceremony producer kernel 已闭合；durable ceremony
service/审批恢复状态机、根源 anti-equivocation、KMS/HSM/mTLS/HA 与跨数据库/DR 认证仍是后续工作。验证见
...
可执行 JAR。该闭环
只证明 pure kernel 的协议产物原子性，不证明 HSM 签名副作用可回滚；durable ceremony idempotency
journal、maker/checker、超时恢复、HSM/KMS/mTLS adapter、publisher anti-equivocation/HA/DR 仍是下一层
交付门禁。验证和嵌入式用法见
[bootstrap-root ceremony kernel verification](resource-gateway-execution-data-control-plane-stage4-bootstrap-root-ceremony-kernel-verification.md)。
