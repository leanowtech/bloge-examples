...
signed challenge-bound HTTPS transport、动态 trust/revocation 与 HA/SLO 仍是下一增量。
完整 Resource Gateway `clean verify` 执行 3089 tests，0 failures、0 errors、2 个条件浏览器跳过，
两个真实浏览器套件的 35 个配置测试完成并成功生成 Spring Boot 可执行 JAR；独立 test-kit `clean verify`
执行 229 tests，0 failures、0 errors、0 skips，并通过权威 Schema 打包、普通/shaded JAR 与 public
