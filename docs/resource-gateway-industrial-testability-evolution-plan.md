...
Byzantine quorum，并由 staging 强制 `f>=1`。bootstrap-root 轮换 ceremony、external notary key
热轮换与生产认证、HA/chaos/外部 SLO 仍待完成。
完整 Resource Gateway `clean verify` 执行 3188 tests，0 failures、0 errors、2 个条件浏览器跳过，
...
JavaDoc 门禁。验证见 [Stage 4 test-secret trust-root rotation verification](resource-gateway-execution-data-control-plane-stage4-test-secret-trust-root-rotation-verification.md)。
...
cohort/crypto kernel、bootstrap-root ceremony、external notary key 热轮换、KMS/HSM/mTLS/HA 与跨数据库/DR
认证仍是后续工作。验证见
...
前检查 flags、HTTPS、非空 key/endpoint、quorum 与 timing。capability 分别公开 publication/root external
anchor、Byzantine quorum 和 composite readiness，health 仅含聚合计数。该能力不等于 bootstrap-root
ceremony、external notary key 热轮换或 notary 产品/HA/DR 认证。验证见
