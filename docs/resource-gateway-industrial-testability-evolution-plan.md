...
与独立 HMAC request-index tombstone、在线轮换、旧行惰性迁移及 live-key readiness 已实现；外部审批绑定、
...
重键关闭低熵 request ID 的 database-only 离线枚举面。仍缺 request-index N/N-1 mixed-binary
write-mode 协议、外部审批/工单绑定、法律保留/备份擦除证明、外部
