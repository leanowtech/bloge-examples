...
以及数据库原子 recovery acquisition/退避/attempt budget 和单 lane scheduler 现已闭合；企业 IAM/PDP、
HSM/KMS custody、provider-confirmed cancellation/process isolation、默认 Spring recovery 接线/跨 root-set
worker、发布失败对账、根源 anti-equivocation 与生产认证、HA/chaos/外部 SLO 仍待完成。
完整 Resource Gateway `clean verify` 执行 3291 tests，0 failures、0 errors、2 个条件浏览器跳过，
...
以及数据库原子 recovery acquisition/退避/attempt budget 和单 lane scheduler 已闭合；企业 IAM/PDP、
provider-confirmed cancellation/process isolation、默认 Spring recovery 接线/跨 root-set worker、发布失败对账、
