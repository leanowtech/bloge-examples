...
当前严格验收基线：Resource Gateway `clean verify` 共 1921 tests、0 failures、0 errors、
...
durable checkpoint/aggregate 聚焦 49 tests 全绿。
...
`bloge.durableTestExecutionCheckpoint.v1`：完整 effective plan（因此保留 exact replay refs）、fixture
...
进入同一内容指纹。受信数据库仓库允许 BLOGE state mutation 使用同一个 test-runtime datasource
...
Stage 4 仍未让 `WorkItemStore` 参与该事务，也尚无
...
execution/checkpoint/wait aggregate 已完成，但 deferred work dispatch 与公开恢复闭环仍未完成，
