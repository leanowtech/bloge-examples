...
和 provider checkpoint 只携带分域配置指纹/哈希 scope，不携带原始值。SECRET 继续拒绝，直到
opaque ref 与外部测试密钥权威闭合，避免把凭据伪装成普通 fixture。
完整 Resource Gateway `clean verify` 执行 3026 tests，0 failures、0 errors、2 个条件浏览器跳过，
34 个配置的真实浏览器测试完成并成功生成 Spring Boot 可执行 JAR；独立 test-kit `clean verify`
执行 228 tests，0 failures、0 errors、0 skips，并通过权威 Schema 打包、普通/shaded JAR 与 public
