...
第五十三增量第二十一子步第一阶段建立 restart-free serving-inventory 运行密钥轮换内核。一个版本化 canonical
material 原子携带 deployment/witness 两组 runtime keys、阈值、四个独立 trust domain、scope/protocol/policy、
...
真实 Ed25519 端到端测试覆盖 A -> B 无重启轮换和撤销。当前尚未接入 Spring/config/staging/capability/Schema，
bootstrap roots 仍需部署 ceremony，整库备份回退仍需外部不可回退锚。验证见
...
相关聚焦门禁执行 31 tests，0 failures、0 errors、0 skips；完整 Resource Gateway `clean verify` 执行
2731 tests，0 failures、0 errors、2 个条件浏览器跳过，并成功重打包可执行 JAR。
