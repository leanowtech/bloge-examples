...
bootstrap-root 轮换 ceremony、external non-equivocation、HA/chaos/外部 SLO 仍待完成。
完整 Resource Gateway `clean verify` 执行 3171 tests，0 failures、0 errors、2 个条件浏览器跳过，
...
动态 `ACTIVE/REVOKED` publication、独立 witness、跨重启 database publication/witness floor、原子刷新与
HTTP 取密前后门禁现已闭合；neutral cohort/crypto kernel、运行 trust-root 轮换、外部 non-equivocation、
KMS/HSM/mTLS/HA 与跨数据库/DR 认证仍是后续工作。验证见
