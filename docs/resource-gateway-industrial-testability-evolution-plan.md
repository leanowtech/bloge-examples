...
M-of-N Ed25519、数据库 revision floor 与全副本 inventory-generation 收敛已落地；动态 inventory
refresh/revoke、signed witness 与 HA/chaos/外部 SLO 仍是下一增量。
完整 Resource Gateway `clean verify` 执行 3139 tests，0 failures、0 errors、2 个条件浏览器跳过，
...
JavaDoc 门禁。验证见 [Stage 4 execution-services verification](resource-gateway-execution-data-control-plane-stage4-execution-services-verification.md)。
