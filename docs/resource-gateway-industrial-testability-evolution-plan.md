...
worker dispatch receipt；内部同步 cold-signal primitive 已可验证，但尚无 dispatcher 消费、worker
poll/run/heartbeat、terminal evidence 与公开 resume 编排。
...
当前严格验收基线：Resource Gateway `clean verify` 共 1978 tests、0 failures、0 errors、
...
durable checkpoint/aggregate/public owner-claim/internal recovery/authorization-bound dispatch 聚焦 107 tests 全绿。
...
heartbeat/terminal evidence、dispatch 消费与 cold-start 重建校验、stream/event fixture、确定性并发调度、
