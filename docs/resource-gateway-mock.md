...
| 文档属性 | 内容 |
|---|---|
| 状态 | Accepted / In implementation；Stage 0 至 Stage 4 的协议、执行、状态世界、Scenario rehearsal、证据与治理闭环已完成。Stage 5 已完成 owner-approved Fidelity inventory、无总分 profile、signed Scenario 与 Shadow source adapter、durable Shadow queue/worker/lifecycle、双重观测 online authority、数据库共享 execution guard、managed trust distribution、在线 v1/detached v2 job、exact signed source binding、detached 双 connector/source resolution，以及 regional TEE online-baseline consumer、strict transport、same-input online candidate strict HTTP authority、独立 online paired-source v2 resolver、动态 readiness、双 loopback HTTP role synthetic regional certification provider、baseline/candidate 双独立 JVM 与双私有 CA/mTLS/SPKI/工作负载身份认证、candidate committed-response-loss 硬退出与无二次生成恢复、双角色同端口 server leaf 双 pin 滚动及连接池重握手认证、candidate commit 后的截断/延迟/停滞响应故障与无二次生成恢复、PostgreSQL 14.22 双连接/双 worker 的 shared queue、guard 与 fencing 认证、v1/v2 proof 数据库兼容迁移、durable worker retry/crash/takeover 组合认证、signed authoritative outcome observation、pre-treatment cohort closure、delayed/censored/conflicting reconciliation、独立业务 authority 边界、受保护 outcome inbox API、strict transport、append-only lifecycle、条件装配、bounded scheduler、PostgreSQL 并发 admission/takeover 认证、细粒度 readiness、outcome source adapter、独立 Test Kit observation/lifecycle verifier，以及 selected-population manifest/chunk、durable registry、coherent assessment cut、受保护 API、三角色隔离、历史 source closure、24 份 strict Schema、独立 Test Kit verifier/client、固定兼容语料、配额隔离的可恢复 staged upload/finalize、数据库时钟驱动的持续 assessment projection、owner/epoch fencing、丢响应 adoption、bounded retry/quarantine、同事务 append-only projection lifecycle、projection/lifecycle 双 CAS 的受控 quarantine remediation 和 PostgreSQL 双实例认证。企业 root-policy/control-plane connector、获授权的 production regional provider/candidate authority、PostgreSQL 多进程/HA/网络分区认证、跨区域 client/CA 撤销轮换、客户生产 outcome connector、object-store/streaming population admission、生产规模连续投影认证、event-driven wakeup、calibration correlation、drift 自动降级、零 DSL case 调整和企业身份任务认证继续实施；任何 protocol/API/connector/readiness 子项均不得冒充完整生产数据面 readiness |
...
- Selected-population completeness 已推进为受保护产品纵切：独立 selection/deletion/assessment
  workload 角色提交完整 denominator、合法删除和 coherent assessment；数据库保存不可变 population/chunk/
  disposition/assessment 与历史 source closure；24 份 strict Schema、Test Kit HTTP client 和无服务端依赖
  verifier 已同步交付，PostgreSQL 双实例认证覆盖并发准入、跨实例 cut 失效与来源重建；server-produced
  public-only 固定 fixture 已闭合 root/chunk/outcome/disposition/assessment/source-page 的跨模块兼容门禁。
  新增可恢复 staged upload：caller-stable intent、乱序 content-addressed chunk、payload-free progress、
  exact replay、scope 级并发/字节配额、TTL/终态回收、finalizer lease/epoch fencing、abort 和既有
  selection-authority admission 复用已形成一条完整纵切。持续 assessment projection 进一步以
  server-owned stream/revision、数据库时间 freshness、owner/epoch lease、source-closure 去重、
  committed-head adoption、bounded retry/quarantine、同事务 append-only lifecycle、调用方游标验链和
  opt-in scheduler 让迟到结果自动撤销旧 current claim；受控 quarantine remediation 以 reviewed
  projection/lifecycle 双 CAS、actor-bound exact replay、不可变 receipt 和同事务 mandatory audit
  恢复调度资格，同时不改写累计尝试、来源闭包或历史 freshness。
...
- 当前验证基线：前端 Vitest `165/165` 全绿并完成 TypeScript/Vite 生产构建；带
  `-Pfrontend` 的 Resource Gateway 完整门禁为 `5612` 项 Java 测试、0 失败、0 错误、
  4 项按环境条件跳过，其中真实 Chrome DOM/工作流套件共 36 项、32 项执行通过、4 项条件跳过，
  可执行 Boot JAR 同步通过。独立 test-kit `526/526` 全绿，195 份打包 Mirror 资源（其中
  177 份 JSON Schema）完成 fail-closed
  引用闭包与打包验证，公共
...
| Fidelity/Outcome | 99.9% | owner-approved inventory、无总分七维 profile、fail-closed freshness/低样本/abstention/source debt、Wilson 95%、exact lineage、strict Schema、managed signing、受保护 API、Scenario source、v3 Shadow authority/policy/source closure、durable queue/worker/lifecycle、shared guard、root-threshold managed trust、在线 v1/detached v2 job、exact signed source binding 与 detached data-plane 已落地；regional TEE baseline consumer、same-input candidate strict HTTP authority、独立 online paired-source v2 resolver、分层动态 readiness、双 loopback HTTP role provider、baseline/candidate 双独立 JVM、双私有 CA/mTLS/SPKI/工作负载身份、错误身份与跨角色信任拒绝、candidate committed-response-loss 恢复、双角色 server leaf 双 pin 滚动与连接池重握手、candidate commit 后的截断/延迟/停滞响应故障和单 generation 恢复、PostgreSQL 14.22 双连接/双 worker 的 ordinal 唯一占用、单次发布、lease takeover fencing、terminal comparison 唯一性与 shared guard 并发预算、v1/v2 proof 数据库迁移、durable worker transient retry/crash/takeover、四角色公钥隔离、request/job/comparison/lifecycle/source/proof 一次性跨进程复验，以及 signed authoritative outcome observation、pre-treatment cohort/stratum closure、event-time delayed/censored/conflicting reconciliation、受保护 outcome inbox API、strict transport、append-only lifecycle、条件 worker/scheduler、PostgreSQL concurrent admission/takeover、六项 readiness、独立 lifecycle verifier 和 TypeScript/Go 固定向量认证；selected-population manifest/chunk、合法删除、durable registry、coherent assessment cut、受保护 API、历史 source closure、三角色隔离、24 份 strict Schema、独立 verifier/client、server/Test Kit 固定全闭包 fixture、配额隔离的可恢复 staged upload/finalize，以及数据库权威 continuous projection、exclusive freshness、owner/epoch fencing、source-closure 去重、丢响应 adoption、bounded retry/quarantine、同事务 append-only lifecycle、projection/head 双 CAS、受控 quarantine remediation、调用方游标验链与可选 scheduler 也已闭合；缺企业 root-policy/control-plane connector、跨区域传播及 client/CA 撤销轮换认证、获授权的 production regional provider/candidate authority、PostgreSQL 多进程/HA/网络分区认证、客户生产 outcome connector、object-store/streaming 总体摄取与生产规模连续投影认证、event-driven wakeup、selected-population/Mirror Evidence 完整跨语言认证、fidelity-to-outcome correlation、drift 降级和校准工作台 |
...
结论：基础设施准备度约 95%，固定权重理想态完成度为 91.37%。剩余差距 8.63%，主要矛盾已经
...
并把已完成 durable admission/worker/scheduler/continuous projection 的 cohort/outcome 产品边界接入
生产权威结果源，为 selected-population 增加 object-store/streaming 总体摄取与生产规模认证，再推进
fidelity-to-outcome calibration，持续校准模拟与真实业务之间的差距，在来源漂移后自动撤销
serving 结论”。
...
### 3.55 2026-07-27 Selected-population 受保护产品与独立复验闭包

3.54 留下的核心问题不是“再增加几个 CRUD”，而是把一个可能改变业务结论的统计分母交给谁、以什么
身份提交、如何证明没有漏项，以及服务端响应丢失后调用方能否无歧义重试。若 selection、legal
deletion、outcome producer 和 assessment projector 共用一个权限，恶意或误配置的结果生产者就可以
自己缩小分母、制造删除豁免并宣布 completeness；若只返回 assessment 汇总，ANEKE 又无法证明它来自
哪些历史 observation/disposition。

本轮沿病根完成一条受保护纵切。

**协议与职责分离**

| 角色 | Purpose | 默认 workload group | 能做什么 | 明确不能做什么 |
|---|---|---|---|---|
| Selection authority | `MIRROR_OUTCOME_SELECTION` | `RESOURCE_GATEWAY_OUTCOME_SELECTION_AUTHORITY` | 提交完整 immutable population/chunks | 不能提交删除、不能触发 assessment |
| Deletion authority | `MIRROR_OUTCOME_DISPOSITION` | `RESOURCE_GATEWAY_OUTCOME_DELETION_AUTHORITY` | 为 exact selected member 提交合法删除 disposition | 不能改 denominator、不能伪造 outcome |
| Fidelity projector | `MIRROR_FIDELITY_GOVERNANCE` | `RESOURCE_GATEWAY_FIDELITY_PROJECTOR` | 对 exact population revision 请求 coherent assessment | 不能选择成员或制造删除 |
| Governance reader | `GOVERNANCE_EVIDENCE_INGESTION` 等受限读 purpose | 由 Integration identity policy 决定 | 读取 exact fact 和历史 source closure | 不能从 query/body 声明 scope |

三种写操作使用不同 `IntegrationOperation`，先认证再解码；只有 `SERVICE`/`WORKLOAD` actor 和 exact
group 才能进入对应应用服务。完整 tenant/organization/project/environment/region 由认证身份派生，
`prod`、`production`、`live` 物理无路由。具备一个角色不隐式获得另两个角色。

**受保护 API**

| 操作 | Route | 响应语义 |
|---|---|---|
| 准入完整 population | `POST /api/mirror/outcome-selected-populations` | `201` + signed immutable bundle；exact retry 返回 `idempotentReplay=true` |
| 读取 exact/current population | `GET .../{populationId}/revisions/{revision}` / `GET .../{populationId}/latest` | 重验 chunk/root closure、RG seal 与 selection authority |
| 准入/读取 legal disposition | `POST .../{populationId}/dispositions` / `GET .../dispositions/{id}/revisions/{revision}` | exact member、retention/deletion approval、deletion authority 闭包 |
| 生成/读取 assessment | `POST .../{populationId}/assessments` / `GET .../assessments/{id}/revisions/{revision}` | bounded coherent-cut retry、签名 totals/strata/source-set fingerprints |
| 分页读取 assessment 来源 | `GET .../assessments/{id}/revisions/{revision}/sources?afterGlobalOrdinal=...&limit=...` | content-addressed、member-ordinal ordered 历史页 |

decoder 对 duplicate/unknown/missing field、trailing JSON、深度超过 64、节点超过 100 万和越界 body
失败关闭。population 完整提交当前硬上限为 64 MiB；disposition 和 assessment 使用更小的专用上限。
所有写请求允许 unsigned candidate，服务端在外部 authority 验真后分配可信 `attestedAt`、内容地址和
Resource Gateway seal；已经签名的持久化事实则必须完整匹配。

**可恢复事务与来源证明**

- population、disposition、assessment 都以 unsigned ingestion-material fingerprint 区分 exact retry
  和正文漂移。首次 recovery 查询与并发提交之间若发生竞态，应用服务会在 `CONTENT_CONFLICT` 后重新
  读取并复验已提交事实；因此“数据库已提交但响应丢失”和两个相同命令并发不会退化成伪冲突。
- repository mutation 与 success audit 在同一事务；audit 不可用时事实不落库。外部 selection、
  business-outcome、deletion authority I/O 全部发生在短事务外。
- assessment source index 在 assessment 提交事务中冻结 exact observation/disposition refs、global
  ordinal、source kind 和 set fingerprint。读取端不能用当前 head 补历史；分页从 cursor 0 到
  `complete=true` 才构成完整来源证明。
- PostgreSQL 14.22 双 datasource/transaction-manager 认证证明并发首次 population admission 收敛为
  create + replay；另一实例追加 outcome 后旧 cut 必须 `CUT_STALE`，新 assessment 和完整 source
  closure 可由另一实例重建。H2 同时覆盖重启读取、索引篡改、角色混淆、authority outage 和 audit
  rollback。随后补充的 public-only 固定 fixture 由服务端生成、Test Kit 独立消费，覆盖完整
  denominator、`MATCH`/`MISMATCH`、合法删除、零 missing assessment 和历史 source page。

**公共 wire authority 与 Test Kit**

`docs/schemas/resource-gateway-mirror/` 新增 12 份 Draft 2020-12 strict Schema，覆盖 manifest、
chunk、bundle、population command/receipt、disposition fact/command/receipt、assessment
command/fact/receipt 和 source page。它们进入 `CapabilityMirrorProtocol` 公共版本/资源常量、
`CapabilityMirrorSchemaValidator` 的 transitive `$ref` allowlist、普通 JAR 和 shaded JAR。

`ResourceGatewayTestClient` 在网络前验证三种 command，在 envelope 后验证所有 fact；每条写链使用
自己的 purpose，读链使用治理证据 purpose。`findCompleteAuthoritativeOutcomeSelectedPopulationAssessmentSources`
带调用方 page budget，拒绝 cursor stall、空的未完成页和服务端游标漂移。

独立 `AuthoritativeOutcomeSelectedPopulationVerifier` 不链接 server/Spring model。它先做 bounded
Schema、canonical content address、chunk/member/root closure、denominator arithmetic、RG Ed25519
seal/key lifecycle 和完整 source-page closure，再调用客户提供的 selection/deletion/source authority；
本地无效输入不会放大成外部治理流量。输出只有 `VERIFIED`、`INVALID`、
`AUTHORITY_UNAVAILABLE`、`KEY_UNAVAILABLE`、`POLICY_REJECTED` 和稳定 reason code，不回显
业务 payload。

**动态 readiness**

capability probe 分开公开：
...
前三项描述已装配产品表面；最后一项还要求 selection、outcome、deletion 和 Resource Gateway signing
四个边界在本次探测中全部可用。Spring 只有在外部 selection/deletion verifier、既有 outcome
integrity、signer、数据库与 Mirror 非生产配置同时存在时才组装本产品，默认演示不会伪造客户 authority。

本纵切当时把 `Fidelity/Outcome` 从 `99.3%` 调至 `99.6%`。固定 `24%` 权重增加 `0.072`，总分从
`91.19%` 调至 `91.26%`，剩余差距 `8.74%`。当时没有继续上调的原因是四个明确未闭合点：

1. 64 MiB 完整 admission 不是百万/千万成员总体的工业摄取协议；下一步必须增加
   content-addressed staged chunk upload、幂等 finalize、过期清理、配额和断点续传；
2. assessment 目前是按请求投影，没有 durable projection job/lifecycle、增量 watermark 和 source
   change 自动重算/撤销；
3. PostgreSQL 认证是同一数据库进程上的双实例语义，不等于多进程 kill、主备切换、网络分区、备份恢复
   和跨区域传播 SLO；
4. 当时尚无 server 生成、Test Kit 消费的 public-only selected-population 固定签名 fixture，也未完成
   TypeScript/Go canonicalization 认证、客户生产 authority connector 与 fidelity-to-outcome
   correlation。

### 3.56 2026-07-27 Selected-population 固定兼容性语料

3.55 的第四项缺口中，“server 生成、Test Kit 消费的固定签名 fixture”已经闭合。新增
`authoritative-outcome-selected-population-stage1-v1.fixture.json`，冻结以下完整、payload-free
生产者输出：

- 三个 selected member 的 manifest + 两个 content-addressed chunk；
- 一条 `MATCH` 和一条 `MISMATCH` authoritative outcome observation；
- 一条由独立 deletion authority 边界签发的 legal disposition；
- `expected=3 / matched=1 / mismatched=1 / legallyDeleted=1 / missing=0` 的 signed assessment；
- 从 cursor 0 到 `complete=true` 的完整 source page、一个 RG 公钥和冻结 verification time。

fixture 不包含 private key、credential、endpoint、request、response 或业务 payload。服务端测试用真实
model 重建所有对象，重算 root/chunk/disposition/assessment/page content address，并使用 fixture 公钥
逐条验证 Ed25519 seal；Test Kit 在无 Spring/server model 依赖下完成 strict Schema、denominator、
source-set fingerprint、source-document/member coordinate 和 key lifecycle 复验。类加载阶段即执行
`fixture.verify()`，依赖升级若改变 wire 或密码学语义会直接失败关闭。

这条跨模块测试还暴露并根治了一个真实的异构消费者问题：旧 verifier 用 `JsonNode.equals()` 对比
artifact ref，Jackson 从 JSON 读取的小整数是 `IntNode`，程序构造的 revision 是 `LongNode`，两者在
JSON 语义上同为整数 `1` 却被误判。现在 artifact ref 按 kind/id/integral revision/fingerprint
字段比较，并把 chunk cursor/reference/coordinate reason code 分开。该修复不是 fixture 特判，而是
消除了所有语言和 JSON 实现都可能遇到的数值节点内部类型泄漏。

本增量把 `Fidelity/Outcome` 保守地从 `99.6%` 调至 `99.7%`，总分由 `91.26%` 调至 `91.28%`，
剩余差距 `8.72%`。它证明 Java server/Test Kit 的 producer/consumer 兼容，尚不等于
TypeScript/Go consumer certification；staged upload、持续投影、客户生产 authority connector、
真实 HA/网络分区和 fidelity-to-outcome correlation 仍是更大的剩余缺口。

### 3.57 2026-07-27 Selected-population 可恢复 staged upload

3.55 的 64 MiB 单请求入口把一个逻辑上可分块的总体强制塞进一次 HTTP 往返。根因不是 body 上限太小，
而是传输会话、不可变 population admission 和外部 selection authority 三个生命周期被压成一步：

- 任一网络中断都会丢失全部传输进度，重试成本与总体大小线性增长；
- 多副本无法判断重复请求是 exact replay、并发上传还是正文漂移；
- 没有 scope 级容量所有权时，单个 workload 可用未完成请求耗尽数据库；
- finalize 期间实例退出时，另一实例无法区分仍在执行与已经失联的 owner；
- 仅靠定时删除无法处理“读到已过期但 sweep 尚未运行”的窗口，也无法证明终态保留边界。

本轮没有扩大原 route 的 body 限制，而是把传输和业务准入拆成两个协议。staging repository 只拥有
upload intent、chunk、配额和 finalizer lease；既有
`AuthoritativeOutcomeSelectedPopulationApplicationService` 仍唯一拥有 population revision、
selection authority 校验、签名、审计和 durable admission。上传成功不能绕过业务 authority，
上传失败也不能污染 population registry。

**五步受保护协议**

| 步骤 | Route | 不变量 |
|---|---|---|
| Begin | `POST /api/mirror/outcome-selected-populations/uploads` | caller-stable `uploadId`、exact predecessor 和完整 manifest 形成 immutable request fingerprint；exact retry 返回原状态 |
| Stage | `PUT .../uploads/{uploadId}/chunks/{chunkIndex}` | 允许乱序；index、scope、manifest descriptor 和 chunk fingerprint 必须一致；相同正文 replay，变化正文冲突 |
| Inspect | `GET .../uploads/{uploadId}` | 只返回状态、计数、字节、`nextMissingChunkIndex`、epoch 和时间，不返回 member/chunk payload |
| Finalize | `POST .../uploads/{uploadId}/finalize` | 只有全部 chunk durable 后才能领取短租约；经既有 population admission 成功后才能进入 `FINALIZED` |
| Abort | `DELETE .../uploads/{uploadId}` | 只允许放弃可变会话；原子删除 staged chunks 并保留 payload-free 终态元数据 |

所有操作沿用 selection authority 的 `MIRROR_OUTCOME_SELECTION` purpose 和
`RESOURCE_GATEWAY_OUTCOME_SELECTION_AUTHORITY` workload group，完整 scope 只来自认证身份。
controller 先认证再解码，且只在启用 Mirror 的 `test`/`staging` profile 装配；`prod`、
`production`、`live` scope 在应用层再次失败关闭。begin、chunk、read、finalize 和 abort 各有独立
payload-free operation audit，日志和错误不携带 member、chunk 或客户 ledger 内容。

**状态、恢复与 fencing**

`OPEN -> FINALIZING -> FINALIZED` 是成功主线；`OPEN` 可进入 `ABORTED` 或在 TTL 后进入
`EXPIRED`。finalize lease 使用半开区间 `[claimedAt, leaseUntil)`，在恰好等于
`leaseUntil` 时旧 owner 已被 fence。每次 takeover 增加 `finalizeEpoch`，旧 epoch 即使稍后返回也不能
terminalize。

population admission 与 staging terminalization 不能放进一个跨外部 authority 的长事务。系统先用短事务
领取 lease，再在事务外调用已有幂等 population admission，最后在新短事务内按 owner、epoch、lease 和
exact admission closure 提交终态。因此：

| 故障窗口 | 恢复语义 |
|---|---|
| chunk 响应丢失 | 相同 index 和 fingerprint 再 PUT，返回 `idempotentReplay=true` |
| begin 响应丢失 | 相同 upload request 再 POST；相同 target 的不同 request 被拒绝 |
| finalizer 在 population commit 前退出 | lease 到期后新 owner 重新执行，未提交的 admission 不可见 |
| population 已提交但 finalizer 未 terminalize | 新 owner 调用既有 exact-retry admission，取得原 admission 后完成 upload |
| 旧 finalizer 在 takeover 后返回 | epoch/owner/lease 任一不符即 `FINALIZATION_FENCED` |
| cleanup 与 stage/finalize 并发 | 先锁 parent upload，再删 chunk 或转状态，不能出现计数存在而 chunk 已消失的中间态 |

`GET` 会惰性推进已过期 `OPEN`/失联 `FINALIZING` 到 `EXPIRED`，不依赖 scheduler 及时运行。
bounded scheduler 默认每 60 秒处理至多 100 个候选；终态到达 retention floor 后先锁 parent，再删除
chunks 和元数据。清理失败只留下可重试 durable state，下一轮恢复，日志不输出 scope 或 payload。

**容量所有权**

默认 server-owned policy 是每个 exact scope 最多 16 个 active upload、单 upload 最多 256 MiB
encoded chunk bytes、scope 最多 1 GiB staged bytes、未完成 TTL 24 小时、finalize lease 2 分钟、
终态保留 7 天。宿主可通过提供自己的
`AuthoritativeOutcomeSelectedPopulationUploadPolicy` bean 在受支持边界内覆盖，不能由请求覆盖。
cleanup batch 可用
`gateway.testing.mirror.selected-population.upload-cleanup-batch-size` 调整，周期可用
`gateway.testing.mirror.selected-population.upload-cleanup-interval-ms` 调整。

数据库为每个完整 scope 建立一行 capacity lock。begin 和首次 chunk byte admission 都先锁该行，再计算
active count 或 staged bytes；初始化 lock row 使用 nested savepoint 消化多实例首次插入竞态。这样配额是
数据库共享事实，不是每 JVM 各自的一份近似计数。容量耗尽返回稳定 429 和 payload-free retry hint，
不会泄漏其他租户是否正在上传。

**协议、SDK 与 readiness**

新增 upload request、status、begin admission 和 chunk admission 四份 Draft 2020-12 strict Schema，
selected-population 协议族共 16 份。Test Kit 在发请求前验证 intent/chunk，在接收后验证四种 response，
并提供 begin、stage、find、finalize、abort 五个 typed client 方法。capability probe 只有在 staging
repository/service/controller 真正装配时才公开 upload object versions、五个 endpoints 和
`mirrorAuthoritativeOutcomeSelectedPopulationStagedUpload=true`；continuous readiness 也把 staged
upload 纳入必要条件。

H2 测试覆盖乱序、exact replay、chunk drift、active/byte quota、懒过期、bounded cleanup、终态 purge、
finalizer busy/takeover/fencing、租约精确边界和同 scope 并发 lock-row 初始化。PostgreSQL 14.22 认证用
两个独立 datasource/repository 实例证明跨连接 quota、chunk、finalize 和 terminal replay；HTTP 测试证明
未授权请求在 JSON decode 前被拒绝；Test Kit 使用真实 JDK HTTP server 跑通五步协议并检查 purpose、
URL encoding、strict request/response Schema。

**仍然存在的诚实边界**

staged upload 解决的是大请求的断点续传、重试放大、共享容量与 finalizer crash ambiguity，不是无限总体
的数据面。manifest 最多声明 4096 个 chunk，每块最多 4096 个 member；默认 256 MiB upload policy 通常
会先成为实际上限。finalize 当前仍把 policy-bounded 完整 chunk list 装入一个 JVM，再调用现有
population admission；它没有 object-store multipart、streaming selection verification、增量 member
index commit、持续 assessment watermark 或 backpressure-aware projection worker。千万级总体应继续
停在治理门外，直到下一纵切把 chunk 数据面、增量 authority verification 和 durable projection lifecycle
闭合，不能通过放大 policy 假装完成。

本增量把 `Fidelity/Outcome` 保守地从 `99.7%` 调至 `99.8%`。固定 `24%` 权重增加
`0.024`，总分由 `91.28%` 调至 `91.30%`，剩余差距 `8.70%`。下一条最有效路径是
selected-population 大总体持续 assessment/projection，其次是客户生产 authority connector 和
fidelity-to-outcome correlation；仅继续增加 transport 上限不会提高成熟度。

### 3.58 2026-07-27 Selected-population 持续 assessment projection

3.57 解决“总体传不完、重试从头来”，但已经签名的一次性 assessment 仍会随着迟到 outcome、冲突
裁决或合法删除状态变化而失真。这里的病根不是缺一个 cron：**immutable evidence 和 current
governance claim 是两个生命周期**。前者必须永不改写，后者必须有受控时效、自动重验和失败关闭。
如果把 `fresh=true` 放进原 assessment，系统只能篡改历史；如果由调用方每次指定 revision 和 freshness，
调用方又能通过停更制造永久绿色结论。

本轮因此引入两层模型：

1. `AuthoritativeOutcomeSelectedPopulationCompletenessAssessment` 继续是签名、不可变、可独立复验的
   业务证据；
2. `AuthoritativeOutcomeContinuousAssessmentProjection` 是 payload-free、可重建、数据库权威的工作
   与 freshness head，只引用最近一份 immutable assessment，不冒充业务证据。

**注册与所有权**

调用方只提交 `projectionId` 和 exact `populationRef`。服务端固定派生
`assessmentId=continuous-assessment:{projectionId}`，并独占 revision allocation、polling interval、
freshness window、lease、retry 和 quarantine。注册先按认证身份恢复完整 scope，再通过外部 selection
authority 重验 population；同 projection/same population 是 exact replay，换 population 或跨 scope
复用 projection id 均冲突。普通手工 assessment API 明确拒绝 `continuous-assessment:` 前缀，只有同包
worker 能调用内部入口，消除了手工和自动 writer 争用同一证据流。

**数据库状态机与 freshness**

| 工作状态 | 数据库不变量 | 消费语义 |
|---|---|---|
| `QUEUED` | 无 lease/failure；`nextEligibleAt >= updatedAt` | 等待第一次或下一次正常重验 |
| `RUNNING` | 非空 opaque owner fingerprint、单调 `leaseEpoch`、exclusive `leaseExpiresAt` | 只能称 `REFRESHING`，不能沿用旧 current |
| `RETRY_WAIT` | 无 lease；稳定 failure code；bounded backoff cursor | 历史 assessment 可读，但 current claim 不续期 |
| `QUARANTINED` | 无 lease；failure code；`terminalAt=updatedAt` | 自治生产终止，必须人工处置 |

`UNINITIALIZED/CURRENT/REFRESHING/STALE/QUARANTINED` 是独立 freshness 轴。`freshUntil` 使用半开区间：
在恰好等于 deadline 的数据库时间，旧结论已经 stale。对外 `ready` 不是存储字段，而是在一次
database-observed read 中由 `sourceFreshness=CURRENT && authoritiesReady` 推导；任何 authority
异常、过期、worker/scheduler 未装配或 quarantine 都不会误绿。

projection 行、scope/index identity 和 `recordFingerprint` 每次读写都复验。数据库查询还交叉检查
population root、assessment head、source closure fingerprint 与 scope；篡改 JSON、索引列、重复行或
把别的 assessment 接到 projection 都失败关闭。lease owner 只保存 domain-separated SHA-256
fingerprint，不把 instance id、scope 或客户标识写入日志和协议。

**worker 一步语义**

每个 turn 最多领取一个到期 projection，使用数据库时间和 owner/epoch fence：

1. 先尝试采用已经提交但 projection 尚未记录的 immutable assessment head，覆盖“业务证据 commit 后
   响应或 projection commit 丢失”窗口；
2. 在数据库一致读中取得 population、observation heads 和 disposition heads，计算 exact source closure；
3. source closure 未变化时只更新 `currentThrough/freshUntil`，不制造一份内容相同的新 evidence；
4. closure 变化时复用既有三 authority 两阶段 assessment 服务，自动分配下一 revision 并发布 immutable
   assessment；
5. 最后按 owner、epoch 和未过期 lease 提交 adoption/freshness；旧 worker 在 takeover 后返回也只能
   得到 fenced outcome。

未知基础设施故障按 bounded retry 处理，而不是立即 quarantine，因为它也可能表示 commit 成功但响应
丢失；下一 turn 必须先做 immutable-head adoption。结构损坏、身份漂移或达到八次连续失败进入
`QUARANTINED`。retry 从 5 秒指数退避到 5 分钟，且从不延长旧 freshness。默认正常 freshness 为
1 分钟、lease 为 5 分钟；这些值由宿主 policy bean 拥有，不能出现在请求中。

**受保护协议与独立消费**

| Route | Purpose | 结果 |
|---|---|---|
| `POST /api/mirror/outcome-continuous-assessments` | `MIRROR_FIDELITY_GOVERNANCE` | `201` + effective status；exact replay 标记 |
| `GET /api/mirror/outcome-continuous-assessments/{projectionId}` | `MIRROR_FIDELITY_GOVERNANCE` / `GOVERNANCE_EVIDENCE_INGESTION` | 数据库 observation time 下的 freshness、authority readiness、work state 和 exact assessment ref |

controller 只存在于启用 Mirror 的 `test`/`staging`，先认证再做三字段 strict decode；service 再拒绝
`prod/production/live` scope。新增 request/projection/status/admission 四份 Draft 2020-12 Schema，
使 selected-population 协议族达到 20 份。Test Kit 在传输前验 request、传输后验 envelope 和完整
response `$ref` 闭包，并按 kind/id/integral revision/fingerprint 比较 population ref，避免
`IntNode/LongNode` 这类 JSON 实现细节造成误判，同时拒绝真正的跨总体替换。

Capability probe 分开公开 API、durable projection、worker readiness、scheduler readiness 和复合
selected-population readiness。scheduler 默认关闭；显式启用时必须给出 exact non-production
environment、region、stable instance id、1..64 个本地 poller 和 bounded drain。启动脚本只透传配置，
绝不安装 permissive customer authority，因此 stock demo 不会为了“看起来绿色”伪造业务信任。

**验证证据**

- H2 覆盖注册 replay/conflict、数据库时间边界、projection/索引篡改、source closure 去重、changed
  source 发布、retry/quarantine、lease takeover/fencing、丢响应 adoption 和 scheduler drain；
- PostgreSQL 14.22 使用两个独立 datasource/repository 证明跨连接 claim、epoch takeover、单 revision
  publication 和旧 owner 拒绝；
- Spring 组合测试证明三 authority 缺任意一个时产品 route 不存在，scheduler 未显式启用时复合
  readiness 为 false；
- 独立 Test Kit 通过真实 JDK HTTP server 验证 POST/GET purpose、URL encoding、四层 Schema、坐标
  防替换和矛盾 `ready` 拒绝。

**仍然存在的诚实边界**

这条纵切对“已经被当前 staged/direct policy 接纳的 bounded population”建立了持续 current claim，
没有把 4096 chunk、256 MiB 默认传输或 finalize 内存边界变成无限规模能力。worker 当前以轮询发现
source change，不是 event-driven wakeup；projection transition 只有当前完整性 head 和 operation audit，
尚无独立 append-only lifecycle page；`QUARANTINED` 也没有 owner-approved pause/resume/rebind
remediation 协议。PostgreSQL 认证仍是单 server 双连接，不证明主备切换、网络分区、备份恢复和跨区域
freshness SLO。

所以“持续投影缺失”已经关闭，“生产规模持续校准”没有关闭。下一根病根不是再调小 poll interval，
而是依次完成：

1. 客户生产 outcome/selection/deletion connector 与企业 root-policy/control-plane；
2. object-store/streaming population admission、增量 authority verification 和容量/背压认证；
3. projection append-only lifecycle、quarantine remediation、event-driven wakeup 与多区域 HA/DR 认证；
4. fidelity-to-outcome correlation、drift 自动降级和 owner calibration workbench。

本增量把 `Fidelity/Outcome` 从 `99.8%` 保守调至 `99.9%`。固定 `24%` 权重增加 `0.024`，
总分由 `91.30%` 调至 `91.32%`，剩余差距 `8.68%`。提升幅度小不是能力价值小，而是理想态评分同时
要求真实企业数据面、跨组织 authority 和统计校准闭环；bounded Java runtime 的完成不能替代这些外部
事实。

### 3.59 2026-07-27 Authoritative outcome TypeScript/Go 消费者认证

Java server 和 Java Test Kit 对同一协议都通过，并不能证明异构消费者真的能实现它。这里的病根是：
如果 canonical JSON、数字表示、时间格式、签名材料或 unknown-field 策略只存在于 Java 实现细节中，
协议升级会在发布后才暴露跨语言漂移。再增加一份 Java verifier 不能根治这个问题，必须让没有任何
server/Test Kit 依赖的消费者独立重建同一结论。

本轮在 `resource-gateway-test-kit/polyglot` 增加 TypeScript 与 Go 两份零第三方依赖 verifier。二者读取
服务端已经发布的 public-only authoritative outcome fixture，各自完成 exact field/version admission、
递归 canonical JSON、SHA-256 内容寻址、pre-treatment selection、attribution time、authority watermark、
source fact identity/order、reconciliation、evidence completeness、domain-separated attestation、X.509
SPKI Ed25519 验签和 key/time policy。测试还分别篡改 producer reconciliation、content address、
authority identity、signature 和 unknown field，拒绝 Java 非规范时间拼写，并证明 `RETIRED` key
仍能复验历史证据；两种语言必须返回与 Java 语义一致的稳定失败码。
...
脚本要求 Node.js 22.18+ 与 Go 1.24+，不执行 `npm install`、`go get` 或任何网络下载，适合进入 CI、
release upgrade gate 和客户隔离环境认证。它证明的是“固定 producer/consumer 向量可互操作”，不是
“客户生产 outcome 权威性已建立”：fixture 中的公开 key、authority facts 和 verification time 都是
冻结测试材料，生产接入仍必须通过客户独立治理的 authority callback、root policy 和实时 source
resolution。

**仍然存在的诚实边界**

- 当前跨语言范围只覆盖 `authoritativeOutcomeObservation.v1`，尚未覆盖 selected-population
  manifest/assessment/continuous projection 的完整闭包；
- Mirror Evidence v3/v4/v5、State Evidence 与 workbook seed 仍缺同等级 TypeScript/Go 固定向量；
- 当前 fixture 使用 JSON 安全整数和 canonical UTC 字符串，尚未冻结语言中立的大整数/小数 profile；
- 固定向量不能替代 Node/Go 版本矩阵、不同 crypto provider、架构和 operating-system 组合认证。

因此 RG-MIR-COMPAT-001 从“未开始”进入“第一条 outcome 纵切完成”，没有宣称整个异构 SDK 矩阵完成。
固定权重总分只从 `91.32%` 保守调至 `91.33%`，剩余差距 `8.67%`。下一条协议兼容工作应覆盖
selected-population 完整闭包；业务价值主链仍优先解决客户生产 connector、生产规模总体摄取和
fidelity-to-outcome correlation。

### 3.60 2026-07-27 Continuous assessment append-only lifecycle

3.58 已让数据库投影具备 freshness、lease、retry 和 quarantine，但只保存当前 head 会留下一个治理
盲区：消费者能知道“现在是什么”，却不能独立回答“为何变成这样、是否漏过一次状态迁移、head 是否被
截断”。operation audit 记录谁调用了接口，不等价于领域状态历史；从当前行反推历史又会把猜测冒充
证据。病根是把**协调投影**和**协调事实序列**混成一个可变对象。

本轮新增 `AuthoritativeOutcomeContinuousAssessmentLifecycleEvent.v1` 和
`AuthoritativeOutcomeContinuousAssessmentLifecyclePage.v1`，使 selected-population 协议族达到
22 份 strict Schema。每次成功 mutation 都追加一条 payload-free event，事件包含：

- exact `projectionId` 下单调且无空洞的 `eventOrdinal`；
- 闭集 transition：`REGISTERED`、`MIGRATED`、`CLAIMED`、`ASSESSMENT_PUBLISHED`、
  `SOURCE_UNCHANGED`、`RETRY_SCHEDULED`、`LEASE_EXPIRED`、`QUARANTINED`；
- 与 projection `updatedAt` 完全一致的 `occurredAt`；
- 不泄漏 instance id 的 domain-separated actor fingerprint；
- transition 后完整、已内容寻址的 projection，而不是可能随版本漏字段的差量；
- `previousEventFingerprint` 和当前 `eventFingerprint`。

**事务与并发不变量**

事件 append、projection 更新和 projection 行上的
`lifecycleHeadOrdinal/lifecycleHeadFingerprint` 推进位于同一数据库事务。更新同时比较旧
projection fingerprint 和旧 lifecycle head；两个 worker 即使读到同一业务 head，也只能有一个
提交。event 先写后若 projection CAS 失败，事务整体回滚，不留下孤儿事实。读取 projection 时会交叉
验证 lifecycle head 对应的真实 event；删 head、截断链、改 event JSON、改索引列或把另一 projection
的事件接入都会失败关闭。

旧数据库升级不伪造历史。已存在但没有 lifecycle head 的投影只有在第一次后续 mutation 时，先写一条
`MIGRATED` 事件，内容是当时完整旧 projection，再追加真实 transition。若 head 为零却已经存在 event，
或 head 与 event 不一致，则视为结构损坏，不走“自动修复”覆盖证据。
...
```text
GET /api/mirror/outcome-continuous-assessments/{projectionId}/lifecycle
    ?afterOrdinal={exclusive-cursor}&limit={1..1000}
```

它使用独立 operation/purpose 审计，返回调用方请求的 `afterOrdinal`、该坐标的
`predecessorFingerprint`、连续 oldest-first events、`nextOrdinal` 和 `hasMore`。调用方必须把已验真的
`nextOrdinal + eventFingerprint` 成对持久化，再作为下一页 checkpoint；不能只相信服务端给出的 next
链接。Test Kit 新增 typed client 和零 server/Spring 依赖 verifier，逐页验证 strict Schema、页面坐标、
projection 内容地址、event 内容地址、ordinal 连续性和 predecessor chain。

**验证证据**

- H2 repository 测试覆盖注册到发布/重试的链、head 删除失败关闭、pre-lifecycle `MIGRATED` 基线；
- service/controller 测试覆盖 bounded page、非法 cursor 稳定拒绝、auth-before-service 和独立 audit；
- capability 测试证明 object version、feature 和 endpoint 只随真实装配暴露；
- Test Kit 通过真实 JDK HTTP server 覆盖 URL/purpose/envelope，并对 projection、event、cursor 和
  predecessor 篡改失败关闭。

**仍然存在的诚实边界**

这条 lifecycle 证明 Resource Gateway 数据库中已提交的协调迁移，不能证明外部 outcome authority
本身没有隐瞒事件，也不替代 immutable completeness assessment 的业务签名。`QUARANTINED` 仍缺
owner-approved pause/resume/rebind remediation；source change 仍靠轮询而非 durable event wakeup；
单数据库事务也不等于 PostgreSQL 多副本、network partition、backup/restore 和跨区域 cursor
传播认证。生命周期长期保留、legal hold、外部 anchor/WORM 与物理删除证明还需纳入企业 evidence
governance policy。

本增量关闭“projection 只有当前 head、无法独立审计迁移”的缺口，不把治理历史误算成生产 connector
或 HA 完成。固定权重总分从 `91.33%` 保守调至 `91.35%`，剩余差距 `8.65%`。下一步应先做
quarantine remediation 与生产规模 connector/certification，再做 event-driven wakeup；只继续增加
transition 名称不会提高可信度。

### 3.61 2026-07-27 Continuous assessment 受控解除隔离

3.60 让 `QUARANTINED` 的形成过程可审计，但“能看到隔离”不等于“能安全解除隔离”。若只增加一个
`retry` 或 `status=QUEUED` 接口，系统会把人工修复、并发控制、权限证明和模糊响应恢复压缩成一次可变
写入。真正的病根不是缺一个按钮，而是**缺少把运维修复建模为不可变、可并发验证、可独立复验的领域
命令**。在复杂组织里，值班人员看到的 projection 可能已经过时，另一名管理员可能已处理，HTTP 响应
可能丢失，普通 projector 身份也可能越权；任何一个问题都足以让旧故障被误清除或让错误恢复被重复执行。

**失败模式与根治手段**

| 失败模式 | 表面修补 | 病根 | 本轮约束 |
|---|---|---|---|
| 对任意状态调用 retry | 判断 `status=QUARANTINED` | 判断与提交之间可漂移 | 同时比较 reviewed projection fingerprint 与 lifecycle head ordinal/fingerprint |
| 两名管理员并发恢复 | 最后写入者获胜 | 没有不可变决策坐标 | projection 行锁 + projection/head 双 CAS，只接受一个 generation |
| 响应丢失后重复提交 | 再执行一次更新 | 命令没有稳定身份 | `commandId` + exact canonical command + actor fingerprint 精确幂等 |
| 同 command id 换参数或换人 | 覆盖原命令 | 幂等键未绑定内容与权限主体 | 内容或 actor 变化稳定冲突，不返回旧 receipt |
| 恢复时顺手刷新 evidence | 设置 `freshUntil=now+TTL` | 把运维判断冒充来源重验 | 只恢复调度资格；旧 freshness、assessment/source closure 全部不变 |
| 只返回当前 projection | 依赖 operation log 解释 | 无法证明恢复前后与命令的闭包 | receipt 内嵌 command、previous projection、accepted event 与内容地址 |
| audit 失败但状态已恢复 | 异步补审计 | 领域变更和责任证据分裂 | event、projection、receipt、mandatory audit 同事务提交 |
| 恢复后 worker 已继续推进，客户端重试 | 要求当前 head 仍等于 receipt | 幂等结果错误依赖后来状态 | receipt 独立持久化，exact retry 在后续 transition 后仍返回原值 |

**协议与状态机**

新增两个 strict Draft 2020-12 对象：

- `AuthoritativeOutcomeContinuousAssessmentRemediationRequest.v1`：包含 caller-stable
  `commandId`、`expectedProjectionFingerprint`、`expectedLifecycleHeadOrdinal`、
  `expectedLifecycleHeadFingerprint` 和 bounded `reasonCode`；
- `AuthoritativeOutcomeContinuousAssessmentRemediationReceipt.v1`：绑定 exact enterprise scope、
  `projectionId`、单调 `remediationGeneration`、actor-bound `commandFingerprint`、原命令、
  reviewed quarantined projection、`REMEDIATION_ACCEPTED` lifecycle event 和
  `receiptFingerprint`。
...
X-Purpose: MIRROR_OUTCOME_CONTINUOUS_ASSESSMENT_ADMIN
```

专用 purpose 防止 `MIRROR_FIDELITY_GOVERNANCE` 读写身份或普通 worker 自行解除隔离；它复用
`RESOURCE_GATEWAY_FIDELITY_PROJECTOR` workload group，避免在既有组织部署中凭空要求第四套服务
账号，但 policy 必须显式授权 admin purpose。鉴权先于 strict JSON 解码，未知字段、超限 body 和非法
坐标不会成为未授权调用方的协议探针。稳定失败面为：
...
接受后的状态变化采用白名单，而不是复制一个“新 projection”请求。`status` 从 `QUARANTINED` 变为
`QUEUED`，`consecutiveFailures` 归零，`failureCode` 清空，`terminalAt` 移除，
`nextEligibleAt=updatedAt=database commit time`。population ref、assessment id/ref、observation/
disposition closure、`currentThrough`、旧 `freshUntil`、累计 `attemptCount`、`leaseEpoch` 和
`createdAt` 必须逐字段保持不变。恢复因此只允许 worker 重新观察，并不会让陈旧结果瞬间变成
`CURRENT`。

**事务算法与可验证不变量**

1. 按完整 scope 和 `projectionId` 锁定数据库 projection，先查 actor-bound `commandId` 的历史
   receipt；exact command 直接重放，内容或 actor 漂移冲突。
2. 验证当前状态为 `QUARANTINED`，并同时比较 reviewed projection fingerprint 与 lifecycle
   head ordinal/fingerprint；任一变化都要求调用方重新读取、复验和决策。
3. 由数据库时间构造白名单 successor，追加 ordinal `head+1` 的
   `REMEDIATION_ACCEPTED` event，并以旧 projection/head 双 CAS 推进 projection。
4. 对 scope、projection、opaque actor fingerprint 和 exact command 做 canonical binding，计算
   command fingerprint；再封装 previous projection 与 accepted event，计算 receipt fingerprint。
5. 写入独立 `mirror_outcome_continuous_remediations` 记录并提交 mandatory operation audit；任何
   一步失败都回滚 event、head、projection 和 receipt。

数据库读取 stored receipt 时不只解析 JSON，还逐项交叉验证 scope、projection id、command id、
generation、command/receipt fingerprint 与索引列，防止“JSON 正确但索引被换绑”。原始 actor 不入库，
只保存 domain-separated fingerprint。精确重放读取持久化 receipt，而不是从当前 projection 重新拼装，
因此即使 worker 已 claim 新 lease，调用方仍得到第一次提交的同一事实。

**独立消费与验证**

capability probe 新增
`mirrorAuthoritativeOutcomeContinuousAssessmentRemediation=true`、request/receipt object version
和 exact endpoint。Test Kit 打包两份新 Schema，typed client 自动使用 admin purpose，并新增零
Resource Gateway/Spring 依赖的
`AuthoritativeOutcomeContinuousAssessmentRemediationVerifier`。它独立检查：

- request/receipt strict Schema 与 projection/lifecycle 既有 Schema；
- previous/current projection 内容地址、event ordinal/predecessor/content address；
- request 中三个 reviewed fence 与 receipt 中 previous/event 的精确对应；
- actor-bound command fingerprint 和 receipt fingerprint；
- `QUARANTINED -> QUEUED` 的状态变量白名单及不可变业务坐标。

这使 ANEKE 或 CI 不必信任服务端派生的“恢复成功”布尔值。正确流程是先独立验证 status 和完整 lifecycle
checkpoint，完成人工或自动化根因修复，再提交 exact command，最后独立验证 receipt。没有
`force=true`、`runNow`、caller-owned freshness 或绕过 source re-observation 的接口。

**验证证据与诚实边界**

- repository 测试覆盖 accepted transition、worker 后续 claim 后 exact replay、非隔离状态、双 fence
  漂移、同 command 换内容/换 actor、stored receipt JSON/索引篡改；
- service/controller/decoder 测试覆盖专用 authority、拒绝矩阵、auth-before-decode、严格字段集与
  stable problem code；
- capability 测试证明 feature、两个 object version 和 endpoint 同步暴露；
- Test Kit 通过真实 JDK HTTP server 验证 route/purpose/envelope，并对 projection、actor、command、
  lifecycle 和 receipt 篡改失败关闭；
- PostgreSQL 双数据源认证覆盖一个实例隔离、另一实例解除、原实例观察新 head 并精确重放 receipt。

本轮解决的是“已审阅隔离如何安全恢复调度”，不是自动证明根因已经修复。当前 `reasonCode` 是 bounded
运营坐标，不是 owner 签名的 repair evidence，也未实现双人审批、工单/incident binding、限时授权或
break-glass 联邦审计；这些应由企业 control plane/ANEKE gate 按风险等级施加。仍缺 event-driven
wakeup、生产规模连续投影压测、客户 outcome connector、PostgreSQL 多进程/HA/网络分区与 backup/
restore 认证、跨区域 cursor/receipt 传播和 selected-population 跨语言固定向量。本增量只将固定权重
理想态完成度从 `91.35%` 保守调至 `91.37%`，剩余差距 `8.63%`。
...
| RG-MIR-009 | 增加 test-kit 协议模型与 compatibility fixtures | `resource-gateway-test-kit` | 已完成 Snapshot/Closure、MirrorEvidence v1/v2/v3/v4/v5、StateEvidence v1/v2/v3、read/transition/write-outcome WorkbookSeed v1、DeploymentIsolationAttestation/Authority、CapabilityObservation、Corpus/Trajectory/Cluster、exact Shadow detached/online source-resolution 与 selected-population 全闭包独立复验；detached/online source-resolution 和 selected-population 固定签名 fixture 已闭合 producer/consumer canonicalization、denominator/source closure、command pairing 与 key-role；selected-population staged upload 的四份 strict Schema 与 begin/stage/find/finalize/abort typed HTTP client 已闭合；continuous assessment 的八份 strict Schema、独立 projection/freshness/lifecycle/remediation verifier、register/find/lifecycle/remediate client、derived-ready 约束、调用方游标验链、双 CAS receipt 和跨总体替换拒绝已闭合；authoritative outcome v1 已有零第三方依赖 TypeScript/Go verifier 和统一认证脚本；共享 fixture 均不携带私钥或业务 payload；不依赖 server/Spring；MirrorEvidence v3/v4/v5 与 selected-population 完整闭包的跨语言固定签名认证仍待补 |
...
| 3 | RG-MIR-FID-001 Fidelity protocol/kernel | P0 / 进行中（Scenario + Shadow + authoritative outcome/selected-population 产品纵切完成） | RG Domain + ANEKE Governance + 数据科学 | Work package 1；owner 冻结 coverage taxonomy | 已完成 `DomainFidelityInventoryRegistrationRequest/Inventory/Profile.v1`、strict Schema、完整 denominator、七维 metric、Wilson 95%、freshness、abstention/source debt、exact lineage、fail-closed projector、独立 Test Kit verifier、full-scope append-only repository、managed signer、受保护 register/read API、同事务 audit、typed capability、Scenario source、signed Shadow v1/v2/v3 comparison/source adapter、v3 AuthorityProof、durable request/job/lifecycle verifier，以及 authoritative outcome observation/cohort/reconciliation、双重 authority、受保护 inbox API、append-only lifecycle、条件 worker/scheduler、PostgreSQL 双连接接管认证、outcome source adapter、selected-population durable registry/API、三角色隔离、coherent assessment、历史 source closure、server/Test Kit 固定全闭包 fixture、配额隔离且可恢复的 staged upload/finalize，以及 database-authoritative continuous assessment projection、fencing、freshness、retry/quarantine、append-only lifecycle、projection/lifecycle 双 CAS quarantine remediation、独立 Test Kit client/verifier 和 authoritative outcome TypeScript/Go 固定向量认证；待 request-space/error-distribution cohort source、object-store/streaming 总体摄取、客户生产 connector、event-driven wakeup、跨区域/HA 认证、selected-population/Mirror Evidence 异构 SDK 认证和 calibration correlation | 空分母/低样本/缺失/陈旧不能高分；合法重签的伪造 metric/expiry/denominator/shadow MATCH/job/lifecycle/authority/outcome/selected-population closure 仍被独立拒绝；审计失败回滚资产；每个 profile 分量可回到 exact inventory/source generation；单次 Scenario/Shadow 不得冒充 outcome/request-space/error-distribution |
...
| 5 | RG-MIR-OUT-001 Outcome calibration | P1 / 进行中（outcome 产品、selected-population completeness/continuous projection 与 outcome v1 异构认证纵切完成） | ANEKE + 业务分析 + RG Domain | Work package 3；权威 outcome owner | 已完成 versioned exact refs、pre-treatment cohort closure、delayed/censored/conflicting reconciliation、双重 authority、source adapter、durable inbox/worker/scheduler、selected denominator/disposition/assessment registry、受保护 API、历史 source closure、独立 verifier、server/Test Kit 固定全闭包 fixture、可恢复 staged population upload/finalize、server-owned continuous assessment stream、数据库 freshness、fencing、dedup/adoption、retry/quarantine、append-only lifecycle、受控 quarantine remediation、调用方游标验链、opt-in scheduler，以及 outcome observation 的 TypeScript/Go 零第三方依赖认证；待生产 connector、streaming population admission、event-driven wakeup、successor stale/revoke、selected-population 跨语言认证、calibration correlation 和 candidate assertion | 多源冲突和迟到结果不覆盖历史；选择性不提交不能伪装为 completeness；候选不经 owner 不 serving；能证明或否证 fidelity 与业务结果关系 |
...
| 8 | RG-MIR-COMPAT-001 Heterogeneous consumer certification | P1 / 进行中（authoritative outcome v1 纵切完成） | RG Protocol + ANEKE SDK | Work package 1/3 协议冻结 | 已交付 outcome v1 冻结 profile、Java/TypeScript/Go 固定向量与无下载统一脚本；待 selected-population、Mirror Evidence/State Evidence/workbook seed 和运行环境兼容矩阵 | 已闭合三语言对 outcome fixture 的相同 fingerprint/signature verdict 与 unknown field fail closed；退出仍要求主协议族和版本/OS/架构矩阵 |
