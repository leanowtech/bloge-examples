...
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-signature-threshold` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_THRESHOLD` | legacy `test` compatibility only; must be `0` with a managed root chain and in `staging` |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-authority-keys-json` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_KEYS_JSON` | legacy `test` compatibility only; must be `[]` with a managed root chain and in `staging` |
...
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.enabled` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOTS_ENABLED` | `false` in `test`, `true` in `staging`; selects complete genesis-to-head root-chain replay |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.required` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOTS_REQUIRED` | `false` in `test`, `true` in `staging`; Java composition also enforces staging regardless of override |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.genesis-json` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_GENESIS_JSON` | deployment-pinned public-only `bloge.externalSequenceAnchorBootstrapRootGenesis.v1`; strict standalone Schema entry point applies |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.accepted-policy-fingerprints` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_POLICY_FINGERPRINTS` | 1..32 accepted transition-ceremony policy fingerprints |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.bundle-uri` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_BUNDLE_URI` | strict complete-chain bundle v1 endpoint; HTTPS mandatory outside explicit local tests |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.refresh-interval-seconds` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_REFRESH_SECONDS` | `30`; jittered single-lane refresh, 1..3600 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.request-timeout-ms` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_TIMEOUT_MS` | `3000`; 100..30000 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.unknown-key-refresh-interval-seconds` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_UNKNOWN_KEY_REFRESH_SECONDS` | `5`; global anti-stampede cooldown, 1..600 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.maximum-snapshot-age-seconds` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_MAXIMUM_AGE_SECONDS` | `60`; hard source-age fence, at least refresh and at most 86400 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.maximum-root-lifetime-seconds` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_MAXIMUM_LIFETIME_SECONDS` | `2592000`; accepted generation lifetime, 3600..31622400 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.clock-skew-seconds` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_CLOCK_SKEW_SECONDS` | `5`; 0..30 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.minimum-remaining-validity-seconds` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_MINIMUM_REMAINING_VALIDITY_SECONDS` | `30`; 0..604800 and below maximum root lifetime |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.maximum-transitions` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_MAXIMUM_TRANSITIONS` | `128`; local replay bound, 1..128 |
| `gateway.testing.test-secrets.authority.http.jwks.cohort.signed-inventory.remote.external-anchor.managed-trust.bootstrap-roots.allow-insecure-loopback` | `RG_TEST_SECRET_AUTHORITY_COHORT_INVENTORY_EXTERNAL_ANCHOR_BOOTSTRAP_ROOT_ALLOW_INSECURE_LOOPBACK` | `false`; local tests only and rejected by staging shell plus Java gates |
...
`testSecretExternalNotaryTrustReady` for the test-secret notary chain. Its bootstrap-root chain adds
`managedTestSecretExternalNotaryBootstrapRoots`,
`restartFreeTestSecretExternalNotaryBootstrapRootRotation`,
`completeTestSecretExternalNotaryBootstrapRootReplay`,
`durableTestSecretExternalNotaryBootstrapRootFloor` and
`testSecretExternalNotaryBootstrapRootsReady`; suite stability uses the corresponding
`SuiteStability` names. The additive `testSecretExternalNotaryTrustChainReady` and
`suiteStabilityExternalNotaryTrustChainReady` facts are false unless both layers are currently
ready; the pre-existing notary-ready facts retain their original compatibility semantics. Every
probe reads immutable local snapshots and performs no remote I/O.
