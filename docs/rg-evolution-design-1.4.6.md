...
认证 fixture 按可观察边界分三次写入，但保持三个 Codex 运行实例：同义召回完成后只加入近义干扰项；近义召回验收后加入多个 EXACT、旧 Feature 和语义漂移资产；依赖不可用、动作桩化和禁止调用等受控案例完成后，才在同一第三运行实例中加入两个同名动作，并立即执行歧义预检和最后一个 Codex 会话。后置歧义资产不得提前污染依赖假设的唯一能力绑定。最终私有 manifest 必须按 `near-meaning → remaining → ambiguity` 顺序关联全部资产、关系和预检，缺少或调换任一阶段均拒绝认证。
