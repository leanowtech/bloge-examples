...
| `online-read-only-shadow-baseline-capability-v1.schema.json` | `OnlineReadOnlyShadowBaselineProtocol.Capability` | Short-lived live sidecar projection for payload isolation, read-only identity, idempotency, vault receipt, write-credential prohibition, and exact artifact reads |
| `online-read-only-shadow-candidate-capability-v1.schema.json` | `OnlineReadOnlyShadowCandidateProtocol.Capability` | Short-lived live sidecar projection for payload isolation, sealed-plan execution, idempotency, signed evidence, production-credential prohibition, and exact artifact reads |
