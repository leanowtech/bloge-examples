...
operator-library import path. The browser Operator Libraries panel exposes this
as a `From AsyncAPI` preview action: authors paste AsyncAPI into the same source
editor, preview the generated library, then reuse the existing Validate/Import
...
| `POST` | `/admin/visual-operator-libraries/from-asyncapi` | Preview-project parsed `asyncApi` or raw JSON/YAML `asyncApiText` into a `bloge.visualOperatorLibrary.v1` draft using runtime-blocked `event-source`, `message-handler`, or `webhook` operators, then return `bloge.asyncApiOperatorLibraryImportResult.v1` with the generated library plus the same validation/profile/impact evidence used by normal imports; this endpoint does not store the generated library | 200 |
