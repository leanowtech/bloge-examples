...
| `GET` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/preview?operatorRef=...` | Preview the candidate executable `OperatorDefinition` surface for a `readiness-recompute-required` operator without mutating the trusted catalog or operator-library registry; currently auto-recomputable only for `loweringMode=native` executor integrations |
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/apply?operatorRef=...&ackWarnings=true&actor=...&reason=...` | Recompute the executable-readiness preview server-side and, when governance evidence is present, apply its candidate operator as a new owning operator-library revision; the request never accepts caller-supplied operator surface, returns `bloge.visualExecutableReadinessRecomputeResult.v1`, and currently mutates only native lowering candidates |
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/evidence-refresh?operatorRef=...&ackWarnings=true&actor=...&reason=...` | Refresh post-apply runtime-plane evidence by creating a new bound implementation binding, adapter activation, and executable lowering integration against the current executable operator fingerprint; returns `bloge.visualExecutableReadinessEvidenceRefreshResult.v1`, supersedes the old binding, retains old evidence for audit, and currently supports native runtime-binding/metadata-only change surfaces |
...
| `VisualExecutableReadinessRecomputeResult` | Governed `bloge.visualExecutableReadinessRecomputeResult.v1` apply result that recomputes the preview server-side, requires warning acknowledgement plus actor/reason evidence, and writes the native candidate surface as a new owning operator-library revision when fingerprints still match |
| `VisualExecutableReadinessEvidenceRefreshResult` | Governed `bloge.visualExecutableReadinessEvidenceRefreshResult.v1` post-apply refresh result that rebinds implementation, activation, and executable lowering evidence to the current executable operator fingerprint while preserving old evidence lineage |
