...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/bind` | Transition a ready-to-bind or review-acknowledged proposal into the active `bound` lifecycle state with actor/reason audit evidence; an exact replay of the same persisted lifecycle event returns idempotent `200 OK`, while different already-bound requests still conflict; rejects missing governance evidence, review-required proposals without `ackReview`, already bound/superseded records, and operators with an existing active binding; repository update failures return `409`, `state=failed`, and `visual.runtimeBindingImplementation.bindPersistenceFailed` with exception metadata instead of leaking generic 500s |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/unbind` | Transition an active bound implementation into `unbound` with actor/reason audit evidence, cascading the active executable lowering integration and adapter activation to `inactive`; an exact replay of the same unbind lifecycle event returns idempotent `200 OK` with the stored inactive evidence, while different already-unbound requests still conflict; if a later cascade step fails after runtime evidence was already deactivated, the controller restores that evidence to `active`, realigns the executable integration with the restored activation revision, and returns `state=failed` with compensation diagnostics instead of silently breaking the active evidence chain; returns `bloge.visualRuntimeBindingDeactivationResult.v1` and removes the evidence chain from active catalog/promotion projections only on successful unbind |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/supersede` | Replace one active bound implementation with another accepted proposal for the same operator, linking `supersedesBindingId` / `supersededByBindingId` and lifecycle events; an exact replay of the same supersede lifecycle pair returns idempotent `200 OK`, while different supersede attempts against an already superseded binding still conflict; if the replacement is bound but marking the current binding as superseded fails, the controller restores the replacement to its prior proposal state and returns a failed lifecycle result instead of leaving two active bound records; the active binding is projected into the operator catalog response as implementation evidence, while executable readiness still waits for real runtime adapter integration |
...
audit events, inactive runtime evidence, and replacement lineage. These records are now control-plane
