...
design graphs. The browser Drafts panel exposes this flow through Export/Import
Bundle controls and a JSON bundle editor, feeds the returned readiness back into
Server Check, and consumes the returned dependency report so imported
...
| `GraphDraftDependencyReport` | Machine-readable stored draft dependency report with distinct operatorRef usage, per-node upstream/downstream lineage, source/lowering/readiness counts, saved-vs-current/scope-mismatch fingerprint state, and scope policy diagnostics |
