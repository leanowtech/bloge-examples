...
scope/filter, stable requirement keys, operator/library/routing counts, and
requirement rows. It can also review the latest exported handoff bundle against
...
| `VisualRuntimeBindingHandoffReview` | Read-only `bloge.visualRuntimeBindingHandoffReview.v1` reconciliation report that compares a handoff bundle with the current runtime-binding requirement read model by stable key, field changes, and exported/current/new-window routing distributions |
