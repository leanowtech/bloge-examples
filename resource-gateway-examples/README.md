...
expected output; running it reuses the publication's frozen DSL, writes a visual
run-history record, and returns pass/fail diagnostics without changing the draft
currently on the canvas. The Publications panel can save a golden case from the
latest publication run output, run the selected case in place, or run the full
golden suite for the selected publication. A successful suite can also be saved
as the publication's latest golden certification without mutating the immutable
...
| `POST` | `/api/visual/golden-cases` | Save a golden regression case for an existing publication |
| `POST` | `/api/visual/golden-cases/{caseId}/run` | Execute a golden case with the publication's frozen DSL and return pass/fail diagnostics |
...
The test suite is organised into four layers (61 test report classes, 650 executed
