...
tag, source, capability, or lowering mode for larger imported catalogs, and see the server-provided catalog mix counts for runtime-executable, design-only, streaming/durable, secret-bound, and external-effect operators, inspect each palette card's input/output port and
...
field counts plus operator runtime readiness across runtime-executable, design-only,
runtime-blocked, and governance-review states, inspect the selected node's output connectability across data,
...
mode to `DESIGN`, and API callers may pass `artifactKind: "DESIGN"` directly, to
freeze a schema-valid but non-executable design artifact. This is useful when
...
for design artifacts. Publish requests reject warning-level validation diagnostics
...
The test suite is organised into four layers (67 test report classes, 903 test
