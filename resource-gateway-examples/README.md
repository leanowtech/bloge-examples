...
When the correctness runtime is enabled, a bounded platform sweep starts after 60 seconds and repeats every six
...
The same scoped projection is available through authenticated
`GET /api/agent-tdd/library-overview` and the READ-level `rg.library.overview.get` MCP tool. It places business-readable platform
...
payload-free and returned with `no-store` rather than sampling provider responses. MCP callers opt into
payload-free sample descriptors with `includeSamples=true`; the default is false. Every response includes a
stable `snapshotFingerprint` bound to tenant, project, environment, building blocks, and world model. Its second-act
