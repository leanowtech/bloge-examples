...
expected output. When `assertions` is empty the case uses exact-output equality;
...
without changing the draft currently on the canvas. The Publications panel can
...
| `POST` | `/api/visual/golden-cases` | Save a golden regression case for an existing publication, with exact-output fallback or explicit output assertions |
...
The test suite is organised into four layers (61 test report classes, 694 executed
