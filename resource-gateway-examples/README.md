...
record and shape-only run trace into the output inspector. The run-history API
...
success rate, blocked/error counts, and p50/p95 latency; `/api/visual/runs/{runId}/trace`
returns node status and result-shape summaries without storing raw runtime
payloads.
...
output with JSON Pointer paths. `OUTPUT_MATCHES_SCHEMA` assertions validate the
selected output against a visual JSON schema or SchemaEnvelope, so golden suites
...
exact-output cases or one browser-authored assertion from the latest publication
run output by choosing assertion mode, optional JSON Pointer path, and expected
JSON value/schema; blank schema assertion values are inferred from the latest
output. It can also run the selected case in place, or run the
...
| `POST` | `/api/visual/golden-cases` | Save a golden regression case for an existing publication, with save-time schema-version/context/output-node/assertion validation, exact-output fallback, explicit output assertions, or output-schema assertions |
...
| `POST` | `/api/visual/golden-cases/{caseId}/run` | Execute a golden case with the publication's frozen DSL and return exact-output, value assertion, or schema assertion diagnostics |
...
| `GET` | `/api/visual/runs/{runId}/trace` | Load one shape-only replay trace with node statuses, selected output marker, result summaries, diagnostics, errors, and generated/frozen DSL |
