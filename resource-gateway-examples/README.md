...
snapshot in the JSON editor, and restore it as a governed latest revision. A
deleted library keeps its history target in the panel, so accidental deletion
can be recovered without relying on browser-local state; SemVer rollback still
requires the explicit `Rollback` toggle and warning acknowledgement.
...
| `GET` | `/admin/visual-operator-libraries/{libraryId}/revisions` | List immutable create/replace/delete/restore registry snapshots for an imported library, newest first; delete snapshots remain queryable after the current library is removed | 200 / 404 |
...
The test suite is organised into four layers (67 test report classes, 934 test
