...
diagnostics back to the affected canvas node, inspect a selected node's upstream
context/data/config/order inputs, downstream consumers, branch routes, and
delete impact directly in the operator inspector, clear incoming input/config,
dependency, or route relations from that impact view, search the operator palette with multi-term
...
status before dragging an edge, create ready connections from that panel through
...
field counts, dynamic schema surfaces, DSL-unsafe fields and output port names,
external effects, and secret-bound operators before importing it into the catalog, validate
user-provided operator library JSON before importing it, opt
...
project one OpenAPI operation from JSON or YAML text into a visual resource contract draft, review the
...
silently overwrite existing operator fingerprints.
...
The OpenAPI projection endpoint accepts either a parsed OpenAPI 3 document in
`openApi` or raw JSON/YAML document text in `openApiText`, plus either
`operationId` or `path` and `method`. It rejects malformed document text with
structured diagnostics. It projects path/query/header/cookie
...
paste raw OpenAPI JSON or YAML, Preview generates contract and descriptor JSON
...
| `POST` | `/admin/visual-operator-libraries/validate` | Validate an operator library without storing it and return `valid`, `diagnostics`, and `bloge.visualOperatorLibraryImpact.v1` with affected draft targets; use `force=true` to suppress stored-draft removal/disablement impact diagnostics; same-ref fingerprint drift and immutable-publication impact are reported as warnings | 200 |
| `POST` | `/admin/visual-operator-libraries` | Import or re-import an operator library; rejected or warning-gated responses include `bloge.visualOperatorLibraryImpact.v1`, reject removal or disablement of stored-draft operator refs unless `force=true`, and require `ackWarnings=true` before storing warning-level replacement impact | 201 / 400 / 409 |
| `GET` | `/admin/visual-operator-libraries/{libraryId}` | Get one imported library | 200 / 404 |
| `PUT` | `/admin/visual-operator-libraries/{libraryId}` | Replace an imported library; rejected or warning-gated responses include `bloge.visualOperatorLibraryImpact.v1`, reject removal or disablement of stored-draft operator refs unless `force=true`, and require `ackWarnings=true` before storing warning-level replacement impact | 200 / 400 / 409 |
...
`items` across input, output, and config schemas, plus non-default output port
names or schema path fields that cannot be rendered as BLOGE DSL path segments.
...
DSL-unsafe schema paths and non-default output port names out of source pickers,
schema-derived node handles, and path-specific graph output options, and hides
...
validation applies the same DSL-safe field-name gate to native input/config and output
...
The test suite is organised into four layers (67 test report classes, 853 executed
