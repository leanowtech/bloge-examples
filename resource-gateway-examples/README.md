...
raw runtime payload values by default. The browser Run History panel refreshes
after visual draft and publication runs, supports source/outcome/limit filters,
shows a compact SLO summary for the same filter window, and opens an individual
record into the output inspector. The run-history API also accepts `sourceKind`,
`draftId`, `publicationId`, `graphName`, `success`, and `limit` query parameters
for browser and automation use; `/api/visual/runs/stats` reuses the same filters
to aggregate total runs, success rate, blocked/error counts, and p50/p95 latency
without storing raw runtime payloads.
...
`OUTPUT_EQUALS`, `PATH_EQUALS`, `PATH_EXISTS`, and `PATH_ABSENT` over the selected
output with JSON Pointer paths. Running a case reuses the publication's frozen
DSL, writes a visual run-history record, and returns pass/fail diagnostics
without changing the draft currently on the canvas. Run-time assertion validation
remains in place for legacy or manually inserted cases that predate the save
gate. The Publications panel can
save exact-output cases or one browser-authored assertion from the latest
publication run output by choosing assertion mode, optional JSON Pointer path,
and expected JSON value. It can also run the selected case in place, or run the
...
| `POST` | `/api/visual/golden-cases` | Save a golden regression case for an existing publication, with save-time schema-version/context/output-node/assertion validation, exact-output fallback, or explicit output assertions |
...
| `POST` | `/api/visual/golden-cases/{caseId}/run` | Execute a golden case with the publication's frozen DSL and return exact-output or assertion diagnostics |
...
publications that were authored with the affected operatorRef. Existing
publications keep running from their frozen DSL, but the warning marks artifacts
...
`ackWarnings=true` before it writes the library.

| Method | Path | Description | Status |
|--------|------|-------------|--------|
| `GET` | `/admin/visual-operator-libraries` | List imported operator libraries | 200 |
| `POST` | `/admin/visual-operator-libraries/validate` | Validate an operator library without storing it; use `force=true` to suppress stored-draft removal/disablement impact diagnostics; same-ref fingerprint drift and immutable-publication impact are reported as warnings | 200 |
| `POST` | `/admin/visual-operator-libraries` | Import or re-import an operator library; rejects removal or disablement of stored-draft operator refs unless `force=true`, and requires `ackWarnings=true` before storing warning-level replacement impact | 201 / 400 / 409 |
| `GET` | `/admin/visual-operator-libraries/{libraryId}` | Get one imported library | 200 / 404 |
| `PUT` | `/admin/visual-operator-libraries/{libraryId}` | Replace an imported library; rejects removal or disablement of stored-draft operator refs unless `force=true`, and requires `ackWarnings=true` before storing warning-level replacement impact | 200 / 400 / 409 |
| `DELETE` | `/admin/visual-operator-libraries/{libraryId}` | Delete an imported library; rejects stored-draft references and published-artifact references unless `force=true` | 204 / 409 |
...
object. Because the current BLOGE object-literal/path grammar does not support quoted
...
graph output options, and schema-derived node handles before authors can wire them
into a draft. Operator library
...
edges, or graph output selections can reach DSL generation. Graph names and node ids are also guarded against DSL reserved
...
The test suite is organised into four layers (64 test report classes, 787 executed
