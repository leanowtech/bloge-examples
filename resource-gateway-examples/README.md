...
example a concrete audit anchor for collaborative authoring and rollback. The
same panel calls the draft revision diff API to show a latest-to-selected
...
| `VisualOperatorCatalogController` / `VisualOperatorUsageController` / `VisualGraphDraftController` | Visual operator discovery, usage impact indexing, draft revision diff, validation, compilation, and execution |
...
The test suite is organised into four layers (69 test report classes, 944 test
