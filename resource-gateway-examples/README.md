...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/supersede` | Replace one active bound implementation with another accepted proposal for the same operator, linking `supersedesBindingId` / `supersededByBindingId` and lifecycle events; the active binding is projected into the operator catalog response as implementation evidence, while executable readiness still waits for real runtime adapter integration |
...
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/apply?operatorRef=...&ackWarnings=true&actor=...&reason=...` | Recompute the readiness preview server-side and, when governance evidence is present, apply its candidate operator as a new owning operator-library revision; the request never accepts caller-supplied operator surface, returns `bloge.visualExecutableReadinessRecomputeResult.v1`, and writes server-managed runtime-binding parameters that user imports cannot spoof |
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/evidence-refresh?operatorRef=...&ackWarnings=true&actor=...&reason=...` | Refresh post-apply runtime-plane evidence by creating a new bound implementation binding, adapter activation, and executable lowering integration against the current trusted operator fingerprint; returns `bloge.visualExecutableReadinessEvidenceRefreshResult.v1`, supersedes the old binding, retains old evidence for audit, and supports native executable plus external-runtime-bound runtime-binding/metadata-only change surfaces |
...
| `VisualExecutableReadinessEvidenceRefreshResult` | Governed `bloge.visualExecutableReadinessEvidenceRefreshResult.v1` post-apply refresh result that rebinds implementation, activation, and executable lowering evidence to the current trusted operator fingerprint while preserving old evidence lineage |
