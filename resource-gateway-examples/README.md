...
| `POST` | `/admin/visual-operator-libraries/validate` | Validate an operator library without storing it and return `valid`, `diagnostics`, and `bloge.visualOperatorLibraryImpact.v1` with affected draft targets and replacement `changeRiskCounts`; use `force=true` to suppress stored-draft removal/disablement impact diagnostics; runtime-capability, governance, unresolved native lowering executable, same-ref fingerprint drift, and immutable-publication impact risks are reported as warnings | 200 |
