...
field names, object-template fields, native config keys, authored
`bloge:transform` assignment keys, and decision-table input/output keys must be
DSL-safe field identifiers; validation/codegen return structured
diagnostics instead of emitting DSL that will fail later. Operator library
...
The test suite is organised into four layers (61 test report classes, 685 executed
