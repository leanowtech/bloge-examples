...
property-count, property-name, or pattern-property requirement that is not a subset of the target domain, a missing required object
...
`uniqueItems`, object property-count, object property-name, and object pattern-property violations, and undeclared config fields when
...
`patternProperties` validation, then
...
and unenforced constraint keywords such as `contains`, dependency, and
...
with regex-keyed `patternProperties` schemas. Built-in bootstrap
...
constraints, object `propertyNames` and `patternProperties` constraints, required object properties, array item schema, and `additionalProperties` policy so
...
`additionalProperties`/`propertyNames`/`patternProperties` rules, so a user-imported fixed value domain cannot
