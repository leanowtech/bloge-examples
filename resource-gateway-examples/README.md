...
`uniqueItems`, object property-count, object property-name, object pattern-property, and object dependent-required violations, and undeclared config fields when
...
`patternProperties` and `dependentRequired` validation, then
...
and unenforced constraint keywords such as `contains`, `dependentSchemas`, and
...
dependencies with `dependentRequired`. Built-in bootstrap
...
`dependentRequired` constraints, required object properties, array item schema, and `additionalProperties` policy so
