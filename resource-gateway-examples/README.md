...
operator schema snapshots, export-time validation diagnostics, and the full
export-time validation/readiness snapshot. Importing a
...
diagnostics plus validation/readiness for repairable issues such as missing
operators or schema-only design graphs. The browser Drafts panel exposes this
flow through Export/Import Bundle controls and a JSON bundle editor, and feeds
the returned readiness back into Server Check so imported design-only graphs keep
their `DESIGN` publication path without requiring an immediate manual validate.
...
readiness and non-executable node rows from the publication snapshot, so a
design artifact can be reviewed without depending on the current catalog.
...
selector instead of depending on somebody remembering the draft id. The same
...
operator. The Drafts panel loads the same report after save, load, import,
restore, and fingerprint rebase, and renders the dependency mix plus
missing-catalog, drifted, and missing-snapshot states beside the revision
review. Operator and node rows in that panel can focus the corresponding canvas
node, and drifted or missing-snapshot node rows expose the same guarded
operator-fingerprint rebase action as the selected-node inspector, so dependency
review stays connected to repair work on the graph without pretending a
catalog-missing operator can be rebased. Operator-library import, delete, and
governed restore also refresh the current draft dependency panel and selected
node risk view because catalog mutations can immediately change stored draft
readiness.
...
| `GET` | `/api/visual/drafts/{draftId}/dependencies` | Summarize a stored draft as `bloge.visualGraphDraftDependencies.v1`, including distinct operator dependencies, per-node binding/edge lineage, source/lowering/runtime-readiness counts, and current/missing/drifted fingerprint state |
| `GET` | `/api/visual/drafts/{draftId}/export` | Export a portable draft bundle with operator snapshots, export-time diagnostics, and validation/readiness |
| `POST` | `/api/visual/drafts/import` | Import a portable draft bundle as a new draft identity with current operator fingerprints/snapshots plus target-environment diagnostics and validation/readiness |
...
| `GraphDraftHistorySummary` | Lightweight active/deleted draft history index entry for browser and external recovery control planes |
...
| `GraphDraftDependencyReport` | Machine-readable stored draft dependency report with distinct operatorRef usage, per-node upstream/downstream lineage, source/lowering/readiness counts, and saved-vs-current fingerprint state |
...
| `GraphDraftExportBundle` | Portable draft package with source identity, draft snapshot, operator snapshots, export-time diagnostics, and validation/readiness |
