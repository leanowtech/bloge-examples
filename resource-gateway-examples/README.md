...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/unbind` | Transition an active bound implementation into `unbound` with actor/reason audit evidence, cascading the active executable lowering integration and adapter activation to `inactive`; returns `bloge.visualRuntimeBindingDeactivationResult.v1` and removes the evidence chain from active catalog/promotion projections without deleting audit records |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/supersede` | Replace one active bound implementation with another accepted proposal for the same operator, linking `supersedesBindingId` / `supersededByBindingId` and lifecycle events; if the replacement is bound but marking the current binding as superseded fails, the controller restores the replacement to its prior proposal state and returns a failed lifecycle result instead of leaving two active bound records; the active binding is projected into the operator catalog response as implementation evidence, while executable readiness still waits for real runtime adapter integration |
