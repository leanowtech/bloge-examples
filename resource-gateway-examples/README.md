...
schema-checked source picker or a manual expression, auto-bind required inputs
...
dependency, or route relations from that impact view, search the operator palette by label, operator ref,
description, source kind/resource id, tag, input/output/config schema fields,
port-qualified field names, or field types and filter it by operator type/tag for
...
field counts, dynamic schema surfaces, DSL-unsafe fields, external effects, and
secret-bound operators before importing it into the catalog, validate
...
| `GET` | `/api/visual/operators` | List native, Java registry, imported, publication-backed subgraph, and resource-backed visual operator definitions; supports `tenantId`, `namespace`, and `environment` policy filtering plus schema-aware search across input/output/config fields and field types |
...
decision-table input/output keys must be DSL-safe field identifiers; validation/codegen return structured
diagnostics instead of emitting DSL that will fail later, and the browser filters
DSL-unsafe schema paths out of source pickers and schema-derived node handles before
authors can wire them into a draft. Operator library
validation applies the same DSL-safe field-name gate to native input/config
...
catalog. Graph names and node ids are also guarded against DSL reserved
