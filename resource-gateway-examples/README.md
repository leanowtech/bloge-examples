...
`operationId`, `channel`, `action`, and `messageName` selectors to generate only
the reviewed subset. The browser Operator Libraries panel exposes this as a
`Discover AsyncAPI` -> operation selection -> `From AsyncAPI` preview action:
authors paste AsyncAPI into the same source editor, choose a candidate when
needed, preview the generated library, then reuse the existing Validate/Import
path and warning acknowledgement flow.
...
| `POST` | `/admin/visual-operator-libraries/from-asyncapi` | Preview-project parsed `asyncApi` or raw JSON/YAML `asyncApiText` into a `bloge.visualOperatorLibrary.v1` draft using runtime-blocked `event-source`, `message-handler`, or `webhook` operators, optionally narrowed by `operationId`, `channel`, `action`, and `messageName` selectors, then return `bloge.asyncApiOperatorLibraryImportResult.v1` with the generated library plus the same validation/profile/impact evidence used by normal imports; this endpoint does not store the generated library | 200 |
