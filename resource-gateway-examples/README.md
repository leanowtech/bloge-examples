...
operator. Operator-level catalog diagnostics are surfaced on palette cards and
inside the selected operator's contract panel, so catalog ownership or projection
warnings stay visible while composing. Input binding rows also summarize how
...
graphs, or Java runtime operators, refs already projected from runtime Java
operators, empty libraries, duplicate port names, unsupported
lowering modes, native lowering without a namespace-safe executable
`operatorRef`, native lowering that targets hidden system executors such as
`visualPublication`, transform lowering without executable `assignments`, branch
...
The test suite is organised into four layers (63 test report classes, 765 executed
