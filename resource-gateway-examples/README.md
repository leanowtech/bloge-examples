...
user-provided operator library JSON before importing it, while the server-side
...
the browser profile marks those same runtime/governance risks as review-level warnings,
...
diagnostics for cross-environment review,
...
the selected service-managed fingerprint snapshots from the active catalog,
...
operator schema snapshots, and export-time validation diagnostics. Importing a
bundle creates a new draft identity and revision, refreshes node fingerprints
from the active catalog for the imported draft's scope, rejects unsupported
...
diagnostics for repairable issues such as missing operators. The browser Drafts panel exposes this
flow through Export/Import Bundle controls and a JSON bundle editor, so complex
canvas compositions can be copied into another environment without using a stale
`draftId` as an overwrite path.
...
layout, and validation/generation reports for audit or later promotion. Publish
requests reject warning-level validation diagnostics until the caller repeats the
request with `ackWarnings=true`, so non-idempotent side effects and similar
promotion risks are explicitly reviewed before an immutable artifact is stored.
Published artifacts can be run directly from their frozen DSL, so execution no longer
depends on whatever the current operator catalog exposes after publication. Each
artifact is also projected back into the operator catalog as a reusable
schema-aware subgraph operator scoped to the publication's tenant, namespace,
and environment. Its projected output schema preserves the publication's saved
output path, including named output-port paths and whole-node multi-port output
objects, so downstream drafts keep the same schema gate when they reuse the
subgraph. Dragging that operator into another visual draft lowers to the
reserved `visualPublication` runtime executor, which loads and runs the frozen
publication DSL by immutable id while keeping internal `_bloge*` call-stack
metadata out of graph-input schema validation. The
...
| `GET` | `/api/visual/operators` | List native, Java registry, imported, publication-backed subgraph, and resource-backed visual operator definitions; supports `tenantId`, `namespace`, and `environment` policy filtering plus multi-term schema-aware search across input/output/config fields and field types |
| `GET` | `/api/visual/operators/{operatorRef}/usage` | Return stored draft and immutable-publication usage of one operatorRef, including saved/frozen fingerprint status and publication changed-surface drift summaries |
...
| `GET` | `/api/visual/drafts/{draftId}/export` | Export a portable draft bundle with operator snapshots and export-time diagnostics |
| `POST` | `/api/visual/drafts/import` | Import a portable draft bundle as a new draft identity with current operator fingerprints and target-environment diagnostics |
...
| `POST` | `/api/visual/drafts/{draftId}/operator-fingerprints/rebase` | Explicitly refresh selected or all service-managed node fingerprint snapshots against the current operator catalog using an `expectedRevision` guard |
...
| `POST` | `/api/visual/drafts/{draftId}/publish` | Validate, compile, and publish an immutable visual graph artifact; optional `expectedRevision` rejects stale publishes with `409 CONFLICT`, and warning-level validation diagnostics require `ackWarnings=true` before storage |
...
fingerprint status, and publication changed-surface summaries next to the local
canvas impact view. Once loaded, the same usage status is reflected on the
...
action for reviewed drift, preserving the rule that ordinary draft saves never
...
summaries when a publication's frozen operator snapshot differs from the current
catalog definition.
...
draft snapshots. Those warnings include a concise changed-surface summary such
as the affected input/output/config schema, capability, policy, or lowering
area, so authors can review and resave affected drafts before runtime
...
`ackWarnings=true` before it writes the library. The impact contract also carries
`draftTargets` with affected node indexes. Affected draft chips in the review
are actionable: selecting one loads the draft through the normal draft loader
and focuses the impacted node so the author can inspect fingerprints, schema
drift, and bindings before choosing a rebase or repair path.

| Method | Path | Description | Status |
|--------|------|-------------|--------|
| `GET` | `/admin/visual-operator-libraries` | List imported operator libraries | 200 |
| `POST` | `/admin/visual-operator-libraries/validate` | Validate an operator library without storing it and return `valid`, `diagnostics`, and `bloge.visualOperatorLibraryImpact.v1` with affected draft targets; use `force=true` to suppress stored-draft removal/disablement impact diagnostics; runtime-capability, governance, unresolved native lowering executable, same-ref fingerprint drift, and immutable-publication impact risks are reported as warnings | 200 |
| `POST` | `/admin/visual-operator-libraries` | Import or re-import an operator library; rejected or warning-gated responses include `bloge.visualOperatorLibraryImpact.v1`, reject removal or disablement of stored-draft operator refs unless `force=true`, and require `ackWarnings=true` before storing warning-level runtime-capability, governance, executable-resolution, or replacement impact | 201 / 400 / 409 |
| `GET` | `/admin/visual-operator-libraries/{libraryId}` | Get one imported library | 200 / 404 |
| `PUT` | `/admin/visual-operator-libraries/{libraryId}` | Replace an imported library; rejected or warning-gated responses include `bloge.visualOperatorLibraryImpact.v1`, reject removal or disablement of stored-draft operator refs unless `force=true`, and require `ackWarnings=true` before storing warning-level runtime-capability, governance, executable-resolution, or replacement impact | 200 / 400 / 409 |
| `DELETE` | `/admin/visual-operator-libraries/{libraryId}` | Delete an imported library; rejects stored-draft references and published-artifact references unless `force=true`, returning `bloge.visualOperatorLibraryImpact.v1` on conflict | 204 / 409 |
...
source kinds, arbitrary non-`user-library` source kinds, refs already projected
from runtime Java operators, user-supplied operator diagnostics, empty
libraries, null operator entries, null input/output port entries, duplicate port names, unsupported
...
input schemas, and output schemas across lowering modes, plus native config
...
| `DefaultVisualOperatorCatalog` | Combines native visual operators, Java registry operators, imported user libraries, publication-backed subgraph operators, and `resource:<resourceId>` virtual operators |
...
| `GraphDraftExportBundle` | Portable draft package with source identity, draft snapshot, operator snapshots, and export-time diagnostics |
| `GraphDraftImportResult` | Import response contract with stored draft identity and target-environment compatibility diagnostics |
...
| `VisualOperatorCatalogController` / `VisualOperatorUsageController` / `VisualGraphDraftController` | Visual operator discovery, usage impact indexing, draft validation, compilation, and execution |
...
The test suite is organised into four layers (67 test report classes, 882 executed
