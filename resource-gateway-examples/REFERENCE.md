...
> Long-form reference retained from the previous README. Start with
> [README.md](README.md) for the focused product overview, demo path, and quick
> start. Use this file when you need the full endpoint catalog,
> implementation notes, historical detail, and test-layer breakdown.
...
single-operator definition endpoint, and selected-node refresh uses the
`includeProjections=true` detail envelope so the inspector can show
server-derived runtime binding and executable promotion blockers without
trusting imported operator definitions to declare their own runtime state. When
an author selects a node, the palette can also ask the server for a
...
schema-field summary plus streaming/durable/suspendable/secret/effect capability badges before dragging, open a catalog-level operator detail panel from operator actions to review source, lowering, port/config contract, server-derived readiness, projection blockers, and add/refresh controls even before the operator is placed on the canvas, inspect the selected node's contract
...
validate/import path expands safe local JSON Pointer schema references such as `#/$defs/*`,
`#/definitions/*`, and `#/components/schemas/*`, blocks unresolved local refs with
...
registry, refresh the palette without leaving the browser, and open a palette
operator's Details review before choosing Add or dragging it onto the canvas,
...
Server-side connection candidate discovery also expands array item and tuple
`prefixItems` target paths such as `items.0` before running the same preflight,
so large custom schemas expose list-element drop targets without browser-side
path guessing.
...
palette cards, the explicit Details operator review, the catalog-level operator detail panel, and inside the selected operator's contract panel, so catalog
...
such as `["string", "null"]`, pure local JSON Pointer references such as
`{"$ref":"#/$defs/Customer"}` or `{"$ref":"#/components/schemas/Customer"}`, strict object `properties`,
...
the canvas-supported subset. Pure local JSON Pointer `$ref` nodes are normalized to
...
New draft saves and full-form saves now also treat repository write failures as
control-plane diagnostics: create returns `visual.draft.createPersistenceFailed`
with the candidate draft readiness preserved, while full save returns
`visual.draft.updatePersistenceFailed` with the current draft snapshot, so a
schema-only design session is not mistaken for a runtime implementation failure.
...
design graphs. If the target repository cannot persist the imported draft, the API returns
`409 CONFLICT` with `visual.draft.importPersistenceFailed`, keeps `imported=false`, and still returns the
target preview validation/dependency/runtime-binding handoff evidence so migration tooling can distinguish storage failure
from schema or runtime implementation gaps. The browser Drafts panel sends
...
Status, Endpoint, Schema, Runtime, Library, Operator, Source, Lowering, and
...
The selected-operator contract panel also renders a bounded schema outline for
complex imported operator schemas. The browser regression imports a design-only
operator with nested objects, `patternProperties`, `additionalProperties`,
array `contains`, `oneOf`/`anyOf`, `dependentRequired`, output `prefixItems`,
and annotations, then verifies the prioritized 24-row outline, overflow
message, stable DOM paths, cross-port schema search for deep paths, Clear
recovery, and 390px mobile search/no-overflow behavior.
Stored draft dependency reports also expose schema drift paths, and the
selected-operator contract panel overlays breaking/compatible drift onto the
affected port summary and matching schema outline rows. The browser regression
saves a draft against a `number` input schema, replaces the library with an
`integer` schema, verifies `inputs.score` breaking drift in the dependency
panel, filters the selected contract with `Schema Search`, and replays the same
overlay under a 390px mobile viewport without horizontal overflow.
The drift contract now also carries the frozen snapshot type, current catalog
type, and a review hint. The dependency panel, port summary, outline row label,
and schema search all use that shared data, so a breaking field drift can show
`number -> integer` with `Review bindings before rebase`, while compatible
schema drift can still expose the first changed path such as
`integer -> number`.
It also carries bounded keyword-level `schemaChanges` rows for the affected
...
roles with position semantics, and drift labels are bound back to the affected
schema row through `aria-describedby`.
...
frozen publication snapshot while rejecting duplicate publication ids and
returning structured persistence diagnostics when the target repository cannot
store the publication. Publish and import storage failures return machine-readable
diagnostics (`visual.publication.persistenceFailed` or
`visual.publication.importPersistenceFailed`) instead of a generic 500, so
browser and external control planes can retry or route the failure without
misclassifying the graph as invalid. The
...
Golden regression persistence failures also return structured `409` diagnostics:
case save/delete failures use `visual.golden.savePersistenceFailed` and
`visual.golden.deletePersistenceFailed`; failed run-history writes during
single-case, suite, or certification execution use
`visual.golden.runHistoryPersistenceFailed`; and failed certification storage
uses `visual.golden.certificationPersistenceFailed`. Diagnostics carry
publication/case/certification context, attempted mutation action, and exception
type so control-plane clients can route platform storage failures separately
from assertion failures or stale certification gates.
...
| `GET` | `/api/visual/assets/runtime-binding-requirements/runtime-evidence` | Return `bloge.visualRuntimeEvidenceWindow.v1`, a pageable mixed runtime fact window for implementation bindings, adapter activations, rollout observations, and executable lowering integrations with `evidenceKind`, operator, binding, activation, lifecycle, rollout, rollout-signal, and breached-only filters; when kind and signal filters are combined, rollout facts identify the chain and the returned window contains only the matching kind from that chain context |
...
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/validate` | Validate a stateless `bloge.visualRuntimeBindingImplementationBinding.v1` implementation proposal against source handoff fingerprint, source requirement keys, a handoff operator contract snapshot, and current catalog fingerprint; current requirement keys for a different operator are rejected, stale/missing source evidence requires review, and the response returns `bloge.visualRuntimeBindingImplementationValidation.v1` with ready-to-bind/requires-review/rejected state, evidence diagnostics, catalog drift status, optional `implementationVersion` / `reimplementationOfBindingId` / `reimplementationStrategy` SemVer governance diagnostics, rollout/canary/rollback `rolloutPlan` diagnostics, and field-level contract-diff diagnostics that reuse shared JSON Schema compatibility checks to reject breaking port/config schema drift with machine-readable reasons while routing compatible schema/runtime/governance/metadata drift to review |
| `GET` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings` | List stored `bloge.visualRuntimeBindingImplementationBindingRecord.v1` implementation proposals with optional `operatorRef` and `state` filters, preserving validation evidence for later bind/unbind/supersede workflow steps |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings` | Submit and persist a valid runtime implementation proposal after the same contract/evidence validation gate; rejected proposals are not stored, an exact stable `bindingId` replay returns the existing record as an idempotent `200 OK`, the same `bindingId` with different submitted evidence returns `409 CONFLICT`, declared reimplementations must reference an existing same-operator base binding and advance SemVer unless marked as rollback, repository persistence failures return structured `409` validation diagnostics instead of generic 500s, and accepted records remain control-plane evidence rather than closing runtime-binding requirements |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/bind` | Transition a ready-to-bind or review-acknowledged proposal into the active `bound` lifecycle state with actor/reason audit evidence; optional `expectedRevision` rejects stale lifecycle acknowledgements with `409` and `visual.runtimeBindingImplementation.revisionConflict`, while an exact replay of the same persisted lifecycle event remains idempotent `200 OK`; different already-bound requests still conflict; rejects missing governance evidence, review-required proposals without `ackReview`, already bound/superseded records, and operators with an existing active binding; repository update failures return `409`, `state=failed`, and `visual.runtimeBindingImplementation.bindPersistenceFailed` with exception metadata instead of leaking generic 500s |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/unbind` | Transition an active bound implementation into `unbound` with actor/reason audit evidence, cascading the active executable lowering integration and adapter activation to `inactive`; optional `expectedRevision` rejects stale unbind acknowledgements with `409`, while an exact replay of the same unbind lifecycle event remains idempotent `200 OK` with the stored inactive evidence; different already-unbound requests still conflict; if a later cascade step fails after runtime evidence was already deactivated, the controller restores that evidence to `active`, realigns the executable integration with the restored activation revision, and returns `state=failed` with compensation diagnostics instead of silently breaking the active evidence chain; returns `bloge.visualRuntimeBindingDeactivationResult.v1` and removes the evidence chain from active catalog/promotion projections only on successful unbind |
| `POST` | `/api/visual/assets/runtime-binding-requirements/implementation-bindings/{bindingId}/supersede` | Replace one active bound implementation with another accepted proposal for the same operator, linking `supersedesBindingId` / `supersededByBindingId` and lifecycle events; optional `expectedRevision` and `expectedReplacementRevision` reject stale current or replacement binding acknowledgements with `409`, while an exact replay of the same supersede lifecycle pair remains idempotent `200 OK`; different supersede attempts against an already superseded binding still conflict; if the replacement is bound but marking the current binding as superseded fails, the controller restores the replacement to its prior proposal state and returns a failed lifecycle result instead of leaving two active bound records; the active binding is projected into the operator catalog response as implementation evidence, while executable readiness still waits for real runtime adapter integration |
...
| `POST` | `/api/visual/assets/runtime-binding-requirements/rollout-observations/validate` | Validate a `bloge.visualRuntimeRolloutObservationRequest.v1` canary/ramp/rollback execution observation against an active adapter activation, bound implementation, current catalog fingerprint, bound `rolloutPlan`, observed traffic, rollback signal, actor/reason, rollout evidence, and optional structured `rolloutSignals[]` guardrails without persisting state; degraded or rolled-back observations are recordable facts when contract identity is current |
| `GET` | `/api/visual/assets/runtime-binding-requirements/rollout-observations` | List stored `bloge.visualRuntimeRolloutObservation.v1` rollout execution facts with optional `activationId`, `bindingId`, `operatorRef`, `state`, structured `rolloutSignal`, and `breachedOnly=true` guardrail filters |
| `POST` | `/api/visual/assets/runtime-binding-requirements/rollout-observations` | Persist one runtime rollout execution observation for an active adapter activation; rejects missing/stale activation, unbound or drifted binding, catalog drift, rollout strategy mismatch, invalid traffic, missing rollback signal when rollback is triggered, missing actor/reason, and missing rollout evidence; optional `rolloutSignals[]` records named guardrails such as `p95-latency`, `error-rate`, or `golden-regression` with breached status for overview aggregation; exact stable `observationId` replay returns the existing fact as `200 OK`, same-id different evidence or signals returns `409`, and accepted observations remain audit/control-plane feedback rather than executable readiness |
...
| `GET` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/preview?operatorRef=...` | Preview the candidate trusted `OperatorDefinition` surface for a `readiness-recompute-required` operator without mutating the trusted catalog or operator-library registry; native integrations derive `RUNTIME_EXECUTABLE`, while non-`design` external integrations derive `EXTERNAL_RUNTIME_BOUND` without claiming local request-response execution |
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/apply?operatorRef=...&ackWarnings=true&actor=...&reason=...` | Recompute the readiness preview server-side and, when governance evidence is present, apply its candidate operator as a new owning operator-library revision; optional `expectedCurrentOperatorFingerprint` and `expectedCandidateOperatorFingerprint` reject stale preview acknowledgements with `409`, while an exact retry after a successful revision write can return the already-written revision when the trusted current operator now matches the expected candidate fingerprint, the previous revision matches the expected current fingerprint, and the current lowering carries server-recomputed runtime-binding evidence; operator-library revision write failures return structured `state=failed` diagnostics instead of leaking as generic 500s; the request never accepts caller-supplied operator surface, returns `bloge.visualExecutableReadinessRecomputeResult.v1`, and writes server-managed runtime-binding parameters that user imports cannot spoof |
| `POST` | `/api/visual/assets/runtime-binding-requirements/executable-readiness-recomputations/evidence-refresh?operatorRef=...&ackWarnings=true&actor=...&reason=...` | Refresh post-apply runtime-plane evidence by creating a new bound implementation binding, adapter activation, and executable lowering integration against the current trusted operator fingerprint; optional `expectedPreviousOperatorFingerprint` and `expectedCurrentOperatorFingerprint` reject stale evidence-refresh acknowledgements with `409`, while an exact retry using the same stable `refreshedBindingId` / `refreshedActivationId` / `refreshedIntegrationId` returns the already-created refreshed chain instead of falsely reporting stale after a lost response; returns `bloge.visualExecutableReadinessEvidenceRefreshResult.v1`, supersedes the old binding, retains old evidence for audit, supports native executable plus external-runtime-bound runtime-binding/metadata-only change surfaces, returns structured failed results for refreshed binding create failures, compensates binding-transition and activation/integration write failures by marking partial refreshed evidence `failed` and restoring the source binding to `bound`, and only reports `current` when the active binding, adapter activation, and executable lowering integration form a complete aligned evidence chain |
...
| `PUT` | `/api/visual/drafts/{draftId}` | Replace a stored visual graph draft when the submitted `revision` matches; stale full saves and repository guarded-update failures return `409 CONFLICT` with current draft diagnostics (`visual.draft.revisionConflict` or `visual.draft.updatePersistenceFailed`) |
...
| `POST` | `/api/visual/golden-cases` | Save a golden regression case for an existing publication, with save-time schema-version/context/output-node/assertion validation, exact-output fallback, explicit output assertions, numeric tolerance assertions, output-schema assertions, and structured save persistence-failure diagnostics |
| `DELETE` | `/api/visual/golden-cases/{caseId}` | Delete one golden regression case; the next certification status reflects the changed case-set fingerprint, and delete persistence failures return structured diagnostics without removing the case |
| `POST` | `/api/visual/golden-cases/{caseId}/run` | Execute a golden case with the publication's frozen DSL and return exact-output, value/tolerance assertion, schema assertion, or run-history persistence diagnostics |
| `POST` | `/api/visual/golden-cases/publications/{publicationId}/run` | Execute every golden case bound to a publication and summarize total/passed/failed counts, including suite-level run-history persistence diagnostics |
...
Resource-contract registry write failures return structured `409` diagnostics
instead of generic server errors. Upsert returns
`visual.resourceContract.upsertPersistenceFailed`; delete returns
`visual.resourceContract.deletePersistenceFailed`. The diagnostics carry the
resource id, contract id, attempted mutation action, and exception type, so the
browser or an external control plane can distinguish platform storage failure
from schema validation, impact review, or runtime-readiness rejection.
...
The same readiness response now also groups those requirements into stable
`runtimeBindingHandoffGroups[]` by owner library, handoff lane/kind/target, and
binding kind, so external runtime-plane teams can claim a batch of missing
bindings without reimplementing multi-field grouping client-side.
...
the current read model and current operator catalog contracts. The review includes
exported/current/new-window routing distributions by operator, owner library,
binding kind, handoff lane/work/target, source, lowering, readiness, and artifact
kind, plus operator contract snapshot status and field-change categories, so
design-time binding work is visible, portable, assignable, and replay-checkable from the canvas without
...
source handoff fingerprint, source requirement keys, operator contract snapshot,
operator fingerprint, current catalog fingerprint, implementation metadata,
test evidence, policy evidence, rollback target, and rollout plan,
...
their validation evidence. Implementation metadata can optionally include
`implementationVersion`, `reimplementationOfBindingId`, and
`reimplementationStrategy` (`compatible`, `breaking`, `adapter-migration`, or
`rollback`). The stateless gate rejects malformed SemVer or unknown strategies,
and the submit gate checks the referenced base binding, same-operator ownership,
and forward SemVer progression before storing reimplementation evidence; strategy/version
mismatches that are not blocking remain `requires-review` evidence.
Implementation metadata can also include `rolloutPlan` with `strategy`,
`initialTrafficPercent`, `maxTrafficPercent`, `rollbackSignal`,
`rollbackWindow`, and rollout evidence. Missing plans, missing rollback triggers,
high-risk immediate rollout, or high-risk rollout without evidence are routed to
review; unsupported strategies or invalid traffic windows are rejected before a
proposal can be stored.
Lifecycle endpoints can then mark a reviewed proposal
as `bound`, unbind one active binding with cascaded runtime evidence deactivation, or supersede one active binding with another, preserving actor/reason
audit events, inactive runtime evidence, and replacement lineage. The browser
Workspace Overview also loads these records as Runtime Implementation Bindings,
supports operator/state filtering, and issues Bind/Unbind/Supersede transition
requests with the visible record revision as `expectedRevision` plus
`expectedReplacementRevision` for supersede replacements, so stale UI
acknowledgements are rejected instead of overwriting newer runtime-plane
decisions. These records are now control-plane
implementation facts. The operator catalog response now exposes server-derived
`runtimeBindingProjections[]` with `binding-required`, `binding-bound`,
`binding-drifted`, `adapter-active`, `adapter-drifted`, and `not-required`
projection states, so palette and external control planes can see active
implementation and adapter activation evidence without letting imported operator
...
evidence before persistence; exact stable integration id replay returns the
existing fact, while same-id different-evidence integration conflicts remain
`409`. Even after integration, the API does not rewrite
...
normalized `evidenceChainHealthCounts` map for `complete`, `partial`,
`missing-activation`, `missing-integration`, `orphan-activation`,
`orphan-integration`, `failed-record`, and `rollout-risk`, plus
`rolloutSignalCounts` and `breachedRolloutSignalCounts` for structured
...
in the palette, route `runtime-evidence` actions to the matching implementation
binding or runtime evidence filter/focus state, and de-duplicate repeated
...
operator is executable by injecting its own readiness metadata. Streaming and
durable runtime-blocked operators still advertise `artifactKinds=["DESIGN"]` in
that readiness contract, matching the draft/publication rule that their schemas
are authorable and freezable even though this request-response runtime cannot
execute them.
...
| `GET` | `/admin/visual-operator-libraries/{libraryId}/export` | Export the current library as `bloge.visualOperatorLibraryExport.v1`, including normalized library snapshot, latest registry revision evidence, and export-time validation/profile/impact result; `{libraryId}=builtin` returns the server Java operator registry as a virtual portable bundle when no stored library shadows that id | 200 / 404 |
...
Operator-library registry mutation failures are returned as structured `409`
diagnostics instead of generic server errors. Create, import-text, and replace
return `visual.library.importPersistenceFailed`; bundle import returns
`visual.library.importBundlePersistenceFailed`; revision restore returns
`visual.library.restorePersistenceFailed`; delete returns
`visual.library.deletePersistenceFailed`. Each diagnostic carries the library
id, attempted mutation action, and exception type so browsers or external
control planes can retry or route the storage failure without confusing it with
schema/readiness rejection.
...
| `OperatorRuntimeBindingProjection` | Server-derived catalog response read model that joins active bound implementation records back to visible operators as missing/bound/drifted/not-required evidence without changing trusted `OperatorDefinition.runtimeReadiness` |
...
| `VisualRuntimeBindingImplementationValidation` | Stateless `bloge.visualRuntimeBindingImplementationValidation.v1` pre-bind gate that validates a runtime team's implementation proposal against source handoff fingerprint, source requirement keys, a handoff operator contract snapshot, current catalog fingerprint, semantic JSON Schema-aware field-level contract diff, implementation metadata, test evidence, policy evidence, rollback target, and rollout/canary/rollback plan without closing runtime-binding workflow state |
...
| `VisualRuntimeEvidenceWindow` | Queryable `bloge.visualRuntimeEvidenceWindow.v1` read model that pages mixed implementation binding, adapter activation, rollout observation, and executable lowering integration facts with normalized kind/operator/binding/activation/lifecycle/rollout/signal filters, total/unfiltered counts, facet counts, rollout signal counts, and chain-aware breached-signal context |
...
| `VisualExecutableReadinessRecomputeResult` | Governed `bloge.visualExecutableReadinessRecomputeResult.v1` apply result that recomputes the preview server-side, requires warning acknowledgement plus actor/reason evidence, writes native executable or external-runtime-bound candidate surfaces as new owning operator-library revisions when fingerprints still match, supports exact lost-response replay against the already-written server-recomputed candidate revision and its prior expected-current revision, and reports revision-write persistence failures as structured failed results |
| `VisualExecutableReadinessEvidenceRefreshResult` | Governed `bloge.visualExecutableReadinessEvidenceRefreshResult.v1` post-apply refresh result that rebinds implementation, activation, and executable lowering evidence to the current trusted operator fingerprint while preserving old evidence lineage, supporting stable-id replay after lost responses, and exposing blocked diagnostics for incomplete current evidence chains |
...
| `VisualDslRunner` | Visual-owned SPI for compiling/running generated BLOGE DSL, keeping `gateway.visual.runtime` and `gateway.visual.simulation` behind an adapter boundary rather than importing the resource-gateway dynamic composer directly |
| `VisualGraphRunService` | Uses `VisualDslRunner` to validate draft input context, compile, and execute visual drafts or frozen publications while returning draft or frozen-publication validation/readiness/action-readiness |
...
The test suite is organised into four layers (75 test report classes, 1153 test
