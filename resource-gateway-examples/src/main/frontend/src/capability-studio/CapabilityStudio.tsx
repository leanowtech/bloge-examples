...
function GovernedBaselineResult({ projection, locale }: { projection: GovernedBaselineSuccessProjection; locale: 'en' | 'zh-CN' }) {
  ...
  return <section className="capability-governed-result" data-testid="governed-baseline-result" aria-labelledby="governed-result-heading">
    <div className="capability-governed-result-heading"><div><CheckCircle2 size={22} aria-hidden="true" /><span><small>{locale === 'zh-CN' ? '开发验证 · 探索性证据' : 'Development verification · exploratory evidence'}</small><strong id="governed-result-heading">{locale === 'zh-CN' ? '27 项业务检查全部通过' : 'All 27 business checks passed'}</strong></span></div><div className="capability-governed-gate"><AlertTriangle size={17} aria-hidden="true" /><span><small>{locale === 'zh-CN' ? '发布门禁' : 'Release gate'}</small><strong>{locale === 'zh-CN' ? '仍不可验收' : 'Still not accepted'}</strong></span></div></div>
    ...
    <details className="capability-technical-details"><summary><ChevronDown size={15} aria-hidden="true" /> {locale === 'zh-CN' ? '验证技术证据' : 'Verification technical evidence'}</summary><dl><div><dt>Suite</dt><dd>{projection.publication.suiteRef.id}@{projection.publication.suiteRef.revision}</dd></div><div><dt>Suite fingerprint</dt><dd>{projection.publication.suiteRef.fingerprint}</dd></div><div><dt>Compilation</dt><dd>{projection.compilationFingerprint}</dd></div><div><dt>Source map</dt><dd>{projection.sourceMapFingerprint}</dd></div><div><dt>Provenance</dt><dd>{projection.provenanceFingerprint}</dd></div>{projection.rounds.map((round) => <div key={round.round}><dt>Round {round.round}</dt><dd>{round.suiteRunId} · {round.evidenceFingerprint}</dd></div>)}{projection.cases.map((caseResult) => <div key={caseResult.caseId}><dt>{caseResult.oracleId}</dt><dd>{caseResult.semanticResultFingerprint}</dd></div>)}</dl></details>
  </section>;
}
...
function ReadinessPanel({ model, text, locale, task, governedBaseline, governedBaselineError, governedBaselineLoading, onNextAction }: { model: CapabilityStudioModel; text: (value: Parameters<typeof localized>[0]) => string; locale: 'en' | 'zh-CN'; task: Task; governedBaseline: GovernedBaselineSuccessProjection | null; governedBaselineError: Error | null; governedBaselineLoading: boolean; onNextAction: () => void }) {
  ...
  const readinessCopy = task === 'tool' && governedBaseline?.status === 'PASSED' ? (locale === 'zh-CN' ? '9 项业务判定与 27 项业务断言已通过；候选构建、环境证明和签署仍阻断发布。' : 'Nine business Oracles and 27 business assertions passed; candidate build, environment attestation, and sign-off still block release.') : (locale === 'zh-CN' ? '设计资产已加载；发布验收证据尚未闭合。' : 'Design assets are loaded; release acceptance evidence is incomplete.');
