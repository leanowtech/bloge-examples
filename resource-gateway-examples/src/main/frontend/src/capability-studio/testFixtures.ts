...
export const governedBaselineProjectionFixture = {
  ...
  cases: [
    ...
    caseId,
    oracleId: `oracle-${caseId.replace(/^case-/, '')}`,
    oracleStatus: 'PASS',
    semanticResultFingerprint: governedBaselineFingerprint(
      `${((caseIndex + 10) % 16).toString(16)}`,
    ),
    assertionsEvaluated: 3,
    assertionsPassed: 3,
    fixtureControlsEvaluated: 18,
    fixtureControlsSatisfied: 18,
    proofs: [
      'BUSINESS_ASSERTION_PASSED',
      'SEMANTIC_RESULT_STABLE',
      'FIXTURE_CONTROL_SATISFIED',
      'ZERO_REAL_EXTERNAL_CALLS',
      ...(caseId === 'case-compensation-history-timeout' ? ['TIMEOUT_FALLBACK_CONFIRMED'] : []),
      ...(caseId === 'case-duplicate-cancellation' ? ['DUPLICATE_IDEMPOTENCY_CONFIRMED'] : []),
      ...(caseId === 'case-forbidden-write-effect' ? ['FORBIDDEN_WRITE_EFFECT_ABSENT'] : []),
    ],
    rounds: Array.from({ length: 3 }, (_, roundIndex) => ({
      round: roundIndex + 1,
      ...
      fixtureFingerprint: governedBaselineFingerprint(
        `${(caseIndex + 1).toString(16)}`,
      ),
      evidenceFingerprint: governedBaselineFingerprint(
        `${((caseIndex * 3 + roundIndex + 1) % 16).toString(16)}`,
      ),
      semanticResultFingerprint: governedBaselineFingerprint(
        `${((caseIndex + 10) % 16).toString(16)}`,
      ),
      ...
      assertionsPassed: 1,
      fixtureControlsEvaluated: 6,
      fixtureControlsSatisfied: 6,
    })),
  })),
  limitations: [
