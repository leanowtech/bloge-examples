  ...
  return (
    <section className="decision-scenario-workbench" data-testid="decision-scenario-workbench">
      <div className="decision-scenario-controls">
        <label><span>{t('Output kind')}</span><select aria-label={t('Decision output kind')} value={outputKind} onChange={(event) => { const next = event.target.value as DecisionOutputKind; setOutputKind(next); props.onOutputKindChange?.(next); }}><option value="object">{t('object')}</option><option value="scalar">{t('scalar')}</option><option value="plan">{t('plan')}</option><option value="dispatch">{t('dispatch (model-only)')}</option></select></label>
