...
/** H2/PostgreSQL-compatible Package command lock and restart-safe receipt journal. */
public final class DatabaseDomainCapabilityPackageSaveReceiptRepository
        implements DomainCapabilityPackageSaveReceiptRepository {
    ...
    public DatabaseDomainCapabilityPackageSaveReceiptRepository(JdbcTemplate jdbc, ObjectMapper mapper) {
    ...
    public <T> T withCommandLock(
        ...
        jdbc.queryForObject("""
                        SELECT idempotency_key FROM business_mirror_package_save_locks
    ...
    public void save(CapabilitySnapshot.Scope scope,
        ...
        jdbc.update("""
                        INSERT INTO business_mirror_package_save_receipts (
    ...
    private Optional<DomainCapabilityPackageSaveReceipt> read(
            String json,
        ...
        try {
            DomainCapabilityPackageSaveReceipt receipt =
                    mapper.readValue(json, DomainCapabilityPackageSaveReceipt.class);
            if (!receipt.requestFingerprint().equals(requestFingerprint)
                ...
                throw new IllegalStateException("Stored Package receipt integrity check failed");
            }
            return Optional.of(receipt);
        } catch (JsonProcessingException failure) {
            throw new IllegalStateException("Failed to decode Package save receipt", failure);
        }
    }

    private String serialize(DomainCapabilityPackageSaveReceipt receipt) {
        try {
            return mapper.writeValueAsString(receipt);
        } catch (JsonProcessingException failure) {
            throw new IllegalStateException("Failed to encode Package save receipt", failure);
        }
    }

    private static Dialect detectDialect(JdbcTemplate jdbc) {
        if (jdbc.getDataSource() == null) {
            throw new IllegalStateException("Package receipt repository requires a JDBC DataSource");
        }
        try (var connection = jdbc.getDataSource().getConnection()) {
            ...
            throw new IllegalStateException("Unsupported Package receipt database: " + product);
        } catch (SQLException failure) {
            throw new IllegalStateException("Failed to inspect Package receipt database", failure);
        }
    }

    private enum Dialect {
        POSTGRESQL("""
                INSERT INTO business_mirror_package_save_locks (
                    tenant_id, organization_id, project_id, environment_id, region_id, idempotency_key
                ) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING
                """),
        H2("""
                MERGE INTO business_mirror_package_save_locks (
                    tenant_id, organization_id, project_id, environment_id, region_id, idempotency_key
