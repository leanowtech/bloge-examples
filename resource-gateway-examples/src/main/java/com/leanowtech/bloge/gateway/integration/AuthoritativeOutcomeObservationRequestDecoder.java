...
/**
 * Strict bounded post-authentication decoder for authoritative outcome observation admission.
 *
 * <p>Callers must authenticate before invoking this decoder. Duplicate, unknown, missing,
 * trailing, oversized, or structurally unbounded JSON is rejected before Jackson constructs the
 * protocol record. Business-authority and Resource Gateway signature checks remain in the
...
@ConditionalOnProperty(
        ...
        name = "enabled",
        havingValue = "true")
public final class AuthoritativeOutcomeObservationRequestDecoder {
    /** Largest admitted raw command, including its small admission envelope. */
    public static final int MAXIMUM_REQUEST_BYTES =
            AuthoritativeOutcomeObservation.MAXIMUM_CANONICAL_BYTES
                    + 16 * 1024;
    ...
    public AuthoritativeOutcomeObservationRequestDecoder(
            ObjectMapper mapper) {
    ...
    /**
     * Decodes one exact payload-free admission command.
     *
     ...
     */
    public AuthoritativeOutcomeObservationAdmissionRequest decode(
            byte[] value,
        ...
        try {
            ...
            return strictMapper.treeToValue(
                    tree,
                    AuthoritativeOutcomeObservationAdmissionRequest.class);
    ...
    private static IntegrationProblemException invalid(
        ...
        return new IntegrationProblemException(
                IntegrationProblem.badRequest(
                        ...
                        "The authoritative outcome admission does not match the bounded public protocol.",
