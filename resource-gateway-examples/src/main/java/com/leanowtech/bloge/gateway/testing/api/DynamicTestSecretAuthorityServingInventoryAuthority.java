...
/**
 * Atomically refreshes a witnessed test-secret serving-inventory publication over HTTPS.
 *
 * <p>Bootstrap verifies the independently signed inventory, active-or-revoked publication,
 * witness checkpoint, exact local authority binding and durable predecessor floor before exposing
 * any observation. A refresh publishes the complete successor or no successor. Transport,
 * protocol, parsing, signature, time, predecessor or floor ambiguity immediately marks the local
 * source unavailable; descriptor and health reads never perform remote I/O.</p>
 *
 * <p>A valid {@code REVOKED} publication is a successful refresh that deliberately closes secret
 * resolution. It is not collapsed into a transport failure. The maximum source age also closes
 * resolution if the scheduler stalls while all signed documents are still cryptographically
 * valid.</p>
 */
public final class DynamicTestSecretAuthorityServingInventoryAuthority
        implements TestSecretAuthorityServingInventoryAuthority, AutoCloseable {

    /** Version-negotiated media type required from the remote publication endpoint. */
    public static final String MEDIA_TYPE =
            "application/vnd.bloge.test-secret-authority-serving-inventory.v1+json";
    /** Exact response header preventing generic-JSON protocol downgrade. */
    ...
    private final List<ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey>
            authorityKeys;
    private final Map<String, ConfiguredTestSuiteStabilityServingInventoryAuthority.AuthorityKey>
            indexedAuthorityKeys;
    private final ConfiguredTestSecretAuthorityServingInventoryAuthority.ExpectedBinding binding;
    private final TestSecretAuthorityServingInventoryPublicationFloor publicationFloor;
    private final String witnessDomain;
    private final int witnessSignatureThreshold;
    private final Map<String, ConfiguredTestSuiteStabilityServingInventoryAuthority.AuthorityKey>
            indexedWitnessKeys;
    private final DynamicTestSecretAuthorityServingInventoryTrustRootAuthority managedTrustRoots;
    ...
    /**
     * Bootstraps one remote witnessed publication and starts background refresh.
     *
     ...
     * @param binding exact local scope, cohort, artifact, protocol, authority, and serving slot
     ...
     * @param settings bounded remote refresh and hard freshness policy
     */
    public DynamicTestSecretAuthorityServingInventoryAuthority(
            ObjectMapper objectMapper,
            String trustDomain,
            Set<String> acceptedPolicyFingerprints,
            int signatureThreshold,
            List<ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey>
                    authorityKeys,
            ConfiguredTestSecretAuthorityServingInventoryAuthority.ExpectedBinding binding,
            TestSecretAuthorityServingInventoryPublicationFloor publicationFloor,
            String witnessDomain,
            int witnessSignatureThreshold,
            List<ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey> witnessKeys,
            Settings settings) {
    ...
    private DynamicTestSecretAuthorityServingInventoryAuthority(
            ObjectMapper objectMapper,
            Clock clock,
            String trustDomain,
            Set<String> acceptedPolicyFingerprints,
            ...
            DocumentFetcher fetcher,
            boolean startScheduler,
            DynamicTestSecretAuthorityServingInventoryTrustRootAuthority managedTrustRoots) {
        ...
        this.indexedAuthorityKeys = indexedKeys(this.authorityKeys, signatureThreshold);
        ...
        List<ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey> witnesses =
                List.copyOf(witnessKeys == null ? List.of() : witnessKeys);
        this.indexedWitnessKeys = indexedKeys(witnesses, witnessSignatureThreshold);
        ...
        if (!this.publicationFloor.durable()) {
            throw new IllegalArgumentException(
                    "Dynamic test-secret inventory requires a durable publication floor");
        }
        if (this.trustDomain.isBlank() || this.witnessDomain.isBlank()
                || this.trustDomain.equals(this.witnessDomain)
                || !independentAuthorities(this.authorityKeys, witnesses)) {
            throw new IllegalArgumentException(
        ...
        if (!refresh() || !observation().available()) {
            throw new IllegalStateException(
                    "Dynamic test-secret inventory publication bootstrap is unavailable");
        }
        this.scheduler = startScheduler ? scheduler() : null;
    }

    /**
     * Bootstraps a witnessed inventory with restart-free, atomically published runtime keys.
     *
     * @param objectMapper application protocol mapper
     * @param acceptedPolicyFingerprints accepted inventory/publication policy revisions
     * @param binding exact local deployment and authority binding
     * @param publicationFloor durable inventory publication/witness floor
     * @param managedTrustRoots atomic dual-quorum runtime-key authority
     * @param settings bounded remote inventory refresh policy
     */
    public DynamicTestSecretAuthorityServingInventoryAuthority(
            ObjectMapper objectMapper,
            Set<String> acceptedPolicyFingerprints,
    ...
    private DynamicTestSecretAuthorityServingInventoryAuthority(
        ...
        this(objectMapper, clock, material.deploymentTrustDomain(),
                ...
                material.deploymentKeys(), binding, publicationFloor,
     ...
     * @param signatureThreshold deployment-authority threshold
     * @param authorityKeysJson public deployment-authority key array
     * @param binding exact local deployment and authority binding
     * @param publicationFloor durable publication/witness floor
     * @param witnessDomain independent witness trust domain
     * @param witnessSignatureThreshold witness-authority threshold
     * @param witnessKeysJson public witness key array
     * @param settings bounded remote source policy
     * @return bootstrapped dynamic authority
     */
    public static DynamicTestSecretAuthorityServingInventoryAuthority fromJson(
            ObjectMapper objectMapper,
            String trustDomain,
        ...
        try {
            ...
            return new DynamicTestSecretAuthorityServingInventoryAuthority(
                    ...
                    publicationFloor, witnessDomain, witnessSignatureThreshold,
                    parseKeys(strict, witnessKeysJson), settings);
        } catch (RuntimeException | IOException
                 | java.security.GeneralSecurityException invalid) {
            throw new IllegalArgumentException(
                    "Dynamic test-secret inventory trust configuration is invalid", invalid);
        }
    }
    ...
    public Observation observation() {
        ...
        if (observed.inventoryAuthority() == null) {
            throw new IllegalStateException(
                    "Test-secret inventory publication is not bootstrapped");
        }
        Observation inventory = observed.inventoryAuthority().observation();
        DynamicTestSecretAuthorityServingInventoryTrustRootAuthority.Snapshot rootSnapshot =
    ...
    public Descriptor descriptor() {
        ...
        return new Descriptor(Descriptor.SCHEMA_VERSION, true, true,
                ...
                Map.ofEntries(
                        ...
                        Map.entry("signatureThreshold",
                                currentDeploymentSignatureThreshold()),
                        Map.entry("runtimeExpiryFence", true),
                        Map.entry("authorityIdentityBound", true),
    ...
    /** @return aggregate process-local refresh state for health and fixed-cardinality telemetry */
    public Snapshot snapshot() {
        ...
        return new Snapshot(Snapshot.SCHEMA_VERSION, inventory.available(),
                effectiveRefreshState(observed), publicationState, sequence,
    ...
    private boolean refresh() {
        synchronized (refreshLock) {
            ...
            try {
                ...
                ConfiguredTestSecretAuthorityServingInventoryAuthority inventoryAuthority;
                String trustRootGenerationFingerprint;
                if (fetched.notModified()) {
                    if (previous.publication() == null
                            || previous.inventoryAuthority() == null) {
                        throw new IllegalArgumentException(
                                "Test-secret inventory source returned 304 before bootstrap");
                    }
                ...
                state = new RefreshState(publication, inventoryAuthority,
                        ...
                        LocalState.HEALTHY, now, previous.refreshSuccessCount() + 1,
                ...
                if (refreshFailureLogged.compareAndSet(false, true)) {
                    log.warn("Dynamic test-secret serving-inventory refresh failed; "
    ...
    private TestSecretAuthorityServingInventoryPublication parse(byte[] body) {
        try {
            return objectMapper.readValue(
                    body, TestSecretAuthorityServingInventoryPublication.class);
        } catch (IOException invalid) {
            throw new IllegalArgumentException(
                    "Test-secret serving-inventory publication JSON is invalid", invalid);
        }
    }
    ...
    private VerifiedPublication verify(
        ...
        if (!publication.fingerprintVerified(objectMapper)
            ...
            throw new IllegalArgumentException(
                    "Test-secret inventory publication identity or policy is invalid");
        }
        requireCurrentWindow(publication.material().issuedAt(),
                publication.material().notBefore(), publication.material().expiresAt(), now,
                "Test-secret inventory publication");
        verifySignatures(verificationTrust.deploymentKeys(),
                verificationTrust.deploymentSignatureThreshold(),
                publication.signatures(), publication.materialFingerprint(),
                publication.material().issuedAt(), publication.material().expiresAt(), now,
                "Test-secret inventory publication");

        TestSecretAuthorityServingInventoryPublication.WitnessMaterial witness =
        ...
        if (!verificationTrust.witnessTrustDomain().equals(witness.witnessDomain())
            ...
            throw new IllegalArgumentException(
                    "Test-secret inventory witness binding is invalid");
        }
        requireCurrentWindow(witness.issuedAt(), witness.notBefore(), witness.expiresAt(), now,
                "Test-secret inventory witness");
        verifySignatures(verificationTrust.witnessKeys(),
                ...
                witness.expiresAt(), now, "Test-secret inventory witness");
        requireSuccessor(publication, previous.publication());

        ConfiguredTestSecretAuthorityServingInventoryAuthority inventoryAuthority =
                new ConfiguredTestSecretAuthorityServingInventoryAuthority(
                        objectMapper, clock, verificationTrust.deploymentTrustDomain(),
                        acceptedPolicyFingerprints,
                        verificationTrust.deploymentSignatureThreshold(),
                        verificationTrust.deploymentKeyList(),
                        publication.inventory(), binding);
    ...
    private VerificationTrust verificationTrust(
        ...
        return new VerificationTrust(keys.deploymentTrustDomain(),
                ...
                keys.deploymentKeys().values().stream()
                        .map(DynamicTestSecretAuthorityServingInventoryAuthority::secretKey)
                        .toList(), keys.generationFingerprint());
    }
    ...
    private TestSecretAuthorityServingInventoryPublicationFloor.Generation
            publicationGeneration(TestSecretAuthorityServingInventoryPublication publication) {
    ...
    private String sourceGenerationFingerprint(RefreshState observed) {
        if (observed.trustRootGenerationFingerprint().isBlank()) {
            return ProtocolFingerprint.of(objectMapper, Map.of(
                    "schemaVersion",
                    "bloge.testSecretAuthorityServingInventoryPublicationHead.v1",
        ...
        return ProtocolFingerprint.of(objectMapper, Map.of(
                "schemaVersion",
                "bloge.testSecretAuthorityServingInventoryManagedGeneration.v1",
                "publicationMaterialFingerprint",
                observed.publication().materialFingerprint(),
                "witnessMaterialFingerprint",
                observed.publication().witness().materialFingerprint(),
    ...
    private static void requireSuccessor(
            TestSecretAuthorityServingInventoryPublication next,
            TestSecretAuthorityServingInventoryPublication previous) {
        if (previous == null) {
            return;
        }
        ...
        if (nextInventoryRevision < previousInventoryRevision
            ...
            throw new IllegalArgumentException(
                    "Test-secret inventory revision is rolled back or forked");
        }
        if (nextSequence == previousSequence) {
            if (!next.materialFingerprint().equals(previous.materialFingerprint())
                    ...
                    previous.inventory().materialFingerprint())) {
                throw new IllegalArgumentException(
                        "Test-secret inventory publication sequence is forked");
            }
            return;
        }
        if (nextSequence != previousSequence + 1
                ...
                previous.witness().materialFingerprint())) {
            throw new IllegalArgumentException(
                    "Test-secret inventory publication chain is rolled back or discontinuous");
        }
    }
    ...
    private ScheduledThreadPoolExecutor scheduler() {
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1, task -> {
            Thread thread = new Thread(task,
                    "resource-gateway-test-secret-inventory-refresh");
    ...
    public record Settings(
        ...
        public Settings validated() {
            ...
            if (maximumAge.compareTo(refresh.plus(timeout)) < 0) {
                throw new IllegalArgumentException(
                        "Test-secret inventory snapshot age must cover refresh plus timeout");
            }
        ...
        private static void validateUri(URI uri, boolean allowInsecureLoopback) {
            if (uri == null || uri.getHost() == null || uri.getUserInfo() != null
                ...
                throw new IllegalArgumentException(
                        "A valid test-secret inventory publication URI is required");
            }
            ...
            if (!allowInsecureLoopback || !loopback
                ...
                throw new IllegalArgumentException(
                        "Test-secret inventory publication source must use HTTPS");
            }
        }
    }
    ...
    record FetchedDocument(byte[] body, String etag, boolean notModified) {
        FetchedDocument {
            ...
            if (etag.length() > 512 || etag.chars().anyMatch(Character::isISOControl)
                ...
                throw new IllegalArgumentException(
                        "Invalid test-secret inventory publication response");
            }
        }
    ...
    private static final class HttpDocumentFetcher implements DocumentFetcher {
            ...
            try {
                ...
                if (response.statusCode() != 200) {
                    ...
                    throw new RemotePublicationUnavailableException(
                            "Test-secret inventory source returned a non-success status");
                }
                requireProtocolHeaders(response);
                ...
                if (declaredLength > MAXIMUM_DOCUMENT_BYTES) {
                    response.body().close();
                    throw new IllegalArgumentException(
                            "Test-secret inventory publication is too large");
                }
                byte[] body;
                ...
                Thread.currentThread().interrupt();
                throw new RemotePublicationUnavailableException(
                        "Test-secret inventory request was interrupted", interrupted);
            } catch (IOException unavailable) {
                throw new RemotePublicationUnavailableException(
                        "Test-secret inventory request failed", unavailable);
            }
        }
        ...
        private static void requireProtocolHeaders(HttpResponse<?> response) {
            ...
            if (!exactMediaType
                    || !TestSecretAuthorityServingInventoryPublication.SCHEMA_VERSION.equals(
                    protocol)) {
                try {
                ...
                throw new IllegalArgumentException(
                        "Test-secret inventory source protocol negotiation failed");
            }
        }
    }
     ...
     * @param available whether the current publication may authorize secret resolution
    ...
    public record Snapshot(
        ...
        public Snapshot {
            ...
            if (!SCHEMA_VERSION.equals(schemaVersion)
                ...
                throw new IllegalArgumentException(
                        "Invalid test-secret inventory refresh snapshot");
            }
        }
    }
    ...
    private static Map<String, ConfiguredTestSuiteStabilityServingInventoryAuthority.AuthorityKey>
        ...
        return ConfiguredTestSuiteStabilityServingInventoryAuthority.indexedKeys(
                keys.stream().map(DynamicTestSecretAuthorityServingInventoryAuthority
                        ::suiteKey).toList(), threshold);
    }

    private static List<ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey>
            parseKeys(ObjectMapper objectMapper, String json)
            throws IOException, java.security.GeneralSecurityException {
        return ConfiguredTestSuiteStabilityServingInventoryAuthority.parseKeys(
                        objectMapper, json).stream()
                .map(key -> new ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey(
                        key.authorityId(), key.keyId(), key.publicKey(), key.notBefore(),
                        key.expiresAt(), key.enabled(), key.revoked()))
                .toList();
    }

    private static ConfiguredTestSuiteStabilityServingInventoryAuthority.AuthorityKey suiteKey(
            ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey key) {
    ...
    private static void verifySignatures(
            ...
            Instant observedAt,
            String label) {
        List<TestSuiteStabilityServingInventory.AuthoritySignature> translated =
                signatures.stream().map(signature ->
                        new TestSuiteStabilityServingInventory.AuthoritySignature(
                                signature.authorityId(), signature.keyId(),
                                signature.algorithm(), signature.signedAt(),
                                signature.signature())).toList();
        ConfiguredTestSuiteStabilityServingInventoryAuthority.verifyDetachedSignatures(
                keys, threshold, translated, fingerprint, issuedAt, expiresAt, observedAt, label);
    }
    ...
    private static boolean independentAuthorities(
        ...
        for (ConfiguredTestSecretAuthorityServingInventoryAuthority.AuthorityKey key
                : deploymentKeys) {
    ...
    private static ManagedMaterial managedMaterial(
            DynamicTestSecretAuthorityServingInventoryTrustRootAuthority roots) {
        var keys = Objects.requireNonNull(roots, "roots").keysFor(List.of(), List.of());
        return new ManagedMaterial(keys.deploymentTrustDomain(), keys.witnessTrustDomain(),
                keys.deploymentSignatureThreshold(), keys.witnessSignatureThreshold(),
                keys.deploymentKeys().values().stream()
                        .map(DynamicTestSecretAuthorityServingInventoryAuthority::secretKey)
                        .toList(),
                keys.witnessKeys().values().stream()
                        .map(DynamicTestSecretAuthorityServingInventoryAuthority::secretKey)
                        .toList());
    }
    ...
    private static Duration bounded(
        ...
        if (result.compareTo(minimum) < 0 || result.compareTo(maximum) > 0) {
            throw new IllegalArgumentException("Invalid test-secret inventory " + label);
        }
        return result;
    }
