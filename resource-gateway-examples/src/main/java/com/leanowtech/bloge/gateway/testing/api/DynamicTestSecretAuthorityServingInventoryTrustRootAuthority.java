 ...
 */
public final class DynamicTestSecretAuthorityServingInventoryTrustRootAuthority
    ...
    private final ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority.ExpectedBinding
            binding;
    ...
    private final List<ConfiguredTestSuiteStabilityServingInventoryAuthority.AuthorityKey>
            witnessRootKeys;
    private final TestSecretAuthorityServingInventoryTrustRootFloor floor;
     ...
     */
    public DynamicTestSecretAuthorityServingInventoryTrustRootAuthority(
            ObjectMapper objectMapper,
            ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority.ExpectedBinding binding,
    ...
    DynamicTestSecretAuthorityServingInventoryTrustRootAuthority(
        ...
        if (!refresh() || !usable(state, clock.instant())) {
            throw new IllegalStateException(
                    "Dynamic test-secret serving-inventory trust-root bootstrap is unavailable");
        }
     ...
     */
    public static DynamicTestSecretAuthorityServingInventoryTrustRootAuthority fromJson(
            ObjectMapper objectMapper,
            ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority.ExpectedBinding binding,
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Dynamic test-secret serving-inventory trust-root configuration is invalid", invalid);
        }
    }
     ...
     */
    ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority.VerifiedKeySet keysFor(
            List<TestSecretAuthorityServingInventory.AuthoritySignature> deploymentSignatures,
            List<TestSecretAuthorityServingInventory.AuthoritySignature> witnessSignatures) {
        ...
        if (!usable(observed, clock.instant())) {
            throw new IllegalStateException(
                    "Dynamic test-secret serving-inventory trust-root snapshot is unavailable");
        }
        ...
        try {
            ...
            if (document.notModified()) {
                ...
                TestSecretAuthorityServingInventoryTrustRootPublication publication =
                        ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority
            ...
            if (refreshFailureLogged.compareAndSet(false, true)) {
                log.warn("Dynamic test-secret serving-inventory trust-root refresh failed; "
    ...
    private static void requireSuccessor(
            TestSecretAuthorityServingInventoryTrustRootPublication previous,
            TestSecretAuthorityServingInventoryTrustRootPublication candidate) {
        if (previous == null) {
            return;
        }
        ...
        if (next == current) {
            if (!candidate.materialFingerprint().equals(previous.materialFingerprint())) {
                throw new IllegalArgumentException(
                        "Test-secret serving-inventory trust-root publication fork");
            }
            return;
        }
        if (next != current + 1
            ...
            throw new IllegalArgumentException(
                    "Test-secret serving-inventory trust-root publication chain is discontinuous");
        }
    }
    ...
    private static boolean unknownKey(
            ...
            List<TestSecretAuthorityServingInventory.AuthoritySignature> deploymentSignatures,
            List<TestSecretAuthorityServingInventory.AuthoritySignature> witnessSignatures) {
        ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority.VerifiedKeySet keys =
    ...
    private static Set<String> references(
        ...
        for (TestSecretAuthorityServingInventory.AuthoritySignature signature
                : signatures == null
                ? List.<TestSecretAuthorityServingInventory.AuthoritySignature>of()
                : signatures) {
    ...
    private ScheduledThreadPoolExecutor scheduler() {
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1, task -> {
            Thread thread = new Thread(task,
                    "resource-gateway-test-secret-inventory-trust-root-refresh");
    ...
    public record Settings(
        ...
        private static void validateUri(URI uri, boolean allowInsecureLoopback) {
            if (uri == null || uri.getHost() == null || uri.getUserInfo() != null
                ...
                throw new IllegalArgumentException(
                        "A valid test-secret serving-inventory trust-root URI is required");
            }
            ...
            if (!allowInsecureLoopback || !loopback
                ...
                throw new IllegalArgumentException(
                        "Test-secret serving-inventory trust-root source must use HTTPS");
            }
        }
    }
    ...
    record FetchedDocument(byte[] body, String etag, boolean notModified) {
        FetchedDocument {
            ...
            if (etag.length() > 512 || etag.chars().anyMatch(Character::isISOControl)
                ...
                throw new IllegalArgumentException(
                        "Invalid test-secret serving-inventory trust-root response");
            }
        }
    ...
    private static final class HttpDocumentFetcher implements DocumentFetcher {
            ...
            try {
                HttpRequest.Builder request = HttpRequest.newBuilder(uri).GET().timeout(timeout)
                        ...
                        .header(PROTOCOL_HEADER,
                                TestSecretAuthorityServingInventoryTrustRootPublication
                                        .SCHEMA_VERSION);
                if (!etag.isBlank()) {
        ...
        private static void requireProtocolHeaders(HttpResponse<?> response) {
            ...
            if (!(contentType.equals(MEDIA_TYPE) || contentType.startsWith(MEDIA_TYPE + ";"))
                ...
                throw new IllegalArgumentException(
                        "Test-secret serving-inventory trust-root protocol negotiation failed");
            }
        }
    }
    ...
    private record State(
            TestSecretAuthorityServingInventoryTrustRootPublication publication,
            ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority authority,
            String etag,
        ...
        private State succeeded(
                TestSecretAuthorityServingInventoryTrustRootPublication nextPublication,
                ConfiguredTestSecretAuthorityServingInventoryTrustRootAuthority nextAuthority,
                String nextEtag,
                Instant observedAt) {
    ...
    public record Snapshot(
        ...
        public Snapshot {
            ...
            if (!SCHEMA_VERSION.equals(schemaVersion) || status.isBlank() || sequence < 0
                ...
                throw new IllegalArgumentException(
                        "Dynamic test-secret serving-inventory trust-root snapshot is invalid");
            }
        }
    }
