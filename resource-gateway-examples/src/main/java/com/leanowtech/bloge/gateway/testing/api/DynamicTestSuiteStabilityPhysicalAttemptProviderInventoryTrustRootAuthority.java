...
/**
 * Atomically refreshes dual-quorum signed physical provider-inventory runtime verification keys.
 *
 ...
 */
public final class DynamicTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority
    ...
    private final ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority.ExpectedBinding
            binding;
    ...
    private final List<ConfiguredTestSuiteStabilityServingInventoryAuthority.AuthorityKey>
            witnessRootKeys;
    private final TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootFloor floor;
     ...
     */
    public DynamicTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority(
            ObjectMapper objectMapper,
            ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority.ExpectedBinding binding,
    ...
    DynamicTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority(
        ...
        if (!this.floor.durable()) {
            throw new IllegalArgumentException(
                    "Dynamic physical provider-inventory trust roots require a durable floor");
        }
        ...
        if (!refresh() || !usable(state, clock.instant())) {
            throw new IllegalStateException(
                    "Dynamic physical provider-inventory trust-root bootstrap is unavailable");
        }
     ...
     */
    public static DynamicTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority fromJson(
            ObjectMapper objectMapper,
            ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority.ExpectedBinding binding,
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Dynamic physical provider-inventory trust-root configuration is invalid", invalid);
        }
    }
     ...
     */
    ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority.VerifiedKeySet keysFor(
        ...
        if (!usable(observed, clock.instant())) {
            throw new IllegalStateException(
                    "Dynamic physical provider-inventory trust-root snapshot is unavailable");
        }
     ...
     *
     * @return current aggregate refresh and availability snapshot
     */
    public Snapshot snapshot() {
        State observed = state;
        Instant now = clock.instant();
        ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority.Snapshot rootSnapshot =
    ...
    private boolean refreshLocked(Instant now, State previous) {
        try {
            ...
            if (document.notModified()) {
                ...
                var authority =
                        new ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority(
            ...
            if (refreshFailureLogged.compareAndSet(false, true)) {
                log.warn("Dynamic physical provider-inventory trust-root refresh failed; "
    ...
    private static void requireSuccessor(
            TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootPublication previous,
            TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootPublication candidate) {
        if (previous == null) {
            return;
        }
        ...
        if (next == current) {
            if (!candidate.materialFingerprint().equals(previous.materialFingerprint())) {
                throw new IllegalArgumentException(
                        "Physical provider-inventory trust-root publication fork");
            }
            return;
        }
        if (next != current + 1
            ...
            throw new IllegalArgumentException(
                    "Physical provider-inventory trust-root publication chain is discontinuous");
        }
    }
    ...
    private static boolean unknownKey(
            ...
            List<TestSuiteStabilityServingInventory.AuthoritySignature> deploymentSignatures,
            List<TestSuiteStabilityServingInventory.AuthoritySignature> witnessSignatures) {
        ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority.VerifiedKeySet keys =
    ...
    private ScheduledThreadPoolExecutor scheduler() {
        ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(1, task -> {
            Thread thread = new Thread(task,
                    "resource-gateway-physical-provider-inventory-trust-root-refresh");
     ...
     *
     * @param publicationUri managed publication endpoint
     * @param refreshInterval periodic background refresh interval
     * @param requestTimeout per-request wall-clock timeout
     * @param unknownKeyRefreshInterval cooldown between unknown-key synchronous refreshes
     * @param maximumSnapshotAge hard local age after which the snapshot becomes unusable
     * @param allowInsecureLoopback permits HTTP loopback only for isolated tests
     */
    public record Settings(
        ...
        /**
         * Validates and normalizes all transport and timing bounds.
         *
        ...
        private static void validateUri(URI uri, boolean allowInsecureLoopback) {
            if (uri == null || uri.getHost() == null || uri.getUserInfo() != null
                ...
                throw new IllegalArgumentException(
                        "A valid physical provider-inventory trust-root URI is required");
            }
            ...
            if (!allowInsecureLoopback || !loopback
                ...
                throw new IllegalArgumentException(
                        "Physical provider-inventory trust-root source must use HTTPS");
            }
        }
    }
    ...
    record FetchedDocument(byte[] body, String etag, boolean notModified) {
        FetchedDocument {
            ...
            if (etag.length() > 512 || etag.chars().anyMatch(Character::isISOControl)
                ...
                throw new IllegalArgumentException(
                        "Invalid physical provider-inventory trust-root response");
            }
        }
    ...
    private static final class HttpDocumentFetcher implements DocumentFetcher {
        ...
        private static void requireProtocolHeaders(HttpResponse<?> response) {
            ...
            if (!(contentType.equals(MEDIA_TYPE) || contentType.startsWith(MEDIA_TYPE + ";"))
                ...
                throw new IllegalArgumentException(
                        "Physical provider-inventory trust-root protocol negotiation failed");
            }
        }
    }
    ...
    private record State(
            TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootPublication publication,
            ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority authority,
            String etag,
        ...
        private State succeeded(
                TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootPublication nextPublication,
                ConfiguredTestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootAuthority nextAuthority,
                String nextEtag,
                Instant observedAt) {
    ...
    public record Snapshot(
        ...
        public Snapshot {
            ...
            if (!SCHEMA_VERSION.equals(schemaVersion) || status.isBlank() || sequence < 0
                ...
                throw new IllegalArgumentException(
                        "Dynamic physical provider-inventory trust-root snapshot is invalid");
            }
        }
    }
