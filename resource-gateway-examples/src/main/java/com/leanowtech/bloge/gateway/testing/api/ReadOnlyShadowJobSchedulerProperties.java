...
/**
 * Strict process-local configuration for autonomous durable Shadow polling.
 *
 * <p>Scheduling is disabled by default and may target only one exact partition in the physically
 * non-production Mirror runtime. Reserved production aliases are rejected as defense in depth;
 * arbitrary enterprise non-production environment identifiers remain valid. Database policy
 * stays authoritative for cross-replica uniqueness, leases, retries, deadlines, and sample
 * ordinals.</p>
 *
...
public record ReadOnlyShadowJobSchedulerProperties(
        Boolean enabled,
        String instanceId,
        String region,
    ...
    /** Configuration prefix shared by application configuration and capability documentation. */
    public static final String PREFIX =
            "gateway.testing.mirror.shadow-job.scheduler";
    ...
    public ReadOnlyShadowJobSchedulerProperties {
        ...
        if (enabled) {
            ...
            if (RESERVED_PRODUCTION_ENVIRONMENTS
                ...
                throw new IllegalArgumentException(
                        "Enabled Shadow scheduler cannot target a reserved production environment");
            }
        }
    }
