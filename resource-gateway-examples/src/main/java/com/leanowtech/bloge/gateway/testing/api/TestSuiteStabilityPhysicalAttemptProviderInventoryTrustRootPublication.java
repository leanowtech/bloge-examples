...
/**
 * Dual-quorum signed publication of physical provider-inventory publication and witness verification keys.
 *
 ...
 */
public record TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootPublication(
        String schemaVersion,
        Material material,
        String materialFingerprint,
        List<TestSuiteStabilityServingInventory.AuthoritySignature> deploymentRootSignatures,
        List<TestSuiteStabilityServingInventory.AuthoritySignature> witnessRootSignatures) {
    ...
    public TestSuiteStabilityPhysicalAttemptProviderInventoryTrustRootPublication {
        ...
        if (!SCHEMA_VERSION.equals(schemaVersion)
            ...
            throw new IllegalArgumentException(
                    "Physical provider-inventory trust-root publication envelope is invalid");
        }
    }

    /**
     * Recomputes and compares the canonical signed-material identity.
     *
    ...
    public record Material(
        ...
        public Material {
            ...
            if (!SCHEMA_VERSION.equals(schemaVersion)
                ...
                throw new IllegalArgumentException(
                        "Physical provider-inventory trust-root publication material is invalid");
            }
        }
    }
    ...
    public record AuthorityKeyMaterial(
        ...
        public AuthorityKeyMaterial {
            ...
            if (!IDENTIFIER.matcher(authorityId).matches()
                ...
                throw new IllegalArgumentException(
                        "Physical provider-inventory trust-root authority key is invalid");
            }
        }
    ...
    private static List<TestSuiteStabilityServingInventory.AuthoritySignature>
        ...
        if (result.isEmpty() || result.size() > 32 || !result.equals(sorted)
            ...
            throw new IllegalArgumentException(
                    "Physical provider-inventory trust-root signatures are invalid");
        }
        return result;
    }
    ...
    private static List<AuthorityKeyMaterial> immutableKeys(List<AuthorityKeyMaterial> values) {
        ...
        if (result.isEmpty() || result.size() > 64 || !result.equals(sorted)
            ...
            throw new IllegalArgumentException(
                    "Physical provider-inventory trust-root key set is invalid");
        }
        return result;
    }
