...
public final class ScenarioPublicationService {
    ...
    public ScenarioPublicationService(
        ...
        this(scenarioDrafts, publications, graphDrafts, contracts, compiler, registry,
    ...
    public StoredScenarioPublication publish(
        ...
        GraphDraft graph = currentGraph(source.draftSet(), identity);
        String visualTargetFingerprint = VisualBundleFingerprint.fromCanonicalValue(
                objectMapper, graph, MAX_TARGET_BYTES);
        ContractDraft contract = contracts.project(graph, visualTargetFingerprint);
        ...
        ScenarioGovernedCompilationPlan plan =
                compiler.compile(graph, contract, source.draftSet(), runtimeTarget);
    ...
    private GraphDraft currentGraph(
            ScenarioDraftSet draftSet,
            IntegrationRequestContext identity) {
        if (draftSet.target().kind() != ContractDraft.TargetKind.GRAPH) {
            throw badRequest(identity, "RG.SCENARIO.PUBLICATION_TARGET_UNSUPPORTED",
                    "This publication version supports graph targets only.", Map.of());
        }
