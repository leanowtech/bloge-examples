-- Resource Gateway Business Mirror Capability Proposal authoring v1.
-- Apply before advertising businessMirrorProposalApi.
...
CREATE TABLE IF NOT EXISTS business_mirror_proposal_draft_revisions (
    ...
    stored_json TEXT NOT NULL,
    PRIMARY KEY (
        tenant_id, organization_id, project_id, environment_id, region_id, proposal_id, revision
    )
);
...
CREATE INDEX IF NOT EXISTS business_mirror_proposal_history_lookup_idx
    ON business_mirror_proposal_draft_revisions (
        tenant_id, organization_id, project_id, environment_id, region_id,
        proposal_id, revision DESC
    );

CREATE INDEX IF NOT EXISTS business_mirror_proposal_receipt_completed_idx
    ON business_mirror_proposal_save_receipts (
