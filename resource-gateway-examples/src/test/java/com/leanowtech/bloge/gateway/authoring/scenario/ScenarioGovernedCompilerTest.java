...
class ScenarioGovernedCompilerTest {
    ...
    @Test
    void compilesAnOperatorScenarioAgainstAnExactOperatorRuntimeTarget() {
        ContractDraft contract =
                new ContractDraftProjectionService().project(ScenarioOperatorTestSupport.operator());
        ScenarioDraftSet.ScenarioDraft scenario = new ScenarioDraftSet.ScenarioDraft(
                ...
                List.of(),
                new ScenarioDraftSet.Then(List.of(assertion(
                        "decision",
        ...
        ScenarioGovernedCompilationPlan plan =
                compiler.compile(null, contract, draftSet, runtimeTarget);

        assertThat(plan.compiled()).isTrue();
        assertThat(plan.runtimeTarget()).isEqualTo(runtimeTarget);
        assertThat(plan.fixtures()).singleElement().satisfies(fixture ->
                assertThat(fixture.request().target().kind()).isEqualTo("OPERATOR"));
        assertThat(plan.suite().testSuite().target().kind()).isEqualTo("OPERATOR");
    }

    @Test
    void lowersVirtualResourceInputAndPreservesDesignAndRuntimeLineage() {
        var operator = ScenarioOperatorTestSupport.resourceOperator();
        ContractDraft contract = new ContractDraftProjectionService().project(operator);
        ScenarioDraftSet.ScenarioDraft scenario = new ScenarioDraftSet.ScenarioDraft(
                "profile",
                "Load user profile",
                "",
                ScenarioDraftSet.CaseType.GOLDEN,
                List.of("operator", "resource"),
                new ScenarioDraftSet.Given(
        ...
        TestExecutionApiRequest.Target runtimeTarget = new TestExecutionApiRequest.Target(
                "OPERATOR",
                ScenarioOperatorTestSupport.RESOURCE_RUNTIME_REF,
                ScenarioValidationServiceTest.fingerprint('d'));

        ScenarioGovernedCompilationPlan plan =
                compiler.compile(null, operator, contract, draftSet, runtimeTarget);

        assertThat(plan.compiled()).isTrue();
        TestSuite suite = (TestSuite) plan.suite().testSuite();
        assertThat(suite.target().id())
                .isEqualTo(ScenarioOperatorTestSupport.RESOURCE_RUNTIME_REF);
        assertThat(suite.cases().getFirst().input()).isEqualTo(Map.of(
                "resourceId", "user-service.getProfile",
                "params", Map.of("userId", "user-42")));
        assertThat(suite.metadata())
                .containsEntry("sourceTargetId", ScenarioOperatorTestSupport.RESOURCE_OPERATOR_REF)
                .containsEntry("runtimeTargetId", ScenarioOperatorTestSupport.RESOURCE_RUNTIME_REF)
                .containsEntry("loweringMode", "resource-descriptor");
    ...
    @Test
    void rejectsARegistryTargetThatDisagreesWithCatalogLowering() {
        var operator = ScenarioOperatorTestSupport.resourceOperator();
        ContractDraft contract = new ContractDraftProjectionService().project(operator);
        ScenarioDraftSet draftSet = new ScenarioDraftSet(
                "",
        ...
        assertThat(plan.compiled()).isFalse();
        assertThat(plan.diagnostics()).extracting("code")
                .contains("visual.scenario.compile.runtimeTargetOperatorMismatch");
    }
