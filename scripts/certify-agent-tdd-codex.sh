...
先了解平台已经具备哪些业务积木，并查找“取消责任方”是否已有含义完全一致、当前可用的业务事实。业务人员也会把这项事实叫作“取消归责”“谁导致取消”或“谁造成了取消”。如果没有，就把它定义为由客服人员在当前争议订单中确认的事实，可能值只有“乘客”和“司机”；无法确认时必须转人工复核。
...
如果平台指出定义不完整或前后不一致，请根据平台反馈修正并继续；两条标准案例成功提交并从待确认清单读回之前，不要结束本次工作。
